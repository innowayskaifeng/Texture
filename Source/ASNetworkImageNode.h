...
/**
 * ASNetworkImageNode is a simple image node that can download and display an image from the
 * network, with support for a placeholder image (<defaultImage>).  The currently-displayed image is
 * always available in the inherited ASImageNode <image> property.
 *
 ...
 * @param downloader The object that implements image downloading for the image node.  Must not be
 * nil.  Weak reference.
 *
 ...
 *
 * @return An ASNetworkImageNode configured to use the NSURLSession-powered ASBasicImageDownloader,
 * and no extra cache.
 */
- (instancetype)init;
 ...
 *
 * @discussion By setting an image to the image property the ASNetworkImageNode will act like a
 * plain ASImageNode. As soon as the URL is set the ASNetworkImageNode will act like an
 * ASNetworkImageNode and the image property will be managed internally. This means the image
 * property will be cleared out and replaced by the placeholder
 * (<defaultImage>) image while loading and the final image after the new image data was downloaded
 * and processed. If you want to use a placholder image functionality use the defaultImage property
 * instead.
 */
@property(nullable) UIImage *image;

/**
 * A placeholder image to display while the URL is loading. This is slightly different than
 * placeholderImage in the ASDisplayNode superclass as defaultImage will *not* be displayed
 * synchronously. If you wish to have the image displayed synchronously, use @c placeholderImage.
 */
 ...
 *
 * @discussion By setting an URL, the image property of this node will be managed internally. This
 * means previously directly set images to the image property will be cleared out and replaced by
 * the placeholder (<defaultImage>) image while loading and the final image after the new image data
 ...
 *
 * @discussion By setting an array of URLs, the image property of this node will be managed
 * internally. This means previously directly set images to the image property will be cleared out
 * and replaced by the placeholder (<defaultImage>) image while loading and the final image after
 * the new image data was downloaded and processed.
 *
 * @deprecated This API has been removed for now due to the increased complexity to the class that
 * it brought. Please use .URL instead.
 */
@property(nullable, copy)
    NSArray<NSURL *> *URLs ASDISPLAYNODE_DEPRECATED_MSG("Please use URL instead.");
...
/**
 * If <URL> is a local file, set this property to YES to take advantage of UIKit's image caching.
 * Defaults to YES.
 */
@property BOOL shouldCacheImage;

/**
 * If the downloader implements progressive image rendering and this value is YES progressive
 * renders of the image will be displayed as the image downloads. Regardless of this properties
 * value, progress renders will only occur when the node is visible. Defaults to YES.
 */
...
/**
 * The methods declared by the ASNetworkImageNodeDelegate protocol allow the adopting delegate to
 * respond to notifications such as finished decoding and downloading an image.
 */
 ...
 *
 * @discussion Called on the main thread if useMainThreadDelegateCallbacks=YES (the default),
 * otherwise on a background thread.
 */
- (void)imageNode:(ASNetworkImageNode *)imageNode
     didLoadImage:(UIImage *)image
             info:(ASNetworkImageLoadInfo *)info;
