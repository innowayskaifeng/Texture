...
AS_EXTERN void ASPerformBlockOnBackgroundThread(
    void (^block)(void));  // DISPATCH_QUEUE_PRIORITY_DEFAULT

#if ASEVENTLOG_ENABLE
#define ASDisplayNodeLogEvent(node, ...)                                                   \
  [node.eventLog                                                                           \
      logEventWithBacktrace:(AS_SAVE_EVENT_BACKTRACES ? [NSThread callStackSymbols] : nil) \
                     format:__VA_ARGS__]
#else
#define ASDisplayNodeLogEvent(node, ...)
#endif
...
+ (BOOL)suppressesInvalidCollectionUpdateExceptions AS_WARN_UNUSED_RESULT
    ASDISPLAYNODE_DEPRECATED_MSG(
        "Collection update exceptions are thrown if assertions are enabled.");
 ...
 *
 * @discussion Set the block to be called after the context has been created and before the node's
 * content is drawn. You can override this to modify the context before the content is drawn. You
 * are responsible for saving and restoring context if necessary. Restoring can be done in
 * contextDidDisplayNodeContent This block can be called from *any* thread and it is unsafe to
 * access any UIKit main thread properties from it.
 */
...
/**
 * @abstract A bitmask representing which actions (layout spec, layout generation) should be
 * measured.
 */
 ...
 * accessibility label will represent an aggregation of all child nodes' accessibility labels. Nodes
 * in this node's subtree that are also accessibility containers will not be included in this
...
/**
 * @abstract Invoked when a user performs a custom action on an accessible node. Nodes that are
 * children of accessibility containers, have an accessibity label and have an interactive
 * UIAccessibilityTrait will automatically receive custom-action handling.
 *
...
/**
 * @abstract Currently used by ASNetworkImageNode and ASMultiplexImageNode to allow their
 * placeholders to stay if they are loading an image from the network. Otherwise, a display pass is
 * scheduled and completes, but does not actually draw anything - and ASDisplayNode considers the
 * element finished.
 */
- (BOOL)placeholderShouldPersist AS_WARN_UNUSED_RESULT;

/**
 * @abstract Indicates that the receiver and all subnodes have finished displaying. May be called
 * more than once, for example if the receiver has a network image node. This is called after the
 * first display pass even if network image nodes have not downloaded anything (text would be done,
 * and other nodes that are ready to do their final display). Each render of every progressive jpeg
 * network node would cause this to be called, so this hook could be called up to 1 + (pJPEGcount *
 * pJPEGrenderCount) times. The render count depends on how many times the downloader calls the
 * progressImage block.
 */
 ...
 * ASLayoutRangeModeVisibleOnly, because this is the only way to ensure an application will not have
 * blank / flashing views as the user navigates back after a memory warning.  Apps that wish to use
 * the more effective / aggressive ASLayoutRangeModeLowMemory may need to take steps to mitigate
 * this behavior, including: restoring a larger range mode to the next controller before the user
 * navigates there, enabling .neverShowPlaceholders on ASCellNodes so that the navigation operation
 * is blocked on redisplay completing, etc.
 */
 ...
 *
 * @discussion
 * When called, causes all descendent nodes' contents to be drawn directly into this node's layer's
 * backing store.
 *
 * If a node's descendants are static (never animated or never change attributes after creation)
 * then that node is a good candidate for rasterization.  Rasterizing descendants has two main
 * benefits: 1) Backing stores for descendant layers are not created.  Instead the layers are drawn
 * directly into the rasterized container.  This can save a great deal of memory. 2) Since the
 * entire subtree is drawn into one backing store, compositing and blending are eliminated in that
 * subtree which can help improve animation/scrolling/etc performance.
 *
 * Rasterization does not currently support descendants with transform, sublayerTransform, or alpha.
 * Those properties will be ignored when rasterizing descendants.
 *
 * Note: this has nothing to do with -[CALayer shouldRasterize], which doesn't work with
 * ASDisplayNode's asynchronous rendering model.
 *
