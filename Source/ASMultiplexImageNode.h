...
typedef NS_ENUM(NSUInteger, ASMultiplexImageNodeErrorCode) {
  ...
  /**
   * Indicates that the best image identifier changed before a download for a worse identifier
   * began.
   */
...
/**
 * @abstract ASMultiplexImageNode is an image node that can load and display multiple versions of an
 * image.  For example, it can display a low-resolution version of an image while the
 * high-resolution version is loading.
 *
 * @discussion ASMultiplexImageNode begins loading images when its resource can either return a
 * UIImage directly, or a URL the image node should load.
 */
 ...
 * @discussion If `cache` is nil, the receiver will not attempt to retrieve images from a cache
 * before downloading them.
 * @return An initialized ASMultiplexImageNode.
 */
- (instancetype)initWithCache:(nullable id<ASImageCacheProtocol>)cache
                   downloader:(nullable id<ASImageDownloaderProtocol>)downloader
...
/**
 * @abstract Whether the receiver should download more than just its highest-quality image. Defaults
 * to NO.
 *
 * @discussion ASMultiplexImageNode immediately loads and displays the first image specified in
 * <imageIdentifiers> (its highest-quality image).  If that image is not immediately available or
 * cached, the node can download and display lesser-quality images.  Set
 * `downloadsIntermediateImages` to YES to enable this behaviour.
 */
@property(nonatomic) BOOL downloadsIntermediateImages;

/**
 * @abstract An array of identifiers representing various versions of an image for
 * ASMultiplexImageNode to display.
 *
 * @discussion An identifier can be any object that conforms to NSObject and NSCopying.  The array
 * should be in decreasing order of image quality -- that is, the first identifier in the array
 * represents the best version.
 *
 ...
 * <imageIdentifiers>.
 *
 * @discussion If a higher-quality image than is currently displayed is now available, it will be
 * loaded.
 */
 ...
 *
 * @discussion This value may differ from <displayedImageIdentifier> if the image hasn't yet been
 * displayed.
 */
...
/**
 * @abstract If the downloader implements progressive image rendering and this value is YES
 * progressive renders of the image will be displayed as the image downloads. Regardless of this
 * properties value, progress renders will only occur when the node is visible. Defaults to YES.
 */
@property(nonatomic) BOOL shouldRenderProgressImages;

/**
 * @abstract The image manager that this image node should use when requesting images from the
 Photos framework. If this is `nil` (the default), then `PHImageManager.defaultManager` is used.
...
#pragma mark -
/**
 * The methods declared by the ASMultiplexImageNodeDelegate protocol allow the adopting delegate to
 * respond to notifications such as began, progressed and finished downloading, updated and
 * displayed an image.
 */
...
- (void)multiplexImageNode:(ASMultiplexImageNode *)imageNode
    didStartDownloadOfImageWithIdentifier:(id)imageIdentifier;
 ...
 * @param imageIdentifier The identifier for `image`.
 * @discussion This method is only called when `image` changes, and not on subsequent redisplays of
 * the same image.
 */
- (void)multiplexImageNode:(ASMultiplexImageNode *)imageNode
 ...
 * @param imageNode The sender.
 * @discussion This method is called every time an image is displayed, whether or not it has
 * changed.
 */
...
#pragma mark -
/**
 * The ASMultiplexImageNodeDataSource protocol is adopted by an object that provides the multiplex
 * image node, for each image identifier, an image or a URL the image node should load.
 */
 ...
 * @discussion If the image is already available to the data source, this method should be used in
 * lieu of providing the URL to the image via -multiplexImageNode:URLForImageIdentifier:.
 ...
 * @discussion Supported URLs include HTTP, HTTPS, AssetsLibrary, and FTP URLs as well as Photos
 * framework URLs (see note).
 *
 * If the image is already available to the data source, it should be provided via
 * <[ASMultiplexImageNodeDataSource multiplexImageNode:imageForImageIdentifier:]> instead.
 ...
 *
 * @discussion This optional method can improve image performance if your data source already has
 * the PHAsset available. If this method is not implemented, or returns nil, the image node will
 * request the asset from the Photos framework.
...
- (nullable PHAsset *)multiplexImageNode:(ASMultiplexImageNode *)imageNode
                 assetForLocalIdentifier:(NSString *)assetLocalIdentifier
 ...
 * created by this method and the image node will attempt to load the image from the Photos
 * framework.
 * @note The `synchronous` flag in `options` is ignored.
 * @note The `Opportunistic` delivery mode is not supported and will be treated as
 * `HighQualityFormat`.
 */
+ (NSURL *)URLWithAssetLocalIdentifier:(NSString *)assetLocalIdentifier
    ...
    AS_WARN_UNUSED_RESULT API_AVAILABLE(ios(8.0), tvos(10.0));
