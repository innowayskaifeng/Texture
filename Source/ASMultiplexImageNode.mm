...
/**
  ...
  @param error An error describing why an image couldn't be loaded, if it failed to load; nil
  otherwise.
 */
typedef void (^ASMultiplexImageLoadCompletionBlock)(UIImage *image,
                                                    id imageIdentifier,
                                                    NSError *error);
...
/**
  ...
  @result The next image identifier, from `_imageIdentifiers`, that should be downloaded, or nil if
  no image should be downloaded next.
 */
- (id)_nextImageIdentifierToDownload;

/**
  @abstract Returns the best image that is immediately available from our datasource without
  downloading or hitting the cache.
  @param imageIdentifierOut Upon return, the image identifier for the returned image; nil otherwise.
  @discussion This method exclusively uses the data source's -multiplexImageNode:imageForIdentifier:
  method to return images. It does not fetch from the cache or kick off downloading.
...
/**
  ...
  @discussion This method obeys `downloadsIntermediateImages`. This method has no effect if nothing
  further should be loaded, as indicated by `_nextImageIdentifierToDownload`. This method will load
  the next image from the data-source, if possible; otherwise, the session's image cache will be
  queried for the desired image, and as a last resort, the image will be downloaded.
 */
- (void)_loadNextImage;

/**
  @abstract Fetches the image corresponding to the given imageIdentifier from the given URL from the
  session's image cache.
  ...
  @param completionBlock The block to be performed when the image has been fetched from the cache,
  if possible. May not be nil.
  @discussion This method queries both the session's in-memory and on-disk caches (with preference
...
/**
  ...
  @param assetURL The assets-library URL (e.g., "assets-library://identifier") of the image to load,
  from ALAsset. May not be nil.
  @param completionBlock The block to be performed when the image has been loaded, if possible. May
  not be nil.
 */
...
/**
 ...
 @param completionBlock The block to be performed when the image has been downloaded, if possible.
 May not be nil.
 */
...
- (instancetype)initWithCache:(id<ASImageCacheProtocol>)cache
  ...
  _downloaderImplementsSetProgress = [downloader
      respondsToSelector:@selector(setProgressImageBlock:callbackQueue:withDownloadIdentifier:)];
  _downloaderImplementsSetPriority = [downloader respondsToSelector:@selector(setPriority:
...
- (void)didExitPreloadState {
  ...
  if (_cacheSupportsClearing && self.loadedImageIdentifier != nil) {
    NSURL *URL = [_dataSource multiplexImageNode:self
                           URLForImageIdentifier:self.loadedImageIdentifier];
    if (URL != nil) {
...
/* displayWillStartAsynchronously in ASNetworkImageNode has a very similar implementation. Changes
 here are likely necessary in ASNetworkImageNode as well. */
...
/* didEnterVisibleState / didExitVisibleState in ASNetworkImageNode has a very similar
 implementation. Changes here are likely necessary in ASNetworkImageNode as well. */
- (void)didEnterVisibleState {
  ...
  if (_downloaderImplementsSetPriority) {
    ASDN::MutexLocker l(_downloadIdentifierLock);
    if (_downloadIdentifier != nil) {
      [_downloader setPriority:ASImageDownloaderPriorityVisible
          withDownloadIdentifier:_downloadIdentifier];
    }
  }
...
- (void)didExitVisibleState {
  ...
  if (_downloaderImplementsSetPriority) {
    ASDN::MutexLocker l(_downloadIdentifierLock);
    if (_downloadIdentifier != nil) {
      [_downloader setPriority:ASImageDownloaderPriorityPreload
          withDownloadIdentifier:_downloadIdentifier];
    }
  }
...
- (void)setImage:(UIImage *)image {
  ASDisplayNodeAssert(
      NO, @"Setting the image directly on an ASMultiplexImageNode is unsafe. It will be cleared in "
...
- (void)setDelegate:(id<ASMultiplexImageNodeDelegate>)delegate {
  ...
  _delegate = delegate;
  _delegateFlags.downloadStart = [_delegate
      respondsToSelector:@selector(multiplexImageNode:didStartDownloadOfImageWithIdentifier:)];
  _delegateFlags.downloadProgress =
      [_delegate respondsToSelector:@selector(multiplexImageNode:
                                        didUpdateDownloadProgress:forImageWithIdentifier:)];
  _delegateFlags.downloadFinish =
      [_delegate respondsToSelector:@selector(multiplexImageNode:
                                        didFinishDownloadingImageWithIdentifier:error:)];
  _delegateFlags.updatedImageDisplayFinish = [_delegate
      respondsToSelector:@selector(multiplexImageNode:didDisplayUpdatedImage:withIdentifier:)];
  _delegateFlags.updatedImage = [_delegate
      respondsToSelector:@selector(multiplexImageNode:
                                       didUpdateImage:withIdentifier:fromImage:withIdentifier:)];
  _delegateFlags.displayFinish =
...
- (void)setDataSource:(id<ASMultiplexImageNodeDataSource>)dataSource {
  ...
  _dataSourceFlags.image = [_dataSource respondsToSelector:@selector(multiplexImageNode:
                                                                imageForImageIdentifier:)];
  _dataSourceFlags.URL = [_dataSource respondsToSelector:@selector(multiplexImageNode:
                                                                URLForImageIdentifier:)];
  if (AS_AVAILABLE_IOS_TVOS(9, 10)) {
    _dataSourceFlags.asset = [_dataSource respondsToSelector:@selector(multiplexImageNode:
                                                                  assetForLocalIdentifier:)];
  }
}
...
- (void)_setDisplayedImageIdentifier:(id)displayedImageIdentifier withImage:(UIImage *)image {
  ...
  // Delegateify.
  // Note that we're using the params here instead of self.image and _displayedImageIdentifier
  // because those can change before the async block below executes.
...
- (UIImage *)_bestImmediatelyAvailableImageFromDataSource:(id *)imageIdentifierOut {
  ...
  for (id imageIdentifier in _imageIdentifiers) {
    ...
    UIImage *image = ASObjectIsEqual(imageIdentifier, _loadedImageIdentifier)
                         ? existingImage
                         : [_dataSource multiplexImageNode:self
                                   imageForImageIdentifier:imageIdentifier];
    if (image) {
...
- (void)_updateProgressImageBlockOnDownloaderIfNeeded {
  ...
  if (shouldRenderProgressImages && ASInterfaceStateIncludesVisible(interfaceState)) {
    __weak __typeof__(self) weakSelf = self;
    progress =
        ^(UIImage *_Nonnull progressImage, CGFloat progress, id _Nullable downloadIdentifier) {
          __typeof__(self) strongSelf = weakSelf;
          if (strongSelf == nil) {
            return;
          }
...
- (void)_clearImage {
  ...
  BOOL shouldReleaseImageOnBackgroundThread =
      imageSize.width > kMinReleaseImageOnBackgroundSize.width ||
...
- (void)_loadNextImage {
  ...
  __weak __typeof__(self) weakSelf = self;
  ASMultiplexImageLoadCompletionBlock finishedLoadingBlock =
      ^(UIImage *image, id imageIdentifier, NSError *error) {
        __typeof__(self) strongSelf = weakSelf;
        if (!strongSelf) return;
  ...
  if (_dataSourceFlags.image) {
    UIImage *image = [_dataSource multiplexImageNode:self
                             imageForImageIdentifier:nextImageIdentifier];
    if (image) {
  ...
  NSURL *nextImageURL = (_dataSourceFlags.URL)
                            ? [_dataSource multiplexImageNode:self
                                        URLForImageIdentifier:nextImageIdentifier]
                            : nil;
  // If we fail to get a URL for the image, we have no source and can't proceed.
  if (!nextImageURL) {
    as_log_error(ASImageLoadingLog(), "Could not acquire URL %@ ident: (%@)", self,
                 nextImageIdentifier);
    ...
    [self _loadALAssetWithIdentifier:nextImageIdentifier
                          ...
                          completion:^(UIImage *downloadedImage, NSError *error) {
                            as_log_verbose(ASImageLoadingLog(),
                                           "Acquired image from assets library for %@ %@", weakSelf,
                                           nextImageIdentifier);
  ...
  if (AS_AVAILABLE_IOS_TVOS(9, 10)) {
    ...
    if (ASPhotosFrameworkImageRequest *request =
            [ASPhotosFrameworkImageRequest requestWithURL:nextImageURL]) {
      [self
          _loadPHAssetWithRequest:request
                       identifier:nextImageIdentifier
                       completion:^(UIImage *image, NSError *error) {
                         as_log_verbose(ASImageLoadingLog(), "Acquired image from Photos for %@ %@",
                                    ...
                                    _downloadImageWithIdentifier:nextImageIdentifier
                                                      ...
                                                      completion:^(UIImage *downloadedImage,
                                                        ...
                                                        if (downloadedImage) {
                                                          as_log_verbose(
                                                              ASImageLoadingLog(),
                                                              "Acquired image from download for %@ "
                                                              "id: %@ img: %@",
                                                              strongSelf, nextImageIdentifier,
                                                              downloadedImage);
                                                        } else {
                                                          as_log_error(ASImageLoadingLog(),
                                                                       ...
                                                                       "for %@ id: %@ err: %@",
                                                                       strongSelf,
                                                                       nextImageIdentifier, error);
                                                        }
                                                        finishedLoadingBlock(downloadedImage,
                                                                             nextImageIdentifier,
                                                                             error);
                                                      }];
                              }];
  ...
  [assetLibrary assetForURL:assetURL
      resultBlock:^(ALAsset *asset) {
        ...
        UIImage *downloadedImage =
            (coreGraphicsImage ? [UIImage imageWithCGImage:coreGraphicsImage] : nil);
...
- (void)_loadPHAssetWithRequest:(ASPhotosFrameworkImageRequest *)request
  ...
  /*
   * Locking rationale:
   * As of iOS 9, Photos.framework will eventually deadlock if you hit it with concurrent fetch
   * requests. rdar://22984886 Concurrent image requests are OK, but metadata requests aren't, so we
   * limit ourselves to one at a time.
   */
  ...
  NSOperation *newImageRequestOp = [NSBlockOperation blockOperationWithBlock:^{
    ...
    if (imageAsset == nil) {
      [phRequestLock lock];
      // -[PHFetchResult dealloc] plays a role in the deadlock mentioned above, so we make sure the
      // PHFetchResult is deallocated inside the critical section
      @autoreleasepool {
        imageAsset = [PHAsset fetchAssetsWithLocalIdentifiers:@[ request.assetIdentifier ]
                                                      options:nil]
                         .firstObject;
      }
    ...
    [imageManager
        requestImageForAsset:imageAsset
  ...
  // If you don't set this, iOS will sometimes infer NSQualityOfServiceUserInteractive and promote
  // the entire queue to that level, damaging system responsiveness
...
- (void)_downloadImageWithIdentifier:(id)imageIdentifier
  ...
  ASPerformBlockOnBackgroundThread(^{
    [self _setDownloadIdentifier:
              [_downloader
                  downloadImageWithURL:imageURL
                         callbackQueue:dispatch_get_main_queue()
                      downloadProgress:downloadProgressBlock
                            completion:^(id<ASImageContainerProtocol> imageContainer,
                                         NSError *error, id downloadIdentifier, id userInfo) {
                              // We dereference iVars directly, so we can't have weakSelf going nil
                              // on us.
                              __typeof__(self) strongSelf = weakSelf;
                              if (!strongSelf) return;

                              ASDN::MutexLocker l(_downloadIdentifierLock);
                              // Getting a result back for a different download identifier, download
                              // must not have been successfully canceled
                              if (ASObjectIsEqual(_downloadIdentifier, downloadIdentifier) == NO &&
                                  downloadIdentifier != nil) {
                                return;
                              }
...
#pragma mark -
- (void)_finishedLoadingImage:(UIImage *)image
                forIdentifier:(id)imageIdentifier
                        error:(NSError *)error {
  // If we failed to load, we stop the loading process.
  // Note that if we bailed before we began downloading because the best identifier changed, we
  // don't bail, but rather just begin loading the best image identifier.
  ...
  // We explicitly perform this check because our datasource often doesn't give back immediately
  // available images, even though we might have downloaded one already. Because we seed this call
  // with bestImmediatelyAvailableImageFromDataSource, we must be careful not to trample an existing
  // image.
  if (image || imageIdentifierCount == 0) {
    as_log_verbose(ASImageLoadingLog(), "[%p] loaded -> displaying (%@, %@)", self, imageIdentifier,
                   image);
