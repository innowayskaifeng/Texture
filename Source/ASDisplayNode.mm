...
#endif
// This is trying to merge non-rangeManaged with rangeManaged, so both range-managed and standalone
// nodes wait before firing their exit-visibility handlers, as UIViewController transitions now do
// rehosting at both start & end of animation. Enable this will mitigate interface updating state
// when coalescing disabled.
// TODO(wsdwsd0829): Rework enabling code to ensure that interface state behavior is not altered
// when ASCATransactionQueue is disabled.
#define ENABLE_NEW_EXIT_HIERARCHY_BEHAVIOR 0

static ASDisplayNodeNonFatalErrorBlock _nonFatalErrorBlock = nil;

// Forward declare CALayerDelegate protocol as the iOS 10 SDK moves CALayerDelegate from an informal
// delegate to a protocol. We have to forward declare the protocol as this place otherwise it will
// not compile compiling with an Base SDK < iOS 10
...
// For classes like ASTableNode, ASCollectionNode, ASScrollNode and similar - we have to be sure to
// set certain properties like setFrame: and setBackgroundColor: directly to the UIView and not
// apply it to the layer only.
...
void StubImplementationWithTwoInterfaceStates(id receiver,
                                              ASInterfaceState s0,
                                              ASInterfaceState s1) {}
 ...
 * unlikely scenario, it turns our our own internal tests use a dynamic class, so it's worth
...
static struct ASDisplayNodeFlags GetASDisplayNodeFlags(Class c, ASDisplayNode *instance) {
  ...
  flags.implementsImageDisplay =
      ([c respondsToSelector:@selector(displayWithParameters:isCancelled:)] ? 1 : 0);
  if (instance) {
    flags.implementsDrawParameters =
        ([instance respondsToSelector:@selector(drawParametersForAsyncLayer:)] ? 1 : 0);
  } else {
    flags.implementsDrawParameters =
        ([c instancesRespondToSelector:@selector(drawParametersForAsyncLayer:)] ? 1 : 0);
  }
...
+ (void)initialize {
  ...
  if (self != [ASDisplayNode class]) {
    ...
    ASDisplayNodeAssert(!ASDisplayNodeSubclassOverridesSelector(self, @selector(layoutThatFits:)),
                        @"Subclass %@ must not override layoutThatFits: method. Instead override "
                        @"calculateLayoutThatFits:.",
                        classString);
    ASDisplayNodeAssert(!ASDisplayNodeSubclassOverridesSelector(self, @selector(layoutThatFits:
                                                                                    parentSize:)),
                        @"Subclass %@ must not override layoutThatFits:parentSize method. Instead "
                        @"override calculateLayoutThatFits:.",
                        classString);
    ASDisplayNodeAssert(
        !ASDisplayNodeSubclassOverridesSelector(self, @selector(recursivelyClearContents)),
        @"Subclass %@ must not override recursivelyClearContents method.", classString);
    ASDisplayNodeAssert(
        !ASDisplayNodeSubclassOverridesSelector(self, @selector(recursivelyClearPreloadedData)),
    ...
    __block IMP originalLayoutSpecThatFitsIMP = ASReplaceMethodWithBlock(
        self, @selector(_locked_layoutElementThatFits:),
        ^(ASDisplayNode *_self, ASSizeRange sizeRange) {
          NSArray *oldSubnodes = _self.subnodes;
          ASLayoutSpec *layoutElement =
              ((ASLayoutSpec * (*)(id, SEL, ASSizeRange)) originalLayoutSpecThatFitsIMP)(
          ...
          ASDisplayNodeAssert(oldSubnodes.count == subnodes.count,
                              @"Adding or removing nodes in layoutSpecBlock or layoutSpecThatFits: "
                              @"is not allowed and can cause unexpected behavior.");
          for (NSInteger i = 0; i < oldSubnodes.count; i++) {
            ASDisplayNodeAssert(
                oldSubnodes[i] == subnodes[i],
                @"Adding or removing nodes in layoutSpecBlock or layoutSpecThatFits: is not "
                @"allowed and can cause unexpected behavior.");
          }
  ...
  // Below we are pre-calculating values per-class and dynamically adding a method
  // (_staticInitialize) to populate these values when each instance is constructed. These values
  // don't change for each class, so there is significant performance benefit in doing it here.
  // +initialize is guaranteed to be called before any instance method so it is safe to add this
  // method here. Note that we take care to detect if the class overrides +respondsToSelector: or
  // -respondsToSelector and take the slow path (recalculating for each instance) to make sure we
  // are always correct.
  ...
  IMP staticInitialize = imp_implementationWithBlock(^(ASDisplayNode *node) {
    ...
    node->_flags = (classOverridesRespondsToSelector || instancesOverrideRespondsToSelector)
                       ...
                       : flags;
    node->_methodOverrides = (classOverridesRespondsToSelector)
                                 ? GetASDisplayNodeMethodOverrides(node.class)
                                 : methodOverrides;
  });
  ...
  if (self == ASDisplayNode.class) {
    ...
    class_addMethod(self, @selector(willCalculateLayout:), (IMP)StubImplementationWithSizeRange,
                    type0.c_str());

    auto interfaceStateType = std::string(@encode(ASInterfaceState));
    auto type1 = "v@:" + interfaceStateType + interfaceStateType;
    class_addMethod(self, @selector(interfaceStateDidChange:fromState:),
                    (IMP)StubImplementationWithTwoInterfaceStates, type1.c_str());
  }
}
...
- (void)_initializeInstance {
  ...
  _defaultLayoutTransitionOptions =
      UIViewAnimationOptionCurveEaseInOut | UIViewAnimationOptionTransitionNone;
...
- (instancetype)initWithViewClass:(Class)viewClass {
  ...
  ASDisplayNodeAssert([viewClass isSubclassOfClass:[UIView class]],
                      @"should initialize with a subclass of UIView");
...
- (instancetype)initWithLayerClass:(Class)layerClass {
  ...
  ASDisplayNodeAssert([layerClass isSubclassOfClass:[CALayer class]],
                      @"should initialize with a subclass of CALayer");
...
- (void)setViewBlock:(ASDisplayNodeViewBlock)viewBlock {
  ...
  ASDisplayNodeAssertNotNil(viewBlock,
                            @"should initialize with a valid block that returns a UIView");
...
- (void)setLayerBlock:(ASDisplayNodeLayerBlock)layerBlock {
  ASDisplayNodeAssertFalse(self.nodeLoaded);
  ASDisplayNodeAssertNotNil(layerBlock,
                            @"should initialize with a valid block that returns a CALayer");
...
- (void)dealloc {
  ...
  // Synchronous nodes may not be able to call the hierarchy notifications, so only enforce for
  // regular nodes.
  ...
  // Remove any subnodes so they lose their connection to the now deallocated parent.  This can
  // happen because subnodes do not retain their supernode, but subnodes can legitimately remain
  // alive if another thing outside the view hierarchy system (e.g. async display, controller code,
  // etc). keeps a retained reference to subnodes.
...
- (void)_locked_loadViewOrLayer {
  ...
  if (_flags.layerBacked) {
    ...
    /**
     * CALayer's .delegate property is documented to be weak, but the implementation is actually
     * assign. Because our layer may survive longer than the node (e.g. if someone else retains it,
     * or if the node begins deallocation on a background thread and it waiting for the -dealloc
     * call to reach main), the only way to avoid a dangling pointer is to use a weak proxy.
     */
...
- (BOOL)isNodeLoaded {
  if (ASDisplayNodeThreadIsMain()) {
    // Because the view and layer can only be created and destroyed on Main, that is also the only
    // thread where the state of this property can change. As an optimization, we can avoid locking.
    return _loaded(self);
  } else {
...
- (void)setLayerBacked:(BOOL)isLayerBacked {
  ...
  ASDisplayNodeAssert(!isLayerBacked || self.supportsLayerBacking,
                      @"Node %@ does not support layer backing.", self);
...
- (void)_fallbackUpdateSafeAreaOnChildren {
  ...
  for (ASDisplayNode *child in self.subnodes) {
    ...
    UIEdgeInsets childInsets = UIEdgeInsetsMake(
        MAX(insets.top - (CGRectGetMinY(childFrame) - CGRectGetMinY(bounds)), 0),
...
#define HANDLE_NODE_RESPONDER_METHOD(__sel)                                                        \
  ...
  ASDisplayNodeAssertMainThread();                                                                 \
  if (checkFlag(Synchronous)) {                                                                    \
    /* If the view is not a _ASDisplayView subclass (Synchronous) just call through to the view as \
     we expect it's a non _ASDisplayView subclass that will respond */                                    \
    return [_view __sel];                                                                          \
  } else {                                                                                         \
...
- (void)checkResponderCompatibility {
  ...
  if (checkFlag(Synchronous)) {
    ...
    NSString *message = @"Overwritting %@ and having a backing view that is not an _ASDisplayView "
                        @"is not supported.";
    ASDisplayNodeAssert(
        !ASDisplayNodeSubclassOverridesSelector(self.class, @selector(canBecomeFirstResponder)),
        ([NSString stringWithFormat:message, @"canBecomeFirstResponder"]));
    ASDisplayNodeAssert(
        !ASDisplayNodeSubclassOverridesSelector(self.class, @selector(becomeFirstResponder)),
        ([NSString stringWithFormat:message, @"becomeFirstResponder"]));
    ASDisplayNodeAssert(
        !ASDisplayNodeSubclassOverridesSelector(self.class, @selector(canResignFirstResponder)),
        ([NSString stringWithFormat:message, @"canResignFirstResponder"]));
    ASDisplayNodeAssert(
        !ASDisplayNodeSubclassOverridesSelector(self.class, @selector(resignFirstResponder)),
        ([NSString stringWithFormat:message, @"resignFirstResponder"]));
    ASDisplayNodeAssert(
        !ASDisplayNodeSubclassOverridesSelector(self.class, @selector(isFirstResponder)),
  ...
  BOOL loaded = NO;
  {
    ...
    if (CGRectEqualToRect(bounds, CGRectZero)) {
      ...
      as_log_debug(OS_LOG_DISABLED,
                   "Warning: No size given for node before node was trying to layout itself: %@. "
                   "Please provide a frame for the node.",
                   self);
      return;
    }

    // If a current layout transition is in progress there is no need to do a measurement and layout
    // pass in here as this is supposed to happen within the layout transition process
    ...
    // Importantly, it will also APPLY the layout to all of our subnodes if (unless parent is
    // transitioning).
    l.unlock();
...
- (ASLayout *)calculateLayoutThatFits:(ASSizeRange)constrainedSize
  ...
  as_activity_scope_verbose(
      as_activity_create("Calculate node layout", AS_ACTIVITY_CURRENT, OS_ACTIVITY_FLAG_DEFAULT));
...
- (CGSize)calculateSizeThatFits:(CGSize)constrainedSize {
  ...
  ASDisplayNodeLogEvent(self, @"calculateSizeThatFits: with constrainedSize: %@",
...
/**
 * After completeTransition: is called on the ASContextTransitioning object in
 * animateLayoutTransition: this delegate method will be called that start the completion process of
 * the transition
 */
...
NSString *const ASRenderingEngineDidDisplayScheduledNodesNotification =
    @"ASRenderingEngineDidDisplayScheduledNodes";
...
- (void)setDisplaysAsynchronously:(BOOL)displaysAsynchronously {
  ...
  // Can't do this for synchronous nodes (using layers that are not _ASDisplayLayer and so we can't
  // control display prevention/cancel)
  if (checkFlag(Synchronous)) {
    return;
  }
...
- (void)enableSubtreeRasterization {
  ...
  if (ASHierarchyStateIncludesRasterized(_hierarchyState)) {
    ASDisplayNodeFailAssert(
        @"Subnode of a rasterized node should not have redundant -enableSubtreeRasterization.");
    return;
  }
  ...
  ASDisplayNode *loadedSubnode =
      ASDisplayNodeFindFirstSubnode(self, ^BOOL(ASDisplayNode *_Nonnull node) {
        return node.nodeLoaded;
      });
  if (loadedSubnode != nil) {
    ASDisplayNodeFailAssert(@"Cannot call %@ on node %@ with loaded subnode %@",
                            NSStringFromSelector(_cmd), self, loadedSubnode);
    return;
  }
...
- (void)displayImmediately {
  ASDisplayNodeAssertMainThread();
  ASDisplayNodeAssert(!checkFlag(Synchronous),
                      @"this method is designed for asynchronous mode only");
...
// The node sending the message should usually be passed as the parameter, similar to the delegation
// pattern.
...
// Helper method to determine if it's safe to call setNeedsDisplay on a layer without throwing away
// the content. For details look at the comment on the canCallSetNeedsDisplayOfLayer flag
...
void recursivelyTriggerDisplayForLayer(CALayer *layer, BOOL shouldBlock) {
  ...
  // 2. Previously in a hierarchy (such as a working window owned by an Intelligent Preloading
  // class, like ASTableView / ASCollectionView / ASViewController)
  // 3. Has no content to display at all
  // Specifically for case 1), we need to explicitly trigger a -display call now.
  // Otherwise, there is no opportunity to block the main thread after CoreAnimation's transaction
  // commit (even a runloop observer at a late call order will not stop the next frame from
  // compositing, showing placeholders).

  ASDisplayNode *node = [layer asyncdisplaykit_node];

  if (node.isSynchronous && [node _canCallSetNeedsDisplayOfLayer]) {
    // Layers for UIKit components that are wrapped within a node needs to be set to be displayed as
    // the contents of the layer get's cleared and would not be recreated otherwise. We do not call
    // this for _ASDisplayLayer as an optimization.
    [layer setNeedsDisplay];
  }

  if ([node _implementsDisplay]) {
    // For layers that do get displayed here, this immediately kicks off the work on the concurrent
    // -[_ASDisplayLayer displayQueue]. At the same time, it creates an associated
    // _ASAsyncTransaction, which we can use to block on display completion.  See
    // ASDisplayNode+AsyncDisplay.mm.
    [layer displayIfNeeded];
  }

  // Kick off the recursion first, so that all necessary display calls are sent and the displayQueue
  // is full of parallelizable work. NOTE: The docs report that `sublayers` returns a copy but it
  // actually doesn't.
  ...
  if (shouldBlock) {
    ...
    // While blocking on one transaction, others may be completing concurrently, so it doesn't
    // matter which blocks first.
    BOOL waitUntilComplete = (!node.shouldBypassEnsureDisplay);
    if (waitUntilComplete) {
      for (_ASAsyncTransaction *transaction in
           [layer.asyncdisplaykit_asyncLayerTransactions copy]) {
        // Even if none of the layers have had a chance to start display earlier, they will still be
        // allowed to saturate a multicore CPU while blocking main. This significantly reduces time
        // on the main thread relative to UIKit.
...
- (void)_layoutClipCornersIfNeeded {
  ...
  for (int idx = 0; idx < NUM_CLIP_CORNER_LAYERS; idx++) {
    ...
    if (_clipCornerLayers[idx]) {
      ...
      _clipCornerLayers[idx].position =
          CGPointMake(isRight ? boundsSize.width : 0.0, isTop ? boundsSize.height : 0.0);
      [_layer addSublayer:_clipCornerLayers[idx]];
    }
  }
}

- (void)_updateClipCornerLayerContentsWithRadius:(CGFloat)radius
  ...
  ASPerformBlockOnMainThread(^{
    for (int idx = 0; idx < NUM_CLIP_CORNER_LAYERS; idx++) {
      ...
      [roundedRect appendPath:[UIBezierPath bezierPathWithRect:CGRectMake(-1, -1, radius * 2 + 1,
                                                                          radius * 2 + 1)]];
...
- (void)_setClipCornerLayersVisible:(BOOL)visible {
  ASPerformBlockOnMainThread(^{
    ...
    if (visible) {
      ...
      [self _updateClipCornerLayerContentsWithRadius:_cornerRadius
      ...
      if (oldRoundingType == ASCornerRoundingTypeDefaultSlowCALayer) {
        if (newRoundingType == ASCornerRoundingTypePrecomposited) {
          ...
          if (oldCornerRadius > 0.0) {
            ...
            [self setNeedsDisplay];  // Async display is OK if we aren't replacing an existing
                                     // .cornerRadius.
          }
        ...
        if (newRoundingType == ASCornerRoundingTypeDefaultSlowCALayer) {
          ...
          [self _updateClipCornerLayerContentsWithRadius:newCornerRadius
                                         backgroundColor:self.backgroundColor];
        }
      }
    }
...
// TODO: Replace this with ASDisplayNodePerformBlockOnEveryNode or a variant with a condition / test
// block.
static void _recursivelySetDisplaySuspended(ASDisplayNode *node, CALayer *layer, BOOL flag) {
  // If there is no layer, but node whose its view is loaded, then we can traverse down its layer
  // hierarchy.  Otherwise we must stick to the node hierarchy to avoid loading views prematurely.
  // Note that for nodes that haven't loaded their views, they can't possibly have
  // subviews/sublayers, so we don't need to traverse the layer hierarchy for them.
  ...
  // Set the flag on the node.  If this is a pure layer (no node) then this has no effect (plain
  // layers don't support preventing/cancelling display).
  node.displaySuspended = flag;

  if (layer && !node.rasterizesSubtree) {
    // If there is a layer, recurse down the layer hierarchy to set the flag on descendants.  This
    // will cover both layer-based and node-based children.
    for (CALayer *sublayer in layer.sublayers) {
      _recursivelySetDisplaySuspended(nil, sublayer, flag);
    }
  } else {
    // If there is no layer (view not loaded yet) or this node rasterizes descendants (there won't
    // be a layer tree to traverse), recurse down the subnode hierarchy to set the flag on
    // descendants.  This covers only node-based children, but for a node whose view is not loaded
    // it can't possibly have nodeless children.
...
// We are only the delegate for the layer when we are layer-backed, as UIView performs this function
// normally
- (id<CAAction>)actionForLayer:(CALayer *)layer forKey:(NSString *)event {
  ...
  ASDisplayNodeAssert(_flags.layerBacked,
                      @"We shouldn't get called back here unless we are layer-backed.");
  return (id)kCFNull;
}
...
ASDISPLAYNODE_INLINE bool shouldDisableNotificationsForMovingBetweenParents(ASDisplayNode *from,
                                                                            ASDisplayNode *to) {
...
- (void)_setSupernode:(ASDisplayNode *)newSupernode {
  ...
  ASDisplayNode *oldSupernode = nil;
  {
    ...
    if (_supernode != newSupernode) {
      oldSupernode = _supernode;  // Access supernode properties outside of lock to avoid remote
                                  // chance of deadlock, in case supernode implementation must
                                  // access one of our properties.
  ...
  if (supernodeDidChange) {
    ASDisplayNodeLogEvent(self, @"supernodeDidChange: %@, oldValue = %@",
                          ASObjectDescriptionMakeTiny(newSupernode),
                          ASObjectDescriptionMakeTiny(oldSupernode));
    // Hierarchy state
    ASHierarchyState stateToEnterOrExit =
        (newSupernode ? newSupernode.hierarchyState : oldSupernode.hierarchyState);

    // Rasterized state
    BOOL parentWasOrIsRasterized =
        (newSupernode ? newSupernode.rasterizesSubtree : oldSupernode.rasterizesSubtree);
    ...
    if (newSupernode) {
      ...
      // If a node was added to a supernode, the supernode could be in a layout pending state. All
      // of the hierarchy state properties related to the transition need to be copied over as well
      // as propagated down the subtree. This is especially important as with automatic subnode
      // management, adding subnodes can happen while a transition is in fly
      ...
      // If a node will be removed from the supernode it should go out from the layout pending state
      // to remove all layout pending state related properties on the node
      ...
      // which has some nice carry-over machinery to handle cases where we are removed from a
      // hierarchy and then added into it again shortly after.
      __instanceLock__.lock();
...
/*
 * Central private helper method that should eventually be called if submethods add, insert or
 * replace subnodes This method is called with thread affinity and without lock held.
 *
 * @param subnode       The subnode to insert
 * @param subnodeIndex  The index in _subnodes to insert it
 * @param viewSublayerIndex The index in layer.sublayers (not view.subviews) at which to insert the
 * view (use if we can use the view API) otherwise pass NSNotFound
 * @param sublayerIndex The index in layer.sublayers at which to insert the layer (use if either
 * parent or subnode is layer-backed) otherwise pass NSNotFound
...
- (void)_insertSubnode:(ASDisplayNode *)subnode
  ...
  as_log_verbose(ASNodeLog(), "Insert subnode %@ at index %zd of %@ and remove subnode %@", subnode,
                 subnodeIndex, self, oldSubnode);
  ...
  if (self.layerBacked && !subnode.layerBacked) {
    ASDisplayNodeFailAssert(@"Cannot add a view-backed node as a subnode of a layer-backed node. "
                            @"Supernode: %@, subnode: %@",
                            self, subnode);
    return;
  }
  ...
  if (subnodeIndex > subnodesCount || subnodeIndex < 0) {
    ASDisplayNodeFailAssert(@"Cannot insert a subnode at index %ld. Count is %ld",
                            (long)subnodeIndex, (long)subnodesCount);
    return;
  }

  // Disable appearance methods during move between supernodes, but make sure we restore their state
  ...
  ASDisplayNodeAssert(
      disableNotifications == shouldDisableNotificationsForMovingBetweenParents(oldParent, self),
      @"Invariant violated");
  if (disableNotifications) {
 ...
 *
 * @param subnode       The subnode to insert
 * @param idx           The index in _view.subviews or _layer.sublayers at which to insert the
 * subnode.view or subnode.layer of the subnode
 */
- (void)_insertSubnodeSubviewOrSublayer:(ASDisplayNode *)subnode atIndex:(NSInteger)idx {
  ASDisplayNodeAssertMainThread();
  ASDisplayNodeAssert(self.nodeLoaded, @"_insertSubnodeSubviewOrSublayer:atIndex: should never be "
                                       @"called before our own view is created");
  ...
  // Because the view and layer can only be created and destroyed on Main, that is also the only
  // thread where the view and layer can change. We can avoid locking.

  // If we can use view API, do. Due to an apple bug, -insertSubview:atIndex: actually wants a LAYER
  // index, which we pass in.
...
- (void)_addSubnode:(ASDisplayNode *)subnode {
  ...
  [self _insertSubnode:subnode
        atSubnodeIndex:subnodesIndex
         sublayerIndex:sublayersIndex
...
- (void)replaceSubnode:(ASDisplayNode *)oldSubnode withSubnode:(ASDisplayNode *)replacementSubnode {
  ASDisplayNodeLogEvent(
      self, @"replaceSubnode: %@ withSubnode: %@ with automaticallyManagesSubnodes: %@", oldSubnode,
...
- (void)_replaceSubnode:(ASDisplayNode *)oldSubnode
            withSubnode:(ASDisplayNode *)replacementSubnode {
  ASDisplayNodeAssertThreadAffinity(self);
  ...
  ASDisplayNodeAssert(!(self.nodeLoaded && !oldSubnode.nodeLoaded),
                      @"We have view loaded, but child node does not.");
  ...
  NSInteger sublayerIndex = NSNotFound;
  {
    ...
    // Don't bother figuring out the sublayerIndex if in a rasterized subtree, because there are no
    // layers in the hierarchy and none of this could possibly work.
    if (subtreeIsRasterized(self) == NO) {
      if (_layer) {
        sublayerIndex = [_layer.sublayers indexOfObjectIdenticalTo:oldSubnode.layer];
        ASDisplayNodeAssert(sublayerIndex != NSNotFound,
                            @"Somehow oldSubnode's supernode is self, yet we could not find it in "
                            @"our layers to replace");
...
- (void)_insertSubnode:(ASDisplayNode *)subnode belowSubnode:(ASDisplayNode *)below {
    ...
    if (subtreeIsRasterized(self) == NO) {
      if (_layer) {
        ...
        ASDisplayNodeAssert(belowSublayerIndex != NSNotFound,
                            @"Somehow below's supernode is self, yet we could not find it in our "
                            @"layers to reference");
      ...
      // If the subnode is already in the subnodes array / sublayers and it's before the below node,
      // removing it to insert it will mess up our calculation
      if (subnode.supernode == self) {
        ...
        if (_layer) {
          NSInteger currentIndexInSublayers =
              [_layer.sublayers indexOfObjectIdenticalTo:subnode.layer];
  ...
  [self _insertSubnode:subnode
        atSubnodeIndex:belowSubnodeIndex
         sublayerIndex:belowSublayerIndex
      andRemoveSubnode:nil];
}
    ...
    if (subtreeIsRasterized(self) == NO) {
      if (_layer) {
        ...
        ASDisplayNodeAssert(aboveSublayerIndex != NSNotFound,
                            @"Somehow above's supernode is self, yet we could not find it in our "
                            @"layers to replace");
...
- (void)_insertSubnode:(ASDisplayNode *)subnode atIndex:(NSInteger)idx {
  ...
  NSInteger sublayerIndex = NSNotFound;
  {
    ...
    if (idx > _subnodes.count || idx < 0) {
      ASDisplayNodeFailAssert(@"Cannot insert a subnode at index %ld. Count is %ld", (long)idx,
                              (long)_subnodes.count);
      return;
    }
    ...
    if (subtreeIsRasterized(self) == NO) {
      ...
      if (_layer && idx == 0) {
        ...
        ASDisplayNode *positionInRelationTo =
            (_subnodes.count > 0 && idx > 0) ? _subnodes[idx - 1] : nil;
        if (positionInRelationTo) {
          sublayerIndex = incrementIfFound(
              [_layer.sublayers indexOfObjectIdenticalTo:positionInRelationTo.layer]);
        }
      }
    }
  }
...
- (void)_removeSubnode:(ASDisplayNode *)subnode {
  ...
  // Don't call self.supernode here because that will retain/autorelease the supernode.  This method
  // -_removeSupernode: is often called while tearing down a node hierarchy, and the supernode in
  // question might be in the middle of its -dealloc.  The supernode is never messaged, only
  // compared by value, so this is safe. The particular issue that triggers this edge case is when a
  // node calls -removeFromSupernode on a subnode from within its own -dealloc method.
...
- (void)_removeFromSupernode:(ASDisplayNode *)supernode view:(UIView *)view layer:(CALayer *)layer {
  ...
  // Clear supernode's reference to us before removing the view from the hierarchy, as
  // _ASDisplayView will trigger us to clear our _supernode pointer in willMoveToSuperview:nil. This
  // may result in removing the last strong reference, triggering deallocation after this method.
...
- (void)__incrementVisibilityNotificationsDisabled {
  ...
  if (visibilityNotificationsDisabled) {
    // Must have just transitioned from 0 to 1.  Notify all subnodes that we are in a disabled
    // state.
...
- (void)__decrementVisibilityNotificationsDisabled {
  ...
  if (visibilityNotificationsDisabled) {
    // Must have just transitioned from 1 to 0.  Notify all subnodes that we are no longer in a
    // disabled state.
    // FIXME: This system should be revisited when refactoring and consolidating the implementation
    // of the addSubnode: and insertSubnode:... methods.  As implemented, though logically
    // irrelevant for expected use cases, multiple nodes in the subtree below may have a non-zero
    // visibilityNotification count and still have the ASHierarchyState bit cleared (the only value
    // checked when reading this state).
...
- (void)_locked_layoutPlaceholderIfNecessary {
  ...
  // Update the placeholderLayer size in case the node size has changed since the placeholder was
  // added.
...
- (void)_locked_setupPlaceholderLayerIfNeeded {
  ...
  if (_placeholderLayer.contents == nil) {
    ...
    if (_placeholderImage) {
      BOOL stretchable =
          !UIEdgeInsetsEqualToEdgeInsets(_placeholderImage.capInsets, UIEdgeInsetsZero);
      if (stretchable) {
...
- (void)__enterHierarchy {
  ...
  // Profiling has shown that locking this method is beneficial, so each of the property accesses
  // don't have to lock and unlock.
  __instanceLock__.lock();
  ...
  if (!_flags.isInHierarchy && !_flags.visibilityNotificationsDisabled &&
    ...
    // And so they can potentially walk up the node tree and cause deadlocks, or do expensive tasks
    // and cause the lock to be held for too long.
    __instanceLock__.unlock();
    ...
    // If we don't have contents finished drawing by the time we are on screen, immediately add the
    // placeholder (if it is enabled and we do have something to draw).
...
- (void)setHierarchyState:(ASHierarchyState)newState {
  ...
  if (newState & ASHierarchyStateRasterized) {
    ASDisplayNodeAssert(checkFlag(Synchronous) == NO,
                        @"Node created using -initWithViewBlock:/-initWithLayerBlock: cannot be "
                        @"added to subtree of node with subtree rasterization enabled. Node: %@",
                        self);
  }
  ...
  if ((newState & ASHierarchyStateRangeManaged) != (oldState & ASHierarchyStateRangeManaged)) {
    if (newState & ASHierarchyStateRangeManaged) {
      ...
      // The case of exiting a range-managed state should be fairly rare.  Adding or removing the
      // node to a view hierarchy will cause its interfaceState to be either fully set or unset (all
      // fields), but because we might be about to be added to a view hierarchy, exiting the
      // interface state now would cause inefficient churn.  The tradeoff is that we may not clear
      // contents / fetched data for nodes that are removed from a managed state and then retained
      // but not used (bad idea anyway!)
    }
  }
  ...
  ASDisplayNodeLogEvent(self, @"setHierarchyState: %@",
                        NSStringFromASHierarchyStateChange(oldState, newState));
  as_log_verbose(ASNodeLog(), "%s%@ %@", sel_getName(_cmd),
                 NSStringFromASHierarchyStateChange(oldState, newState), self);
}

- (void)willEnterHierarchy {
  ASDisplayNodeAssertMainThread();
  ASDisplayNodeAssert(_flags.isEnteringHierarchy,
                      @"You should never call -willEnterHierarchy directly. Appearance is "
                      @"automatically managed by ASDisplayNode");
  ASDisplayNodeAssert(
...
- (void)didEnterHierarchy {
  ASDisplayNodeAssertMainThread();
  ASDisplayNodeAssert(!_flags.isEnteringHierarchy,
                      @"You should never call -didEnterHierarchy directly. Appearance is "
                      @"automatically managed by ASDisplayNode");
  ASDisplayNodeAssert(
      !_flags.isExitingHierarchy,
      @"ASDisplayNode inconsistency. __enterHierarchy and __exitHierarchy are mutually exclusive");
  ASDisplayNodeAssert(
      _flags.isInHierarchy,
      @"ASDisplayNode inconsistency. __enterHierarchy and __exitHierarchy are mutually exclusive");
  ASAssertUnlocked(__instanceLock__);
}
...
- (void)didExitHierarchy {
  ...
  ASDisplayNodeAssert(_flags.isExitingHierarchy,
                      @"You should never call -didExitHierarchy directly. Appearance is "
                      @"automatically managed by ASDisplayNode");
  ASDisplayNodeAssert(
  ...
  // This case is important when tearing down hierarchies.  We must deliver a
  // visibileStateDidChange:NO callback, as part our API guarantee that this method can be used for
  // things like data analytics about user content viewing.  We cannot call the method in the
  // dealloc as any incidental retain operations in client code would fail. Additionally, it may be
  // that a Standard UIView which is containing us is moving between hierarchies, and we should not
  // send the call if we will be re-added in the same runloop.  Strategy: strong reference (might be
  // the last!), wait one runloop, and confirm we are still outside the hierarchy (both layer-backed
  // and view-backed).
  // TODO: This approach could be optimized by only performing the dispatch for root elements +
  // recursively apply the interface state change. This would require a closer integration with
  // _ASDisplayLayer to ensure that the superlayer pointer has been cleared by this stage (to check
  // if we are root or not), or a different delegate call.
  ...
  if (ASInterfaceStateIncludesVisible(self.pendingInterfaceState)) {
    void (^exitVisibleInterfaceState)(void) = ^{
      ...
      // layer may be thrashed, we need to remember the state so we can reset if it enters in same
      // runloop later.
...
/**
 * We currently only set interface state on nodes in table/collection views. For other nodes, if
 * they are in the hierarchy we enable all ASInterfaceState types with
 * `ASInterfaceStateInHierarchy`, otherwise `None`.
 */
...
- (void)exitInterfaceState:(ASInterfaceState)interfaceState {
  ...
  ASDisplayNodeLogEvent(self, @"%s %@", sel_getName(_cmd),
                        NSStringFromASInterfaceState(interfaceState));
  ASDisplayNodePerformBlockOnEveryNode(nil, self, YES, ^(ASDisplayNode *node) {
    node.interfaceState &= (~interfaceState);
  });
}
...
- (void)recursivelySetInterfaceState:(ASInterfaceState)newInterfaceState {
  ...
  // setInterfaceState: skips this when handling range-managed nodes (our whole subtree has this
  // set). If our range manager intends for us to be displayed right now, and didn't before, get
  // started!
  BOOL shouldScheduleDisplay = [self supportsRangeManagedInterfaceState] &&
                               [self shouldScheduleDisplayWithNewInterfaceState:newInterfaceState];
  ASDisplayNodePerformBlockOnEveryNode(nil, self, YES, ^(ASDisplayNode *node) {
...
- (void)applyPendingInterfaceState:(ASInterfaceState)newPendingState {
  // This method is currently called on the main thread. The assert has been added here because all
  // of the did(Enter|Exit)(Display|Visible|Preload)State methods currently guarantee calling on
  // main.
  ASDisplayNodeAssertMainThread();

  // This method manages __instanceLock__ itself, to ensure the lock is not held while
  // didEnter/Exit(.*)State methods are called, thus avoid potential deadlocks
  ASAssertUnlocked(__instanceLock__);
  ...
  ASDisplayNodeAssertFalse(ASInterfaceStateIncludesVisible(newState) &&
                           !ASInterfaceStateIncludesDisplay(newState));
  ...
  // For the Preload and Display ranges, we don't want to call -clear* if not being managed by a
  // range controller. Otherwise we get flashing behavior from normal UIKit manipulations like
  // navigation controller push / pop. Still, the interfaceState should be updated to the current
  // state of the node; just don't act on the transition.
  ...
  if (nowPreload != wasPreload) {
    if (nowPreload) {
      ...
      // We don't want to call -didExitPreloadState on nodes that aren't being managed by a range
      // controller. Otherwise we get flashing behavior from normal UIKit manipulations like
  ...
  if (nowDisplay != wasDisplay) {
    if ([self supportsRangeManagedInterfaceState]) {
      ...
      // NOTE: This case isn't currently supported as setInterfaceState: isn't exposed externally,
      // and all internal use cases are range-managed.  When a node is visible, don't mess with
      // display - CA will start it.
      if (!ASInterfaceStateIncludesVisible(newState)) {
        // Check _implementsDisplay purely for efficiency - it's faster even than calling
        // -asyncLayer.
        if ([self _implementsDisplay]) {
          if (nowDisplay) {
            [ASDisplayNode scheduleNodeForRecursiveDisplay:self];
  ...
  // Became visible or invisible.  When range-managed, this represents literal visibility - at least
  // one pixel is onscreen.  If not range-managed, we can't guarantee more than the node being
  // present in an onscreen window.
  ...
  // Log this change, unless it's just the node going from {} -> {Measure} because that change
  // happens for all cell nodes and it isn't currently meaningful.
  BOOL measureChangeOnly = ((oldState | newState) == ASInterfaceStateMeasureLayout);
  if (!measureChangeOnly) {
    as_log_verbose(ASNodeLog(), "%s %@ %@", sel_getName(_cmd),
                   NSStringFromASInterfaceStateChange(oldState, newState), self);
  }

  ASDisplayNodeLogEvent(self, @"interfaceStateDidChange: %@",
                        NSStringFromASInterfaceStateChange(oldState, newState));
  [self _interfaceStateDidChange:newState fromState:oldState];
}

- (void)prepareForCATransactionCommit {
  // Apply _pendingInterfaceState actual _interfaceState, note that ASInterfaceStateNone is not
  // used.
...
- (void)addInterfaceStateDelegate:(id<ASInterfaceStateDelegate>)interfaceStateDelegate {
  ...
  ASDisplayNodeFailAssert(@"Exceeded interface state delegate limit: %d",
                          AS_MAX_INTERFACE_STATE_DELEGATES);
}
...
- (void)_didEnterPreloadState {
  ...
  // If this node has ASM enabled and is not yet visible, force a layout pass to apply its
  // applicable pending layout, if any, so that its subnodes are inserted/deleted and start
  // preloading right away.
  //
  // - If it has an up-to-date layout (and subnodes), calling -layoutIfNeeded will be fast.
  //
  // - If it doesn't have a calculated or pending layout that fits its current bounds, a measurement
  // pass will occur (see -__layout and -_u_measureNodeWithBoundsIfNecessary:). This scenario is
  // uncommon, and running a measurement pass here is a fine trade-off because preloading any time
  // after this point would be late.

  if (self.automaticallyManagesSubnodes &&
      !ASActivateExperimentalFeature(ASExperimentalDidEnterPreloadSkipASMLayout)) {
    [self layoutIfNeeded];
  }
  [self enumerateInterfaceStateDelegates:^(id<ASInterfaceStateDelegate> del) {
...
- (void)clearContents {
  ...
  if (_flags.canClearContentsOfLayer) {
    // No-op if these haven't been created yet, as that guarantees they don't have contents that
...
- (void)_locked_applyPendingStateToViewOrLayer {
  ...
  // If no view/layer properties were set before the view/layer were created, _pendingViewState will
  // be nil and the default values for the view/layer are still valid.
...
- (void)_locked_applyPendingViewState {
  ASDisplayNodeAssertMainThread();
  ASAssertLocked(__instanceLock__);
  ASDisplayNodeAssert([self _locked_isNodeLoaded],
                      @"Expected node to be loaded before applying pending state.");
...
// This method has proved helpful in a few rare scenarios, similar to a category extension on
// UIView, but assumes knowledge of _ASDisplayView. It's considered private API for now and its use
// should not be encouraged.
- (ASDisplayNode *)_supernodeWithClass:(Class)supernodeClass
                    checkViewHierarchy:(BOOL)checkViewHierarchy {
...
#if TIME_DISPLAYNODE_OPS
  NSString *creationTypeString = [NSString
      stringWithFormat:@"cr8:%.2lfms dl:%.2lfms ap:%.2lfms ad:%.2lfms",
                       1000 * _debugTimeToCreateView, 1000 * _debugTimeForDidLoad,
                       1000 * _debugTimeToApplyPendingState, 1000 * _debugTimeToAddSubnodeViews];
...
- (NSString *)_recursiveDescriptionHelperWithIndent:(NSString *)indent {
  NSMutableString *subtree = [[[indent stringByAppendingString:self.debugDescription]
      stringByAppendingString:@"\n"] mutableCopy];
  for (ASDisplayNode *n in self.subnodes) {
    [subtree
        appendString:[n _recursiveDescriptionHelperWithIndent:[indent
                                                                  stringByAppendingString:@" | "]]];
  }
  return subtree;
}
...
- (NSString *)detailedLayoutDescription {
  ...
  if (_calculatedDisplayNodeLayout.layout) {
    ...
    [props addObject:@{
      @"calculatedConstrainedSize" :
          NSStringFromASSizeRange(_calculatedDisplayNodeLayout.constrainedSize)
    }];
  ...
  if (_pendingDisplayNodeLayout.layout) {
    ...
    [props addObject:@{
      @"pendingConstrainedSize" : NSStringFromASSizeRange(_pendingDisplayNodeLayout.constrainedSize)
    }];
...
// We use associated objects as a last resort if our view is not a _ASDisplayView ie it doesn't have
// the _node ivar to write to
