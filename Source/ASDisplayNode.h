...
/**
 * ASDisplayNode loaded callback block. This block is called BEFORE the -didLoad method and is
...
/**
 * ASDisplayNode layout spec block. This block can be used instead of implementing
 * layoutSpecThatFits: in subclass
 */
typedef ASLayoutSpec *_Nonnull (^ASLayoutSpecBlock)(__kindof ASDisplayNode *node,
                                                    ASSizeRange constrainedSize);

/**
 * AsyncDisplayKit non-fatal error block. This block can be used for handling non-fatal errors.
 * Useful for reporting errors that happens in production.
 */
...
/**
 * An `ASDisplayNode` is an abstraction over `UIView` and `CALayer` that allows you to perform
 * calculations about a view hierarchy off the main thread, and could do rendering off the main
 * thread as well.
 *
 ...
 * ## Subclassing
 *
 * `ASDisplayNode` can be subclassed to create a new UI element. The subclass header
 * `ASDisplayNode+Subclasses` provides necessary declarations and conveniences.
 *
 * Commons reasons to subclass includes making a `UIView` property available and receiving a
 * callback after async display.
 *
 */
...
@interface ASDisplayNode : NSObject <ASLocking> {
 @public
  /**
   * The _context ivar is unused by Texture, but provided to enable advanced clients to make
   * powerful extensions to base class functionality. For example, _context can be used to implement
   * category methods on ASDisplayNode that add functionality to all node subclass types. Code
   * demonstrating this technique can be found in the CatDealsCollectionView example.
   */
  void *_context;
}
 ...
 *
 * @return An ASDisplayNode instance whose view will be a subclass that enables asynchronous
 * rendering, and passes through -layout and touch handling methods.
 */
 ...
 *
 * @return An ASDisplayNode instance that loads its view with the given block that is guaranteed to
 * run on the main queue. The view will render synchronously and -layout and touch handling methods
 * on the node will not be called.
 */
 ...
 *
 * @param viewBlock The block that will be used to create the backing view.
 * @param didLoadBlock The block that will be called after the view created by the viewBlock is
 * loaded
 *
 * @return An ASDisplayNode instance that loads its view with the given block that is guaranteed to
 * run on the main queue. The view will render synchronously and -layout and touch handling methods
 ...
 *
 * @return An ASDisplayNode instance that loads its layer with the given block that is guaranteed to
 * run on the main queue. The layer will render synchronously and -layout and touch handling methods
 * on the node will not be called.
 */
 ...
 *
 * @param layerBlock The block that will be used to create the backing layer.
 * @param didLoadBlock The block that will be called after the layer created by the layerBlock is
 * loaded
 *
 * @return An ASDisplayNode instance that loads its layer with the given block that is guaranteed to
 * run on the main queue. The layer will render synchronously and -layout and touch handling methods
...
/**
 * @abstract Add a block of work to be performed on the main thread when the node's view or layer is
 * loaded. Thread safe.
 * @warning Be careful not to retain self in `body`. Change the block parameter list to
 * `^(MYCustomNode *self) {}` if you want to shadow self (e.g. if calling this during `init`).
 *
 ...
 *
 * @warning The first access to it must be on the main thread, and should only be used on the main
 * thread thereafter as well.
 */
 ...
 *
 * @return The current ASInterfaceState of the node, indicating whether it is visible and other
 ...
 * property can be useful for cases when Async Display Kit can recover from an abnormal behavior,
 * but still gives the opportunity to use a reporting mechanism to catch occurrences in production.
 * In development, Async Display Kit will assert instead of calling this block.
 *
 ...
 *
 * @discussion The subnode's view will automatically be added to this node's view, lazily if the
 * views are not created yet.
 */
 ...
 *
 * @discussion If the views are loaded, the subnode's view will be inserted below the given node's
 * view in the hierarchy even if there are other non-displaynode views.
 */
 ...
 *
 * @discussion If the views are loaded, the subnode's view will be inserted above the given node's
 * view in the hierarchy even if there are other non-displaynode views.
 */
 ...
 * @param idx The index in the array of the subnodes property at which to insert the node. Subnodes
 * indices start at 0 and cannot be greater than the number of subnodes.
 *
 * @discussion If this node's view is loaded, ASDisplayNode insert the subnode's view after the
 * subnode at index - 1's view even if there are other non-displaynode views.
 */
 ...
 *
 * @discussion Should both subnode and replacementSubnode already be subnodes of self, subnode is
 * removed and replacementSubnode inserted in its place. If subnode is not a subnode of self, this
 * method will throw an exception. If replacementSubnode is nil, this method will throw an exception
 */
 ...
 *
 * @discussion If this flag is set, then the node will participate in the current
 * asyncdisplaykit_async_transaction and do its rendering on the displayQueue instead of the main
 * thread.
 *
 ...
 * The delegate provides contents via this method and an operation is added to the
 * asyncdisplaykit_async_transaction Once all rendering is complete for the current
 * asyncdisplaykit_async_transaction, the completion for the block sets the contents on all of the
 ...
 *
 * @discussion A subclass may check this flag during -display or -drawInContext: to cancel a display
 ...
 *
 * If a setNeedsDisplay occurs while displaySuspended is YES, and displaySuspended is set to NO,
 * then the layer will be automatically displayed.
 */
 ...
 * If the node is removed from a visible hierarchy and then re-added, it will automatically trigger
 * a new asynchronous display, as long as displaySuspended is not set. If the node remains in the
 * hierarchy throughout, -setNeedsDisplay is required to trigger a new asynchronous display.
 *
...
/**
 * @abstract Toggle displaying a placeholder over the node that covers content until the node and
 * all subnodes are displayed.
 *
...
/**
 * @abstract Set the time it takes to fade out the placeholder when a node's contents are finished
 * displaying.
 *
...
/**
 * @abstract Determines drawing priority of the node. Nodes with higher priority will be drawn
 * earlier.
 *
 * @discussion Defaults to ASDefaultDrawingPriority. There may be multiple drawing threads, and some
 * of them may decide to perform operations in queued order (regardless of drawingPriority)
 */
 ...
 *
 * @discussion When set to a non-zero inset, increases the bounds for hit testing to make it easier
 * to tap or perform gestures on this node.  Default is UIEdgeInsetsZero.
 *
 * This affects the default implementation of -hitTest and -pointInside, so subclasses should call
 * super if you override it and want hitTestSlop applied.
 */
...
- (CGPoint)convertPoint:(CGPoint)point
               fromNode:(nullable ASDisplayNode *)node AS_WARN_UNUSED_RESULT;
...
/**
 * @abstract Converts a rectangle from the coordinate system of another node to that of the
 * receiver.
 *
...
/**
 * Whether or not the node layout should be automatically updated when it receives
 ...
 *
 * Flattened layouts use less memory and are faster to lookup. On the other hand, unflattened
 * layouts are useful for debugging because they preserve original information.
 *
 * Defaults to NO.
 */
 ...
 * ## UIView bridge
 *
 * ASDisplayNode provides thread-safe access to most of UIView and CALayer properties and methods,
 * traditionally unsafe.
 *
 * Using them will not cause the actual view/layer to be created, and will be applied when it is
 * created (when the view or layer property is accessed).
 *
 * - NOTE: After the view or layer is created, the properties pass through to the view or layer
 * directly and must be called on the main thread.
 *
...
/**
 * Marks the view as needing display. Convenience for use whether the view / layer is loaded or not.
 ...
 * Safe to call from a background thread.
 *
 * If the node determines its own desired layout size will change in the next layout pass, it will
 * propagate this information up the tree so its parents can have a chance to consider and apply if
 * necessary the new size onto the node.
 *
...
/**
 * Performs a layout pass on the node. Convenience for use whether the view / layer is loaded or
 * not. Safe to call from a background thread.
 */
- (void)layoutIfNeeded;
 ...
 * There are three types of corner rounding provided by Texture: CALayer, Precomposited, and
 * Clipping.
 *
 ...
 *
 * - ASCornerRoundingTypePrecomposited: corners are drawn using bezier paths to clip the content in
 * a CGContext / UIGraphicsContext. This requires .backgroundColor and .cornerRadius to be set. Use
 * opaque background colors when possible for optimal efficiency, but transparent colors are
 * supported and much more efficient than CALayer. The only limitation of this approach is that it
 * cannot clip children, and thus works best for ASImageNodes or containers showing a background
 * around their children.
 *
 * - ASCornerRoundingTypeClipping: overlays 4 seperate opaque corners on top of the content that
 * needs corner rounding. Requires .backgroundColor and .cornerRadius to be set. Use clip corners in
 * situations in which is movement through the corner, with an opaque background (no movement
 * underneath the corner). Clipped corners are ideal for animating / resizing views, and still
 * outperform CALayer.
 *
 ...
 */
@property ASCornerRoundingType cornerRoundingType;  // default=ASCornerRoundingTypeDefaultSlowCALayer
                                                    // .cornerRadius (offscreen rendering)

/** @abstract The radius to use when rounding corners of the ASDisplayNode.
 *
 * @discussion This property is thread-safe and should always be preferred over CALayer's
 * cornerRadius property, even if corner rounding type is ASCornerRoundingTypeDefaultSlowCALayer.
 */
...
@property(getter=isUserInteractionEnabled)
    BOOL userInteractionEnabled;  // default=YES (NO for layer-backed nodes)
#if TARGET_OS_IOS
 ...
 *
 * @discussion This is like UIView's contentMode property, but better. We do our own mapping to
 * layer.contentsGravity in _ASDisplayView. You can set needsDisplayOnBoundsChange independently.
 * Thus, UIViewContentModeRedraw is not allowed; use needsDisplayOnBoundsChange = YES instead, and
 * pick an appropriate contentMode for your content while it's being re-rendered.
 */
...
@property UIViewAutoresizing
    autoresizingMask;  // default==UIViewAutoresizingNone (undefined for layer-backed nodes)
 ...
 * automaticallyRelayoutOnLayoutMarginsChanges to ensure that the layout gets automatically updated
 * when the value of this property changes. Or you can override layoutMarginsDidChange and make all
 * the necessary updates manually.
 */
@property UIEdgeInsets layoutMargins;
@property BOOL
    preservesSuperviewLayoutMargins;  // default is NO - set to enable pass-through or cascading
                                      // behavior of margins from this view’s parent to its children
 ...
 *
 * If your layout depends on this property, you should probably enable
 * automaticallyRelayoutOnSafeAreaChanges to ensure that the layout gets automatically updated when
 * the value of this property changes. Or you can override safeAreaInsetsDidChange and make all the
 * necessary updates manually.
 */
...
@property(nullable, copy)
    NSAttributedString *accessibilityAttributedLabel API_AVAILABLE(ios(11.0), tvos(11.0));
@property(nullable, copy) NSString *accessibilityHint;
@property(nullable, copy)
    NSAttributedString *accessibilityAttributedHint API_AVAILABLE(ios(11.0), tvos(11.0));
@property(nullable, copy) NSString *accessibilityValue;
@property(nullable, copy)
    NSAttributedString *accessibilityAttributedValue API_AVAILABLE(ios(11.0), tvos(11.0));
 ...
 *
 * @discussion Ideal for use by subclasses in -layout, having already prompted their subnodes to
 * calculate their size by calling -layoutThatFits: on them in -calculateLayoutThatFits.
 *
 * @return Size already calculated by -calculateLayoutThatFits:.
 *
 * @warning Subclasses must not override this; it returns the last cached measurement and is never
 * expensive.
 */
...
/**
 * @abstract The amount of time it takes to complete the default transition animation. Default is
 * 0.2.
 */
@property NSTimeInterval defaultLayoutTransitionDuration;

/**
 * @abstract The amount of time (measured in seconds) to wait before beginning the default
 * transition animation. Default is 0.0.
 */
...
/**
 * @discussion A place to perform your animation. New nodes have been inserted here. You can also
 * use this time to re-order the hierarchy.
 */
...
/**
 * @abstract Transitions the current layout with a new constrained size. Must be called on main
 * thread.
 *
 * @param animated Animation is optional, but will still proceed through your
 * `animateLayoutTransition` implementation with `isAnimated == NO`.
 ...
 *
 * @discussion If the passed constrainedSize is the the same as the node's current constrained size,
 * this method is noop. If passed YES to shouldMeasureAsync it's guaranteed that measurement is
 * happening on a background thread, otherwise measaurement will happen on the thread that the
 * method was called on. The measurementCompletion callback is always called on the main thread
...
/**
 * @abstract Invalidates the layout and begins a relayout of the node with the current
 * `constrainedSize`. Must be called on main thread.
 *
...
/**
 * @abstract A boolean that shows whether the node automatically inserts and removes nodes based on
 * the presence or absence of the node and its subnodes is completely determined in its
 * layoutSpecThatFits: method.
 *
 * @discussion If flag is YES the node no longer require addSubnode: or removeFromSupernode method
 * calls. The presence or absence of subnodes is completely determined in its layoutSpecThatFits:
 * method.
 */
...
/*
 * ASDisplayNode participates in ASAsyncTransactions, so you can determine when your subnodes are
 * done rendering. See: -(void)asyncdisplaykit_asyncTransactionContainerStateDidChange in
 * ASDisplayNodeSubclass.h
 */
...
/**
 * Convenience method, equivalent to [view addSubview:node.view] or [view.layer
 * addSublayer:node.layer] if layer-backed.
 *
