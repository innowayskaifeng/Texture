...
typedef ASTraitCollection *_Nonnull (^ASDisplayTraitsForTraitCollectionBlock)(
 ...
 * handles @c ASVisibilityDepth, automatic range mode and propogating @c ASDisplayTraits to
 * contained nodes.
 *
 * You can opt-out of node backed hierarchy and use it like a normal UIViewController.
 * More importantly, you can use it as a base class for all of your view controllers among which
 * some use a node hierarchy and some don't. See examples/ASDKgram project for actual
 * implementation.
 */
@interface ASViewController<__covariant DisplayNodeType : ASDisplayNode *>
    : UIViewController <ASVisibilityDepth>

/**
 * ASViewController initializer.
 *
 * @param node An ASDisplayNode which will provide the root view (self.view)
 * @return An ASViewController instance whose root view will be backed by the provided
 * ASDisplayNode.
 *
...
/**
 * Set this block to customize the ASDisplayTraits returned when the VC transitions to the given
 * traitCollection.
 */
 ...
 */
@property(nonatomic,
          copy) ASDisplayTraitsForTraitWindowSizeBlock overrideDisplayTraitsWithWindowSize
    ASDISPLAYNODE_DEPRECATED_MSG("This property is actually never accessed inside the framework");
...
/**
 * Automatically adjust range mode based on view events. If you set this to YES, the view controller
 * or its node must conform to the ASRangeControllerUpdateRangeProtocol.
 *
 * Default value is YES *if* node or view controller conform to ASRangeControllerUpdateRangeProtocol
 * otherwise it is NO.
 */
