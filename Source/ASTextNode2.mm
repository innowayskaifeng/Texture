 ...
 */
static NS_RETURNS_RETAINED ASTextLayout *ASTextNodeCompatibleLayoutWithContainerAndText(
    ASTextContainer *container, NSAttributedString *text) {
  ...
  // Lock the cache item for the rest of the method. Only after acquiring can we release the
  // NSCache.
  ...
  CGRect containerBounds = (CGRect){.size = container.size};
  {
    for (const auto &t : cacheValue->_layouts) {
      ...
      // 1. CoreText can return frames that are narrower than the constrained width, for obvious
      // reasons.
      // 2. CoreText can return frames that are slightly wider than the constrained width, for some
      // reason.
      //    We have to trust that somehow it's OK to try and draw within our size constraint,
      //    despite the return value.
      // 3. Thus, those two values (constrained width & returned width) form a range, where
      //    intermediate values in that range will be snapped. Thus, we can use a given layout as
      //    long as our width is in that range, between the min and max of those two values.
  ...
  // Store the result in the cache.
  {
    ...
    // another thread requests this cache item while a layout is being calculated, so they don't
    // race.
...
- (void)setTextContainerInset:(UIEdgeInsets)textContainerInset {
  ASLockScopeSelf();
  if (ASCompareAssignCustom(_textContainer.insets, textContainerInset,
...
- (CGSize)calculateSizeThatFits:(CGSize)constrainedSize {
  ...
  // If the constrained size has a max/inf value on the text's forward direction, the text node is
  // calculating its intrinsic size. Need to consider both width and height when determining if it
  // is calculating instrinsic size. Even the constrained width is provided, the height can be inf
  // it may provide a text that is longer than the width and require a wordWrapping line break mode
  // and looking for the height to be calculated.
  ...
  ASTextLayout *layout =
      ASTextNodeCompatibleLayoutWithContainerAndText(_textContainer, mutableText);
  if (layout.truncatedLine != nil &&
      layout.truncatedLine.size.width > layout.textBoundingSize.width) {
    return (CGSize){MIN(constrainedSize.width, layout.truncatedLine.size.width),
                    layout.textBoundingSize.height};
  }
...
- (void)setAttributedText:(NSAttributedString *)attributedText {
  ...
  self.isAccessibilityElement =
      (length != 0);  // We're an accessibility element by default if there is a string.
  ...
  [attributedString
      enumerateAttribute:NSParagraphStyleAttributeName
              ...
              usingBlock:^(NSParagraphStyle *style, NSRange range, BOOL *_Nonnull stop) {
                ...
                // Only "left" and "justified" alignments are supported while calculating intrinsic
                // size. Other alignments like "right", "center" and "natural" cause the size to be
                // bigger than needed and thus should be ignored/overridden.
...
- (NSObject *)drawParametersForAsyncLayer:(_ASDisplayLayer *)layer {
  ...
  NSMutableAttributedString *mutableText =
      [_attributedText mutableCopy] ?: [[NSMutableAttributedString alloc] init];
...
- (id)_linkAttributeValueAtPoint:(CGPoint)point
  ...
  ASTextLayout *layout =
      ASTextNodeCompatibleLayoutWithContainerAndText(containerCopy, _attributedText);
  ...
  for (__strong NSString *attributeName in self.linkAttributeNames) {
    id value = [self.attributedText attribute:attributeName
    ...
    if (highlighting &&
        [delegate respondsToSelector:@selector(textNode:
                                         shouldHighlightLinkAttribute:value:atPoint:)] &&
...
- (BOOL)_locked_pointInsideAdditionalTruncationMessage:(CGPoint)point
                                            withLayout:(ASTextLayout *)layout {
    ...
    if (stringIndexForPosition != kCFNotFound) {
      ...
      switch (_textContainer.truncationType) {
        ...
        case ASTextTruncationTypeMiddle: {
          CFIndex composedTruncationTextLineGlyphCount =
              truncationTokenLineGlyphCount + additionalTruncationTokenLineGlyphCount;
          CFIndex firstTruncatedTokenIndex =
              (truncatedCTLineGlyphCount - composedTruncationTextLineGlyphCount) / 2.0;
          if ((firstTruncatedTokenIndex + truncationTokenLineGlyphCount) < stringIndexForPosition &&
              stringIndexForPosition <
                  (firstTruncatedTokenIndex + composedTruncationTextLineGlyphCount)) {
            inAdditionalTruncationMessage = YES;
          }
          break;
        }
        case ASTextTruncationTypeEnd: {
          if (stringIndexForPosition >
              (truncatedCTLineGlyphCount - additionalTruncationTokenLineGlyphCount)) {
            inAdditionalTruncationMessage = YES;
          }
          break;
        }
...
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gestureRecognizer {
  ...
  if (gestureRecognizer == _longPressGestureRecognizer) {
    ...
    id<ASTextNodeDelegate> delegate = self.delegate;
    if ([delegate respondsToSelector:@selector(textNode:
...
- (BOOL)_pendingTruncationTap {
  return [ASLockedSelf(_highlightedLinkAttributeName)
 ...
 * We should probably ignore these properties and tell developers to set the shadow into their
 * attributed text instead.
 */
...
- (BOOL)isTruncated {
  return ASLockedSelf(
      [self locked_textLayoutForSize:[self _locked_threadSafeBounds].size].truncatedLine != nil);
}
