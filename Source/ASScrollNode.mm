...
- (ASLayout *)calculateLayoutThatFits:(ASSizeRange)constrainedSize
  ...
  if (_automaticallyManagesContentSize) {
    // To understand this code, imagine we're containing a horizontal stack set within a vertical
    // table node. Our parentSize is fixed ~375pt width, but 0 - INF height.  Our stack measures
    // 1000pt width, 50pt height. In this case, we want our scrollNode.bounds to be 375pt wide, and
    // 50pt high.  ContentSize 1000pt, 50pt. We can achieve this behavior by:
...
- (void)layout {
  ...
  if (_automaticallyManagesContentSize) {
    ...
    if (ASIsCGSizeValidForLayout(contentSize) == NO) {
      NSLog(@"%@ calculated a size in its layout spec that can't be applied to .contentSize: %@. "
            @"Applying parentSize (scrollNode's bounds) instead: %@.",
