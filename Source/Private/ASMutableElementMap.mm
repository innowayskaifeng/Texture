...
typedef NSMutableArray<NSMutableArray<ASCollectionElement *> *>
    ASMutableCollectionElementTwoDimensionalArray;
...
- (instancetype)initWithSections:(NSArray<ASSection *> *)sections
  ...
  if (self = [super init]) {
    ...
    _supplementaryElements =
        [ASMutableElementMap deepMutableCopyOfElementsDictionary:supplementaryElements];
  }
  return self;
}
...
- (void)removeSupplementaryElementsAtIndexPaths:(NSArray<NSIndexPath *> *)indexPaths
                                           kind:(NSString *)kind {
...
- (void)migrateSupplementaryElementsWithSectionMapping:(ASIntegerMap *)mapping {
  ...
  [_supplementaryElements
      enumerateKeysAndObjectsUsingBlock:^(
        ...
        // Note: it's tempting to update the dictionary in-place but because of the likely collision
        // between old and new index paths, subtle bugs are possible. Note that this process is rare
        // (only on section-level updates), that this work is done off-main, and that the typical
        // supplementary element use case is just 1-per-section (header).
        ...
        [supps enumerateKeysAndObjectsUsingBlock:^(NSIndexPath *_Nonnull oldIndexPath,
                                                   ASCollectionElement *_Nonnull obj,
                                                   BOOL *_Nonnull stop) {
          ...
          if (oldSection == newSection) {
            ...
            NSIndexPath *newIndexPath = [NSIndexPath indexPathForItem:oldIndexPath.item
...
+ (ASMutableSupplementaryElementDictionary *)deepMutableCopyOfElementsDictionary:
  ...
  [originalDict
      enumerateKeysAndObjectsUsingBlock:^(
          NSString *_Nonnull key, NSDictionary<NSIndexPath *, ASCollectionElement *> *_Nonnull obj,
          BOOL *_Nonnull stop) {
