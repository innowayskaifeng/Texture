...
static const ASRangeTuningParameters kASDefaultMeasureRangeTuningParameters = {
    .leadingBufferScreenfuls = 2.0, .trailingBufferScreenfuls = 2.0};

static const ASScrollDirection kASStaticScrollDirection =
    (ASScrollDirectionRight | ASScrollDirectionDown);
...
+ (ASCollectionLayoutState *)calculateLayoutWithContext:(ASCollectionLayoutContext *)context {
  ...
  ASCollectionLayoutState *layout =
      [context.layoutDelegateClass calculateLayoutWithContext:context];
  ...
  CGRect initialRect =
      CGRectMake(contentOffset.x, contentOffset.y, viewportSize.width, viewportSize.height);
  CGRect measureRect = CGRectExpandToRangeWithScrollableDirections(
      initialRect, kASDefaultMeasureRangeTuningParameters, context.scrollableDirections,
      kASStaticScrollDirection);
  // The first call to -layoutAttributesForElementsInRect: will be with a rect that is way bigger
  // than initialRect here. If we only block on initialRect, a few elements that are outside of
  // initialRect but inside measureRect may not be available by the time
  // -layoutAttributesForElementsInRect: is called. Since this method is usually run off main, let's
  // spawn more threads to measure and block on all elements in measureRect.
...
- (void)prepareLayout {
  ...
  ASCollectionLayoutContext *context =
      [self layoutContextWithElements:_collectionNode.visibleElements];
...
/**
 * NOTE: It is suggested practice on the Web to override
 * invalidationContextForInteractivelyMovingItems… and call out to the data source to move the item
 * (so that if e.g. the item size depends on the data, you get the data you expect). However, as of
 * iOS 11 this doesn't work, because UICV machinery will also call out to the data source to move
 * the item after the interaction is done. The result is that your data source state will be
 * incorrect due to this last move call. Plus it's just an API violation.
 *
 * Things tried:
 *   - Doing the speculative data source moves, and then UNDOING the last one in
 * invalidationContextForEndingInteractiveMovementOfItems… but this does not work because the UICV
 * machinery informs its data source before it calls that method on us, so we are too late.
 *
 * The correct practice is to use the UIDataSourceTranslating API introduced in iOS 11. Currently
 * Texture does not support this API but we can build it if there is demand. We could add an
 * id<UIDataSourceTranslating> field onto the layout context object, and the layout client can use
 * data source index paths when it reads nodes or other data source data.
 */
...
- (NSArray<UICollectionViewLayoutAttributes *> *)layoutAttributesForElementsInRect:
  ...
  CGRect measureRect = CGRectExpandToRangeWithScrollableDirections(
      blockingRect, kASDefaultMeasureRangeTuningParameters, _layout.context.scrollableDirections,
      kASStaticScrollDirection);
  [ASCollectionLayout _measureElementsInRect:measureRect blockingRect:blockingRect layout:_layout];

  NSArray<UICollectionViewLayoutAttributes *> *result =
      [_layout layoutAttributesForElementsInRect:blockingRect];
...
- (BOOL)shouldInvalidateLayoutForBoundsChange:(CGRect)newBounds {
  return (!CGSizeEqualToSize([ASCollectionLayout _boundsForCollectionNode:_collectionNode],
                             newBounds.size));
}
...
+ (CGSize)_viewportSizeForCollectionNode:(nonnull ASCollectionNode *)collectionNode
  ...
  // TODO: Consider using adjustedContentInset on iOS 11 and later, to include the safe area of the
  // scroll view
...
/**
 * Measures all elements in the specified rect and blocks the calling thread while measuring those
 * in the blocking rect.
 */
+ (void)_measureElementsInRect:(CGRect)rect
                  blockingRect:(CGRect)blockingRect
                        layout:(ASCollectionLayoutState *)layout {
  ...
  ASPageToLayoutAttributesTable *attrsTable =
      [layout getAndRemoveUnmeasuredLayoutAttributesPageTableInRect:rect];
  ...
  // Use ordered sets here because some items may span multiple pages, and the sets will be accessed
  // by indexes later on.
  ...
  NSMutableOrderedSet<UICollectionViewLayoutAttributes *> *nonBlockingAttrs =
  ...
  for (id pagePtr in attrsTable) {
    ...
    if (hasBlockingRect && CGRectContainsRect(blockingRect, pageRect)) {
      ...
      // The page intersects the blocking rect. Some elements in this page are blocking, some are
      // not.
...
ASDISPLAYNODE_INLINE void ASCollectionLayoutSetSizeToElement(CGSize size,
