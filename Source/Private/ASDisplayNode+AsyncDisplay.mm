...
- (void)_recursivelyRasterizeSelfAndSublayersWithIsCancelledBlock:
  ...
  // if super node is rasterizing descendants, subnodes will not have had layout calls because they
  // don't have layers
  ...
  // If this is the root container node, use a frame with a zero origin to draw into. If not,
  // calculate the correct frame using the node's position, transform and anchorPoint.
  if (self.rasterizesSubtree) {
    ...
    // Pretty hacky since full 3D transforms aren't actually supported, but attempt to compute the
    // transformed frame of this node so that we can composite it into approximately the right spot.
  ...
  // We'll display something if there is a display block, clipping, translation and/or a background
  // color.
  ...
  if (shouldDisplay) {
    dispatch_block_t pushAndDisplayBlock = ^{
      ...
      // If there is a display block, call it to get the image, then copy the image into the current
      // context (which is the rasterized container's backing store).
      if (displayBlock) {
  ...
  for (ASDisplayNode *subnode in self.subnodes) {
    [subnode _recursivelyRasterizeSelfAndSublayersWithIsCancelledBlock:isCancelledBlock
...
- (asyncdisplaykit_async_transaction_operation_block_t)
  ...
  // We always create a graphics context, unless a -display method is used, OR if we are a subnode
  // drawing into a rasterized parent.
  ...
  // Capture drawParameters from delegate on main thread, if this node is displaying itself rather
  // than recursively rasterizing.
  ...
  ASDisplayNodeAssert(rasterizing || !(_hierarchyState & ASHierarchyStateRasterized),
                      @"Rasterized descendants should never display unless being drawn into the "
  ...
  if (shouldBeginRasterizing) {
    ...
    [self _recursivelyRasterizeSelfAndSublayersWithIsCancelledBlock:isCancelledBlock
                                                      displayBlocks:displayBlocks];
    CHECK_CANCELLED_AND_RETURN_NIL();

    // If [UIColor clearColor] or another semitransparent background color is used, include alpha
    // channel when rasterizing. Unlike CALayer drawing, we include the backgroundColor as a base
    ...
    displayBlock = ^id {
      ...
      // _willDisplayNodeContentWithRenderingContext or _didDisplayNodeContentWithRenderingContext
      // blocks. It's up to the implementation of -display... to do what it needs.
      [self __willDisplayNodeContentWithRenderingContext:currentContext
                                          drawParameters:drawParameters];

      if (usesImageDisplay) {  // If we are using a display method, we'll get an image back
                               // directly.
        image = [self.class displayWithParameters:drawParameters isCancelled:isCancelledBlock];
      } else if (usesDrawRect) {  // If we're using a draw method, this will operate on the
                                  // currentContext.
...
- (void)__didDisplayNodeContentWithRenderingContext:(CGContextRef)context
  ...
  if (cornerRoundingType == ASCornerRoundingTypePrecomposited && cornerRadius > 0.0f) {
    ...
    ASDisplayNodeAssert(UIGraphicsGetCurrentContext(),
                        @"context is expected to be pushed on UIGraphics stack %@", self);
    ...
    if (borderWidth > 0.0f) {  // Don't create roundedPath and stroke if borderWidth is 0.0
      ...
      roundedPath =
          [UIBezierPath bezierPathWithRoundedRect:CGRectInset(bounds, strokeInset, strokeInset)
...
- (void)displayAsyncLayer:(_ASDisplayLayer *)asyncLayer asynchronously:(BOOL)asynchronously {
  ...
  // for async display, capture the current displaySentinel value to bail early when the job is
  // executed if another is enqueued for sync display, do not support cancellation

  // FIXME: what about the degenerate case where we are calling setNeedsDisplay faster than the jobs
  // are dequeuing from the displayQueue?  Need to not cancel early fails from displaySentinel
  // changes.
  ...
  // Set up displayBlock to call either display or draw on the delegate and return a UIImage
  // contents
  asyncdisplaykit_async_transaction_operation_block_t displayBlock =
  ...
  // This block is called back on the main thread after rendering at the completion of the current
  // async transaction, or immediately if !asynchronously
  ...
  if (asynchronously) {
    // Async rendering operations are contained by a transaction, which allows them to proceed and
    // concurrently while synchronizing the final application of the results to the layer's contents
    // property (completionBlock).

    // First, look to see if we are expected to join a parent's transaction container.
    CALayer *containerLayer = layer.asyncdisplaykit_parentTransactionContainer ?: layer;

    // In the case that a transaction does not yet exist (such as for an individual node outside of
    // a container), this call will allocate the transaction and add it to _ASAsyncTransactionGroup.
...
- (void)setWillDisplayNodeContentWithRenderingContext:
    (ASDisplayNodeContextModifier)contextModifier {
  ASDN::MutexLocker l(__instanceLock__);
