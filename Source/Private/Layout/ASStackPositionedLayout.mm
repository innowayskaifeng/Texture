...
static void positionItemsInLine(const ASStackUnpositionedLine &line,
  ...
  for (const auto &item : line.items) {
    ...
    item.layout.position =
        p + directionPoint(style.direction, 0,
                           crossOffsetForItem(item, style, line.crossSize, line.baseline));

    p = p + directionPoint(
                style.direction,
                stackDimension(style.direction, item.layout.size) + item.child.style.spacingAfter,
                0);
  }
}
...
ASStackPositionedLayout ASStackPositionedLayout::compute(const ASStackUnpositionedLayout &layout,
  ...
  crossOffsetAndSpacingForEachLine(numOfLines, crossViolation, alignContent, crossOffset,
                                   crossSpacing);
  ...
  for (const auto &line : lines) {
    ...
    stackOffsetAndSpacingForEachItem(items.size(), stackViolation, justifyContent, stackOffset,
                                     stackSpacing);
  ...
  const CGSize finalSize =
      directionSize(direction, layout.stackDimensionSum, layout.crossDimensionSum);
