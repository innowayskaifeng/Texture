...
static CGFloat resolveCrossDimensionMaxForStretchChild(const ASStackLayoutSpecStyle &style,
  ...
  // stretched children may have a cross direction max that is smaller than the minimum size
  // constraint of the parent.
  const CGFloat computedMax =
      (style.direction == ASStackLayoutDirectionVertical
           ? ASLayoutElementSizeResolve(child.style.size, ASLayoutElementParentSizeUndefined)
                 .max.width
           : ASLayoutElementSizeResolve(child.style.size, ASLayoutElementParentSizeUndefined)
                 .max.height);
...
static CGFloat resolveCrossDimensionMinForStretchChild(const ASStackLayoutSpecStyle &style,
  ...
  // stretched children will have a cross dimension of at least crossMin, unless they explicitly
  // define a child size that is smaller than the constraint of the parent.
...
static ASLayout *crossChildLayout(const ASStackLayoutSpecChild &child,
  ...
  ASDisplayNodeCAssertNotNil(
      layout, @"ASLayout returned from -layoutThatFits:parentSize: must not be nil: %@",
      child.element);
...
/**
 Computes the violation by comparing a cross dimension sum with the overall allowable size range for
 the stack.

 Violation is the distance you would have to add to the unbounded cross-direction length of the
 stack spec's lines in order to bring the stack within its allowed sizeRange.  The diagram below
 shows 3 vertical stacks, each contains 3-5 vertical lines, with the different types of violation.
...
/**
 Stretches children to lay out along the cross axis according to the alignment stretch settings of
 the children (child.alignSelf), and the stack layout's alignment settings (style.alignItems).  This
 does not do the actual alignment of the items once stretched though; ASStackPositionedLayout will
 do centering etc.

 Finds the maximum cross dimension among child layouts.  If that dimension exceeds the minimum cross
 layout size then we must stretch any children whose alignItems specify
 ASStackLayoutAlignItemsStretch.

 The diagram below shows 3 children in a horizontal stack.  The second child is larger than the
 minCrossDimension, so its height is used as the childCrossMax.  Any children that are stretchable
 (which may be all children if style.alignItems specifies stretch) like the first child must be
 stretched to match that maximum.  All children must be at least minCrossDimension in cross
 dimension size, which is shown by the sizing of the third child.
...
static void stretchItemsAlongCrossDimension(std::vector<ASStackLayoutSpecItem> &items,
  ...
  dispatchApplyIfNeeded(items.size(), concurrent, ^(size_t i) {
    auto &item = items[i];
    const ASStackLayoutAlignItems alignItems =
        alignment(item.child.style.alignSelf, style.alignItems);
    if (alignItems == ASStackLayoutAlignItemsStretch) {
      ...
      // Only stretch if violation is positive. Compare against kViolationEpsilon here to avoid
      // stretching against a tiny violation.
      if (violation > kViolationEpsilon) {
        item.layout =
            crossChildLayout(item.child, style, stack, stack, crossSize, crossSize, parentSize);
      }
    }
  });
}
...
static void stretchLinesAlongCrossDimension(std::vector<ASStackUnpositionedLine> &lines,
  ...
  const CGFloat violation = ASStackUnpositionedLayout::computeCrossViolation(
      computeLinesCrossDimensionSum(lines, style), style, sizeRange);
  // Don't stretch if the stack is single line, because the line's cross size was clamped against
  // the stack's constrained size.
  const BOOL shouldStretchLines =
      (numOfLines > 1 && style.alignContent == ASStackLayoutAlignContentStretch &&
...
static BOOL itemIsBaselineAligned(const ASStackLayoutSpecStyle &style,
  ...
  return alignItems == ASStackLayoutAlignItemsBaselineFirst ||
...
static void computeLinesCrossSizeAndBaseline(std::vector<ASStackUnpositionedLine> &lines,
  ...
  // If the stack is single-line and has a definite cross size, the cross size of the line is the
  // stack's definite cross size.
  ...
  for (auto &line : lines) {
    ...
    for (const auto &item : items) {
      if (itemIsBaselineAligned(style, item)) {
        // Step 1. Collect all the items whose align-self is baseline. Find the largest of the
        // distances between each item’s baseline and its hypothetical outer cross-start edge (aka.
        // its baseline value), and the largest of the distances between each item’s baseline and
        ...
        maxBaselineToEndDistance = MAX(
            maxBaselineToEndDistance, crossDimension(style.direction, item.layout.size) - baseline);
      } else {
        // Step 2. Among all the items not collected by the previous step, find the largest outer
        // hypothetical cross size.
        maxItemCrossSize = MAX(maxItemCrossSize, crossDimension(style.direction, item.layout.size));
      }
    }

    // Step 3. The used cross-size of the flex line is the largest of the numbers found in the
    // previous two steps and zero.
    line.crossSize = MAX(maxStartToBaselineDistance + maxBaselineToEndDistance, maxItemCrossSize);
    if (isSingleLine) {
      // If the stack is single-line, then clamp the line’s cross-size to be within the stack's min
      // and max cross-size properties.
...
/**
 ...
 @param violation The amount that the stack layout violates its size range.  See header for sign
 interpretation.
 */
static std::function<CGFloat(const ASStackLayoutSpecItem &)> flexFactorInViolationDirection(
...
static inline CGFloat scaledFlexShrinkFactor(const ASStackLayoutSpecItem &item,
  ...
  return stackDimension(style.direction, item.layout.size) *
         (item.child.style.flexShrink / flexFactorSum);
}

/**
 Returns a lambda that computes a flex shrink adjustment for a given item based on the provided
 violation.
...
static std::function<CGFloat(const ASStackLayoutSpecItem &)> flexShrinkAdjustment(
    ...
    const CGFloat violation,
    const CGFloat flexFactorSum) {
  const CGFloat scaledFlexShrinkFactorSum = std::accumulate(
      items.begin(), items.end(), 0.0, [&](CGFloat x, const ASStackLayoutSpecItem &item) {
        return x + scaledFlexShrinkFactor(item, style, flexFactorSum);
      });
  return [style, scaledFlexShrinkFactorSum, violation,
          flexFactorSum](const ASStackLayoutSpecItem &item) {
    ...
    // Unlike the flex grow adjustment the flex shrink adjustment needs to take the size of each
    // item into account.
...
/**
 Returns a lambda that computes a flex grow adjustment for a given item based on the provided
 violation.
 @param items The unpositioned items from the original unconstrained layout pass.
 @param violation The amount that the stack layout violates its size range.
 @param flexFactorSum The sum of each item's flex factor as determined by the provided violation.
...
static std::function<CGFloat(const ASStackLayoutSpecItem &)> flexGrowAdjustment(
    ...
    const CGFloat violation,
    const CGFloat flexFactorSum) {
  // To compute the flex grow adjustment distribute the violation proportionally based on each
  // item's flex grow factor.
...
/**
 The flexible children may have been left not laid out in the initial layout pass, so we may have to
 go through and size these children at zero size so that the children layouts are at least present.
 */
static void layoutFlexibleChildrenAtZeroSize(std::vector<ASStackLayoutSpecItem> &items,
  ...
  dispatchApplyIfNeeded(items.size(), concurrent, ^(size_t i) {
    ...
    if (isFlexibleInBothDirections(item.child)) {
      item.layout =
          crossChildLayout(item.child, style, 0, 0, crossDimension(style.direction, sizeRange.min),
...
static CGFloat computeItemsStackDimensionSum(const std::vector<ASStackLayoutSpecItem> &items,
  ...
  const CGFloat childSpacingSum =
      std::accumulate(items.begin(), items.end(),
  ...
  const CGFloat childStackDimensionSum = std::accumulate(
      items.begin(), items.end(), childSpacingSum, [&](CGFloat x, const ASStackLayoutSpecItem &l) {
        return x + stackDimension(style.direction, l.layout.size);
      });
  return childStackDimensionSum;
}

// TODO move this up near computeCrossViolation and make both methods share the same code path, to
// make sure they share the same concept of "negative" and "positive" violations.
/**
 Computes the violation by comparing a stack dimension sum with the overall allowable size range for
 the stack.

 Violation is the distance you would have to add to the unbounded stack-direction length of the
 stack spec's children in order to bring the stack within its allowed sizeRange.  The diagram below
 shows 3 horizontal stacks with the different types of violation.
...
/**
 If we have a single flexible (both shrinkable and growable) child, and our allowed size range is
 set to a specific number then we may avoid the first "intrinsic" size calculation.
 */
ASDISPLAYNODE_INLINE BOOL useOptimizedFlexing(const std::vector<ASStackLayoutSpecChild> &children,
                                              const ASStackLayoutSpecStyle &style,
                                              const ASSizeRange &sizeRange) {
  const NSUInteger flexibleChildren =
      std::count_if(children.begin(), children.end(), isFlexibleInBothDirections);
  return ((flexibleChildren == 1) && (stackDimension(style.direction, sizeRange.min) ==
                                      stackDimension(style.direction, sizeRange.max)));
}

/**
 Flexes children in the stack axis to resolve a min or max stack size violation. First, determines
 which children are flexible (see computeStackViolation and isFlexibleInViolationDirection). Then
 computes how much to flex each flexible child and performs re-layout. Note that there may still be
 a non-zero violation even after flexing.

 The actual CSS flexbox spec describes an iterative looping algorithm here, which may be adopted in
 t5837937: http://www.w3.org/TR/css3-flexbox/#resolve-flexible-lengths

 @param lines reference to unpositioned lines and items from the original, unconstrained layout
 pass; modified in-place
 @param style layout style to be applied to all children
 @param sizeRange the range of allowable sizes for the stack layout component
 @param parentSize Size of the stack layout component. May be undefined in either or both
 directions.
 */
static void flexLinesAlongStackDimension(std::vector<ASStackUnpositionedLine> &lines,
  ...
  for (auto &line : lines) {
    ...
    const CGFloat violation = ASStackUnpositionedLayout::computeStackViolation(
        computeItemsStackDimensionSum(items, style), style, sizeRange);
    std::function<CGFloat(const ASStackLayoutSpecItem &)> flexFactor =
        flexFactorInViolationDirection(violation);
    // The flex factor sum is needed to determine if flexing is necessary.
    // This value is also needed if the violation is positive and flexible items need to grow, so
    // keep it around.
    const CGFloat flexFactorSum = std::accumulate(
    ...
    if (flexFactorSum == 0) {
      // If optimized flexing was used then we have to clean up the unsized items and lay them out
      // at zero size.
    ...
    const CGFloat remainingViolation = std::accumulate(
        items.begin(), items.end(), violation,
    ...
    dispatchApplyIfNeeded(items.size(), concurrent, ^(size_t i) {
      ...
      if (currentFlexAdjustment != 0) {
        ...
        // Only apply the remaining violation for the first flexible item that has a flex grow
        // factor.
        const CGFloat flexedStackSize =
            originalStackSize + currentFlexAdjustment +
            (i == firstFlexItem && item.child.style.flexGrow > 0 ? remainingViolation : 0);
        item.layout =
            crossChildLayout(item.child, style, MAX(flexedStackSize, 0), MAX(flexedStackSize, 0),
 ...
 */
static std::vector<ASStackUnpositionedLine> collectChildrenIntoLines(
    const std::vector<ASStackLayoutSpecItem> &items,
    const ASStackLayoutSpecStyle &style,
    const ASSizeRange &sizeRange) {
  ...
  for (auto it = items.begin(); it != items.end(); ++it) {
    ...
    const BOOL negativeViolationIfAddItem =
        (ASStackUnpositionedLayout::computeStackViolation(
             lineStackDimensionSum + interitemSpacing + itemAndSpacingStackDimension, style,
             sizeRange) < 0);
...
/**
 Performs the first unconstrained layout of the children, generating the unpositioned items that are
 then flexed and stretched.
 */
  ...
  dispatchApplyIfNeeded(items.size(), concurrent, ^(size_t i) {
    ...
    if (useOptimizedFlexing && isFlexibleInBothDirections(item.child)) {
      ...
      item.layout = crossChildLayout(
          item.child, style,
          ASDimensionResolve(item.child.style.flexBasis,
                             stackDimension(style.direction, parentSize), 0),
          ASDimensionResolve(item.child.style.flexBasis,
                             stackDimension(style.direction, parentSize), INFINITY),
          minCrossDimension, maxCrossDimension, parentSize);
    }
  });
}

ASStackUnpositionedLayout ASStackUnpositionedLayout::compute(
    const std::vector<ASStackLayoutSpecChild> &children,
    const ASStackLayoutSpecStyle &style,
    const ASSizeRange &sizeRange,
    const BOOL concurrent) {
  if (children.empty()) {
  ...
  // If we have a fixed size in either dimension, pass it to children so they can resolve
  // percentages against it. Otherwise, we pass ASLayoutElementParentDimensionUndefined since it
  // will depend on the content.
  const CGSize parentSize = {
      (sizeRange.min.width == sizeRange.max.width) ? sizeRange.min.width
                                                   : ASLayoutElementParentDimensionUndefined,
      (sizeRange.min.height == sizeRange.max.height) ? sizeRange.min.height
                                                     : ASLayoutElementParentDimensionUndefined,
  };
  ...
  // We do a first pass of all the children, generating an unpositioned layout for each with an
  // unbounded range along the stack dimension.  This allows us to compute the "intrinsic" size of
  // each child and find the available violation which determines whether we must grow or shrink the
  // flexible children.
  layoutItemsAlongUnconstrainedStackDimension(items, style, concurrent, sizeRange, parentSize,
                                              optimizedFlexing);
  ...
  // This should be done before `lines` are moved to a new ASStackUnpositionedLayout struct (i.e
  // `std::move(lines)`)
