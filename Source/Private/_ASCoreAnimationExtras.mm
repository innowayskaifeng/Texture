...
void ASDisplayNodeSetResizableContents(id<ASResizableContents> obj, UIImage *image) {
  // FIXME (https://github.com/TextureGroup/Texture/issues/1046): This method does not currently
  // handle UIImageResizingModeTile, which is the default. See also
  // https://developer.apple.com/documentation/uikit/uiimage/1624157-resizingmode?language=objc I'm
  // not sure of a way to use CALayer directly to perform such tiling on the GPU, though the stretch
  // is handled by the GPU, and CALayer.h documents the fact that contentsCenter is used to stretch
  // the pixels.
  ...
  if (image) {
    ASDisplayNodeCAssert(
        ...
        @"Image insets must be all-zero or resizingMode has to be UIImageResizingModeStretch. "
        @"XCode assets default value is UIImageResizingModeTile which is not supported by Texture "
        @"because of GPU-accelerated CALayer features.");
    ...
    // These are lifted from what UIImageView does by experimentation. Without these exact values,
    // the stretching is slightly off.
    ...
    if (insets.left > 0 || insets.right > 0) {
      ...
      contentsCenter.size.width =
          (imageSize.width - (insets.left + insets.right + 1.f) + otherPixelFudge) /
          imageSize.width;
    }
    if (insets.top > 0 || insets.bottom > 0) {
      contentsCenter.origin.y = ((insets.top + halfPixelFudge) / imageSize.height);
      contentsCenter.size.height =
          (imageSize.height - (insets.top + insets.bottom + 1.f) + otherPixelFudge) /
          imageSize.height;
    }
  ...
  for (size_t i = 0; i < lutSize; ++i) {
    if (ASObjectIsEqual(lut[i].string, contentsGravity)) {
      ...
      if (currentCacheIndex < ContentModeCacheSize) {
        // Cache the input value.  This is almost always a different pointer than in our LUT and
        // will frequently be the same value for an overwhelming majority of inputs.
