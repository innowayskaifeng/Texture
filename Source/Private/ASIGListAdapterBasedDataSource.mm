...
- (instancetype)initWithListAdapter:(IGListAdapter *)listAdapter
  ...
  if (self = [super init]) {
    ...
    ASDisplayNodeAssert(
        [listAdapter conformsToProtocol:@protocol(UICollectionViewDelegateFlowLayout)],
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView
  ...
  // IGListAdapter doesn't implement scrollViewWillEndDragging yet (pending pull request), so we
  // need this check for now. Doesn't hurt to have it anyways :)
...
- (BOOL)shouldBatchFetchForCollectionNode:(ASCollectionNode *)collectionNode {
  ...
  // If they implement shouldBatchFetch, call it. Otherwise, just say YES if they implement
  // beginBatchFetch.
  ASIGSectionController *ctrl = [self sectionControllerForSection:sectionCount - 1];
  ASSectionControllerOverrides o =
      [ASIGListAdapterBasedDataSource overridesForSectionControllerClass:ctrl.class];
...
- (void)collectionNode:(ASCollectionNode *)collectionNode
  ...
  if ([_collectionDelegate respondsToSelector:@selector(collectionNode:
...
/**
 * Note: It is not documented that ASCollectionNode will forward these UIKit delegate calls if they
 * are implemented. It is not considered harmful to do so, and adding them to documentation will
 * confuse most users, who should instead using the ASCollectionDelegate callbacks.
 */
...
- (ASSizeRange)collectionNode:(ASCollectionNode *)collectionNode
  ...
  if ([ASIGListAdapterBasedDataSource overridesForSupplementarySourceClass:[src class]]
          .sizeRangeForSupplementary) {
    return [src sizeRangeForSupplementaryElementOfKind:UICollectionElementKindSectionHeader
                                               atIndex:0];
  } else {
...
- (ASSizeRange)collectionNode:(ASCollectionNode *)collectionNode
  ...
  if ([ASIGListAdapterBasedDataSource overridesForSupplementarySourceClass:[src class]]
          .sizeRangeForSupplementary) {
    return [src sizeRangeForSupplementaryElementOfKind:UICollectionElementKindSectionFooter
                                               atIndex:0];
  } else {
    return ASSizeRangeZero;
  }
...
- (ASSizeRange)collectionNode:(ASCollectionNode *)collectionNode
  ...
  ASIGSectionController *ctrl = [self sectionControllerForSection:indexPath.section];
  if ([ASIGListAdapterBasedDataSource overridesForSectionControllerClass:ctrl.class]
          .sizeRangeForItem) {
...
- (ASCellNodeBlock)collectionNode:(ASCollectionNode *)collectionNode
  ...
  id<ASSupplementaryNodeSource> ctrl =
      [self supplementaryElementSourceForSection:indexPath.section];
  ASDisplayNodeAssert(
      [ctrl respondsToSelector:@selector(nodeBlockForSupplementaryElementOfKind:atIndex:)],
      @"Expected section controller to respond to to %@. Controller: %@",
...
- (ASCellNode *)collectionNode:(ASCollectionNode *)collectionNode
  ...
  id<ASSupplementaryNodeSource> ctrl =
      [self supplementaryElementSourceForSection:indexPath.section];
  ASDisplayNodeAssert(
      [ctrl respondsToSelector:@selector(nodeForSupplementaryElementOfKind:atIndex:)],
      @"Expected section controller to respond to to %@. Controller: %@",
...
- (id<ASIGSupplementaryNodeSource>)supplementaryElementSourceForSection:(NSInteger)section {
  ...
  id<ASIGSupplementaryNodeSource> src =
      (id<ASIGSupplementaryNodeSource>)ctrl.supplementaryViewSource;
...
- (ASIGSectionController *)sectionControllerForSection:(NSInteger)section {
  ...
  ASIGSectionController *ctrl =
      (ASIGSectionController *)[_listAdapter sectionControllerForObject:object];
...
+ (ASSupplementarySourceOverrides)overridesForSupplementarySourceClass:(Class)c {
  ...
  if (obj == nil) {
    o.sizeRangeForSupplementary =
        [c instancesRespondToSelector:@selector(sizeRangeForSupplementaryElementOfKind:atIndex:)];
...
+ (ASSectionControllerOverrides)overridesForSectionControllerClass:(Class)c {
  ...
  if (obj == nil) {
    ...
    o.beginBatchFetchWithContext =
        [c instancesRespondToSelector:@selector(beginBatchFetchWithContext:)];
