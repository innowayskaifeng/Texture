...
/**
 Hierarchy state is propagated from nodes to all of their children when certain behaviors are
 required from the subtree. Examples include rasterization and external driving of the
 .interfaceState property. By passing this information explicitly, performance is optimized by
 avoiding iteration up the supernode chain. Lastly, this avoidance of supernode traversal protects
 against the possibility of deadlocks when a supernode is simultaneously attempting to materialize
 views / layers for its subtree (as many related methods require property locking)

 Note: as the hierarchy deepens, more state properties may be enabled.  However, state properties
 may never be disabled / cancelled below the point they are enabled.  They continue to the leaves of
 the hierarchy.
 */

typedef NS_OPTIONS(NSUInteger, ASHierarchyState) {
  /** The node may or may not have a supernode, but no supernode has a special hierarchy-influencing
     option enabled. */
...
ASHierarchyStateIncludesTransitioningSupernodes(ASHierarchyState hierarchyState) {
  return ((hierarchyState & ASHierarchyStateTransitioningSupernodes) ==
          ASHierarchyStateTransitioningSupernodes);
}
...
// These methods are recursive, and either union or remove the provided interfaceState to all
// sub-elements.
- (void)enterInterfaceState:(ASInterfaceState)interfaceState;
- (void)exitInterfaceState:(ASInterfaceState)interfaceState;
- (void)recursivelySetInterfaceState:(ASInterfaceState)interfaceState;

// These methods are recursive, and either union or remove the provided hierarchyState to all
// sub-elements.
...
// Call willEnterHierarchy if necessary and set inHierarchy = YES if visibility notifications are
// enabled on all of its parents
- (void)__enterHierarchy;
// Call didExitHierarchy if necessary and set inHierarchy = NO if visibility notifications are
// enabled on all of its parents
 ...
 *
 * @return The current ASHierarchyState of the node, indicating whether it is rasterized or managed
 * by a range controller.
 *
 ...
 *
 * @discussion Currently only set interface state on nodes in table and collection views. For other
 * nodes, if they are in the hierarchy we enable all ASInterfaceState types with
 ...
 *
 * @discussion Calling this method on the main thread after a node is added to the view hierarchy
 * will ensure that placeholder states are never visible to the user.  It is used by ASTableView,
 * ASCollectionView, and ASViewController to implement their respective ".neverShowPlaceholders"
 * option.
 *
 * If all nodes have layer.contents set and/or their layer does not have -needsDisplay set, the
 * method will return immediately.
 *
 * This method is capable of handling a mixed set of nodes, with some not having started display,
 * some in progress on an asynchronous display operation, and some already finished.
 *
 ...
 * This method is used to notify the node that it should purge any content that is both expensive to
 * fetch and to retain in memory.
 *
 * @see [ASDisplayNode(Subclassing) didExitPreloadState] and [ASDisplayNode(Subclassing)
 ...
 *
 * @see [ASDisplayNode(Subclassing) didEnterPreloadState] and [ASDisplayNode(Subclassing)
 * didExitPreloadState]
 */
- (void)recursivelyPreload;

/**
 * @abstract Triggers a recursive call to -didEnterPreloadState when the node has an interfaceState
 * of ASInterfaceStatePreload
 */
 ...
 *
 * ASImageNode uses the default of NO, as it is often used for UI images that are expected to
 * synchronize with ensureDisplay.
 *
 * ASNetworkImageNode and ASMultiplexImageNode set this to YES, because they load data from a
 * database or server, and are expected to support a placeholder state given that display is often
 * blocked on slow data fetching.
 */
@property BOOL shouldBypassEnsureDisplay;

/**
 * @abstract Checks whether a node should be scheduled for display, considering its current and new
 * interface states.
 */
- (BOOL)shouldScheduleDisplayWithNewInterfaceState:(ASInterfaceState)newInterfaceState;

/**
 * @abstract safeAreaInsets will fallback to this value if the corresponding UIKit property is not
 * available (due to an old iOS version).
 *
 * @discussion This should be set by the owning view controller based on it's layout guides.
 * If this is not a view controllet's node the value will be calculated automatically by the parent
 * node.
 */
 ...
 *
 * YES here only means that this node is used as an ASViewController node. It doesn't mean that this
 * node is a root of ASDisplayNode hierarchy, e.g. when its view controller is parented by another
 * ASViewController.
 */
...
/**
 * @abstract Subclass hook for nodes that are acting as root nodes. This method is called if one of
 * the subnodes size is invalidated and may need to result in a different size as the current
 * calculated size.
 */
...
/**
 * If one or multiple layout transitions are in flight this methods returns if the current layout
 * transition that happens in in this particular thread was invalidated through another thread is
 * starting a transition for this node
 */
...
/**
 * Internal method that can be overriden by subclasses to add specific behavior after the
 * measurement of a layout transition did finish.
 */
