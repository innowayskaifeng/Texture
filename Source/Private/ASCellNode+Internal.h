...
/**
 * Notifies the delegate that a specified cell node invalidates it's size what could result into a
 * size change.
 *
...
/*
 * Back-pointer to the containing scrollView instance, set only for visible cells.  Used for Cell
 * Visibility Event callbacks.
 */
 ...
 *   that it's always safe simply to retain it, and copy if needed. Since @c
 * UICollectionViewLayoutAttributes is always mutable, @c copy is never "free" like it is for e.g.
 * NSString.
 */
