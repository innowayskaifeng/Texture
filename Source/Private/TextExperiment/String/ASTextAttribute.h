...
typedef NS_OPTIONS(NSInteger, ASTextLineStyle) {
  ...
  ASTextLineStylePatternDashDot =
      0x300,  ///< (— ‑ — ‑ — ‑) Draw a line of alternating dashes and dots.
  ASTextLineStylePatternDashDotDot =
      0x400,  ///< (— ‑ ‑ — ‑ ‑) Draw a line of alternating dashes and two dots.
...
/// Use this attribute to store the original plain text if it is replaced by something else (such as
/// attachment).
...
UIKIT_EXTERN NSString *const
    ASTextAttachmentToken;  ///< Object replacement character (U+FFFC), used for text attachment.
UIKIT_EXTERN NSString
    *const ASTextTruncationToken;  ///< Horizontal ellipsis (U+2026), used for text truncation  "…".
 ...
 */
typedef void (^ASTextAction)(UIView *containerView,
...
@property(nullable, nonatomic)
    ASTextShadow *subShadow;  ///< a sub shadow which will be added above the parent shadow

+ (instancetype)shadowWithNSShadow:(NSShadow *)nsShadow
    NS_RETURNS_RETAINED;  ///< convert NSShadow to ASTextShadow
...
+ (instancetype)borderWithFillColor:(nullable UIColor *)color
                       cornerRadius:(CGFloat)cornerRadius NS_RETURNS_RETAINED;
