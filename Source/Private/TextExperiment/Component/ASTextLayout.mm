...
- (void)modifyLines:(NSArray *)lines
  ...
  if (container.verticalForm) {
    for (NSUInteger i = 0, max = lines.count; i < max; i++) {
      ...
      pos.x = container.size.width - container.insets.right - line.row * _fixedLineHeight -
              _fixedLineHeight * 0.9;
      line.position = pos;
    }
  } else {
    for (NSUInteger i = 0, max = lines.count; i < max; i++) {
...
- (NSString *)description {
  return [NSString stringWithFormat:@"lines: %ld, visibleRange:%@, textBoundingRect:%@",
                                    [self.lines count], NSStringFromRange(self.visibleRange),
...
#define FAIL_AND_RETURN                     \
  ...
  if (container.path == nil && container.exclusionPaths.count == 0) {
    ...
    cgPath = CGPathCreateWithRect(rect, NULL);  // let CGPathIsRect() returns true
             ...
             container.exclusionPaths.count == 0) {
    ...
    if (path) {
      [layout.container.exclusionPaths
          enumerateObjectsUsingBlock:^(UIBezierPath *onePath, NSUInteger idx, BOOL *stop) {
  ...
  ctFrame = CTFramesetterCreateFrame(ctSetter, ASTextCFRangeFromNSRange(range), cgPath,
                                     (CFDictionaryRef)frameAttrs);
    ...
    if (newRow && rowMaySeparated && position.x != lastPosition.x) {
      if (isVerticalForm) {
        if (rect.size.width > lastRect.size.width) {
          ...
          if (lastRect.origin.x > position.x &&
              position.x > lastRect.origin.x - lastRect.size.width)
            newRow = NO;
        }
      } else {
        if (rect.size.height > lastRect.size.height) {
          ...
          if (lastRect.origin.y < position.y &&
              position.y < lastRect.origin.y + lastRect.size.height)
            newRow = NO;
        }
      }
    }
    ...
    if (needTruncation) {
      ...
      if (container.truncationType != ASTextTruncationTypeNone) {
        ...
        if (container.truncationToken) {
          ...
          truncationTokenLine =
              CTLineCreateWithAttributedString((CFAttributedStringRef)truncationToken);
        } else {
          ...
          if (runCount > 0) {
            ...
            [attrs
                removeObjectsForKeys:[NSMutableAttributedString as_allDiscontinuousAttributeKeys]];
            ...
            if (uiFont) {
              font = CTFontCreateWithName((__bridge CFStringRef)uiFont.fontName, uiFont.pointSize,
                                          NULL);
            } else {
              font = NULL;
            }
          ...
          truncationToken = [[NSAttributedString alloc] initWithString:ASTextTruncationToken
                                                            attributes:attrs];
          truncationTokenLine =
              CTLineCreateWithAttributedString((CFAttributedStringRef)truncationToken);
        }
        if (truncationTokenLine) {
          ...
          NSMutableAttributedString *lastLineText =
              [text attributedSubstringFromRange:lastLine.range].mutableCopy;
          ...
          int i = 0;
          if (type != kCTLineTruncationStart) {  // Middle or End/Tail wants text preceding
                                                 // truncated content.
            i = (int)removedLines.count - 1;
            while (atLeastOneLine < truncatedWidth && i >= 0) {
              if (lastLineText.length > 0 &&
                  [lastLineText.string characterAtIndex:lastLineText.string.length - 1] ==
                      '\n') {  // Explicit newlines are always "long enough".
                [lastLineText
                    deleteCharactersInRange:NSMakeRange(lastLineText.string.length - 1, 1)];
                break;
              }
              [lastLineText
                  appendAttributedString:[text attributedSubstringFromRange:removedLines[i].range]];
          ...
          if (type != kCTLineTruncationEnd &&
              removedLines.count >
                  0) {  // Middle or Start/Head wants text following truncated content.
            i = 0;
            atLeastOneLine = removedLines[i].width;
            ...
            for (i--; i >= 0; i--) {
              NSAttributedString *nextLine =
                  [text attributedSubstringFromRange:removedLines[i].range];
          ...
          CTLineRef ctLastLineExtend =
              CTLineCreateWithAttributedString((CFAttributedStringRef)lastLineText);
          if (ctLastLineExtend) {
      ...
      for (NSUInteger r = 0; r < runCount; r++) {
        ...
        for (NSUInteger g = 0; g < glyphCount; g++) {
          ...
          if (isColorGlyph) {
            ...
            NSString *glyphStr =
                [layoutStr substringWithRange:NSMakeRange(runStrIdx[g], runStrLen)];
            BOOL glyphRotate =
                [glyphStr rangeOfCharacterFromSet:rotateCharset].location != NSNotFound;
            if (glyphRotate)
              glyphRotateMove =
                  [glyphStr rangeOfCharacterFromSet:rotateMoveCharset].location != NSNotFound;
          }

          ASTextRunGlyphDrawMode mode = glyphRotateMove
                                            ? ASTextRunGlyphDrawModeVerticalRotateMove
                                            : (glyphRotate ? ASTextRunGlyphDrawModeVerticalRotate
                                                           : ASTextRunGlyphDrawModeHorizontal);
          if (g == 0) {
            prevMode = mode;
          } else if (mode != prevMode) {
            ASTextRunGlyphRange *aRange =
                [ASTextRunGlyphRange rangeWithRange:NSMakeRange(prevIdx, g - prevIdx)
                                           drawMode:prevMode];
        ...
        if (prevIdx < glyphCount) {
          ASTextRunGlyphRange *aRange =
              [ASTextRunGlyphRange rangeWithRange:NSMakeRange(prevIdx, glyphCount - prevIdx)
                                         drawMode:prevMode];
          [runRanges addObject:aRange];
        }
      }
  ...
  if (visibleRange.length > 0) {
    ...
    void (^block)(NSDictionary *attrs, NSRange range, BOOL *stop) =
    ...
    [layout.text
        enumerateAttributesInRange:visibleRange
                           options:NSAttributedStringEnumerationLongestEffectiveRangeNotRequired
                        usingBlock:block];
    if (truncatedLine) {
      [truncationToken
          enumerateAttributesInRange:NSMakeRange(0, truncationToken.length)
                             options:NSAttributedStringEnumerationLongestEffectiveRangeNotRequired
                          usingBlock:block];
    }
  }
...
+ (NSArray *)layoutWithContainers:(NSArray *)containers
                             text:(NSAttributedString *)text
                            range:(NSRange)range {
...
- (NSUInteger)_rowIndexForEdge:(CGFloat)edge {
  ...
  while (lo <= hi) {
    ...
    if (isVertical ? (oneEdge.foot <= edge && edge <= oneEdge.head)
...
- (BOOL)_insideComposedCharacterSequences:(ASTextLine *)line
                                 position:(NSUInteger)position
                                    block:(void (^)(CGFloat left,
                                                    CGFloat right,
                                                    NSUInteger prev,
                                                    NSUInteger next))block {
...
- (ASTextRange *)_correctedRangeWithEdge:(ASTextRange *)range {
  ...
  if (end.offset == visibleRange.location + visibleRange.length &&
      start.affinity == ASTextAffinityForward) {
...
- (ASTextPosition *)closestPositionToPoint:(CGPoint)point {
  ...
  if (position <= _visibleRange.location) {
    return [ASTextPosition positionWithOffset:_visibleRange.location
                                     affinity:ASTextAffinityForward];
  ...
  if (binding && bindingRange.length > 0) {
    NSUInteger headLineIdx =
        [self lineIndexForPosition:[ASTextPosition positionWithOffset:bindingRange.location]];
    NSUInteger tailLineIdx = [self
        lineIndexForPosition:[ASTextPosition
                                 positionWithOffset:bindingRange.location + bindingRange.length
                                           affinity:ASTextAffinityBackward]];
    if (headLineIdx == lineIndex && lineIndex == tailLineIdx) {  // all in same line
      ...
      CGFloat right = [self offsetForTextPosition:bindingRange.location + bindingRange.length
                                        lineIndex:lineIndex];
  ...
  if (line.range.length == 0) {
    ...
    return
        [ASTextPosition positionWithOffset:line.range.location
  ...
  if (lineIndex == _lines.count - 1 &&
      (isVertical ? (point.x < line.left) : (point.y > line.bottom))) {
  ...
  [self _insideComposedCharacterSequences:line
                                 position:position
                                    block:^(CGFloat left, CGFloat right, NSUInteger prev,
                                            NSUInteger next) {
                                      if (isVertical) {
  ...
  if (!finalAffinityDetected) {
    ...
    if (ofs != CGFLOAT_MAX) {
      ...
      if (position >= line.range.location + line.range.length) {
        ...
        finalAffinity = (ofs < (isVertical ? point.y : point.x) && !RTL) ? ASTextAffinityForward
                                                                         : ASTextAffinityBackward;
      }
    }
  }
...
- (ASTextRange *)textRangeAtPoint:(CGPoint)point {
  ...
  if (lineIndex == NSNotFound) return nil;
  NSUInteger textPosition = [self textPositionForPoint:point
                                             lineIndex:[self lineIndexForPoint:point]];
  ...
  if (_container.verticalForm) {
    ASTextRange *range = [self
        textRangeByExtendingPosition:pos
...
- (ASTextRange *)closestTextRangeAtPoint:(CGPoint)point {
  ...
  if (pos.offset >= line.range.location + line.range.length) {
    ...
    if (_container.verticalForm) {
      direction =
          (rect.origin.y >= point.y && !RTL) ? UITextLayoutDirectionUp : UITextLayoutDirectionDown;
    } else {
      direction = (rect.origin.x >= point.x && !RTL) ? UITextLayoutDirectionLeft
                                                     : UITextLayoutDirectionRight;
    }
  }
...
- (ASTextRange *)textRangeByExtendingPosition:(ASTextPosition *)position
  ...
  if (isVerticalForm) {
    verticalMove =
        direction == UITextLayoutDirectionLeft || direction == UITextLayoutDirectionRight;
  ...
  ASTextRange *allForward =
      [ASTextRange rangeWithStart:fromRange.start
                              end:[ASTextPosition positionWithOffset:visibleEnd]];
  ASTextRange *allBackward =
      [ASTextRange rangeWithStart:[ASTextPosition positionWithOffset:visibleStart]
                              end:fromRange.end];
  ...
  if (verticalMove) {  // up/down in text layout
    ...
    NSUInteger pos;
    if (isVerticalForm) {
      pos = [self textPositionForPoint:CGPointMake(insideLine.position.x, ofs)
                             lineIndex:insideIndex];
    } else {
      pos = [self textPositionForPoint:CGPointMake(ofs, insideLine.position.y)
                             lineIndex:insideIndex];
    }
    ...
    ASTextPosition *toPosition =
        [ASTextPosition positionWithOffset:position.offset + (forwardMove ? offset : -offset)];
...
- (NSArray *)selectionRectsForRange:(ASTextRange *)range {
  ...
  if (startLine.row == endLine.row) {  // same row
    ...
    if (isVertical) {
      rect.rect = CGRectMake(startLine.bounds.origin.x, offsetStart,
                             MAX(startLine.width, endLine.width), offsetEnd - offsetStart);
    } else {
    ...
    if (topRun && (CTRunGetStatus(topRun) & kCTRunStatusRightToLeft)) {
      if (isVertical) {
        topRect.rect =
            CGRectMake(startLine.left, _container.path ? startLine.top : _container.insets.top,
                       startLine.width, topOffset - startLine.top);
      } else {
        topRect.rect = CGRectMake(_container.path ? startLine.left : _container.insets.left,
                                  startLine.top, topOffset - startLine.left, startLine.height);
      }
      topRect.writingDirection = UITextWritingDirectionRightToLeft;
    } else {
      if (isVertical) {
        topRect.rect =
            CGRectMake(startLine.left, topOffset, startLine.width,
                       (_container.path ? startLine.bottom
                                        : _container.size.height - _container.insets.bottom) -
                           topOffset);
      } else {
        topRect.rect = CGRectMake(
            topOffset, startLine.top,
            (_container.path ? startLine.right : _container.size.width - _container.insets.right) -
                topOffset,
            startLine.height);
      }
    }
    ...
    if (bottomRun && (CTRunGetStatus(bottomRun) & kCTRunStatusRightToLeft)) {
      if (isVertical) {
        bottomRect.rect = CGRectMake(
            ...
            (_container.path ? endLine.bottom : _container.size.height - _container.insets.bottom) -
                bottomOffset);
      } else {
        bottomRect.rect = CGRectMake(
            bottomOffset, endLine.top,
            (_container.path ? endLine.right : _container.size.width - _container.insets.right) -
                bottomOffset,
            endLine.height);
      }
      ...
      if (startLineDetected) {
        if (isVertical) {
          if (!_container.path) {
            ...
            r.size.height =
                _container.size.height - _container.insets.bottom - _container.insets.top;
          }
...
static void ASTextDrawRun(ASTextLine *line,
  ...
  CFDictionaryRef runAttrs = CTRunGetAttributes(run);
  NSValue *glyphTransformValue = (NSValue *)CFDictionaryGetValue(
      runAttrs, (__bridge const void *)(ASTextGlyphTransformAttributeName));
  if (!isVertical && !glyphTransformValue) {  // draw run
    ...
    CGColorRef fillColor =
        (CGColorRef)CFDictionaryGetValue(runAttrs, kCTForegroundColorAttributeName);
    ...
    CGContextSaveGState(context);
    {
      ...
      if (!strokeWidth || strokeWidth.floatValue == 0) {
        ...
        CGColorRef strokeColor =
            (CGColorRef)CFDictionaryGetValue(runAttrs, kCTStrokeColorAttributeName);
        if (!strokeColor) strokeColor = fillColor;
        CGContextSetStrokeColorWithColor(context, strokeColor);
        CGContextSetLineWidth(context,
                              CTFontGetSize(runFont) * fabs(strokeWidth.floatValue * 0.01));
...
static void ASTextSetLinePatternInContext(ASTextLineStyle style,
                                          CGFloat width,
                                          CGFloat phase,
                                          CGContextRef context) {
  ...
  if (pattern == ASTextLineStylePatternSolid) {
    ...
    CGFloat lengths[6] = {width * dash,  width * space, width * dot,
                          width * space, width * dot,   width * space};
...
static void ASTextDrawBorder(ASTextLayout *layout,
  ...
  NSArray *lines = layout.lines;
  NSString *borderKey = (type == ASTextBorderTypeNormal ? ASTextBorderAttributeName
                                                        : ASTextBackgroundBorderAttributeName);
    ...
    for (NSInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
      ...
      for (NSInteger ll = l; ll < lMax; ll++) {
        ...
        for (NSInteger rr = (ll == l) ? r : 0, rrMax = CFArrayGetCount(iRuns); rr < rrMax; rr++) {
          ...
          CGFloat iRunWidth =
              CTRunGetTypographicBounds(iRun, CFRangeMake(0, 0), &ascent, &descent, NULL);

          if (isVertical) {
            ASTEXT_SWAP(iRunPosition.x, iRunPosition.y);
            iRunPosition.y += iLine.position.y;
            CGRect iRect = CGRectMake(verticalOffset + line.position.x - descent, iRunPosition.y,
                                      ascent + descent, iRunWidth);
            ...
            CGRect iRect =
                CGRectMake(iRunPosition.x, iLine.position.y - ascent, iRunWidth, ascent + descent);
            if (CGRectIsNull(extLineRect)) {
              extLineRect = iRect;
            } else {
              extLineRect = CGRectUnion(extLineRect, iRect);
...
static void ASTextDrawDecoration(ASTextLayout *layout,
  ...
  for (NSUInteger l = 0, lMax = layout.lines.count; l < lMax; l++) {
    ...
    for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
      ...
      if (runRange.location + runRange.length > layout.text.length) continue;
      NSString *runStr =
          [layout.text attributedSubstringFromRange:NSMakeRange(runRange.location, runRange.length)]
              .string;
        ...
        while (shadow) {
          ...
          CGContextSaveGState(context);
          {
            ...
            CGContextSaveGState(context);
            {
              ...
              ASTextDrawLineStyle(context, length, thickness, underline.style, underlineStart,
                                  color, isVertical);
            }
            CGContextRestoreGState(context);
          }
      ...
      if (needDrawStrikethrough) {
        ...
        ASTextDrawLineStyle(context, length, thickness, strikethrough.style, strikethroughStart,
                            color, isVertical);
      }
    }
  }
  ...
  CGContextSaveGState(context);
  {
    ...
    for (NSUInteger l = 0, lMax = layout.lines.count; l < lMax; l++) {
      ...
      for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
        ...
        ASTextShadow *nsShadow =
            [ASTextShadow shadowWithNSShadow:attrs[NSShadowAttributeName]];  // NSShadow compatible
        if (nsShadow) {
      ...
      while (shadow) {
        ...
        CGContextSaveGState(context);
        {
          ...
          CGContextBeginTransparencyLayer(context, NULL);
          {
            ...
            CGContextSetShadowWithColor(context, shadow.offset, shadow.radius,
                                        opaqueShadowColor.CGColor);
            ...
            CGContextBeginTransparencyLayer(context, NULL);
            {
              ...
              CGContextBeginTransparencyLayer(context, NULL);
              {
                ASTextDrawRun(line, run, context, size, isVertical, lineRunRanges[r],
                              verticalOffset);
              }
              CGContextEndTransparencyLayer(context);
            }
...
static void ASTextDrawDebug(
      ...
      if (layout.container.pathLineWidth > 0) {
        CGContextSaveGState(context);
        {
          CGContextBeginTransparencyLayer(context, NULL);
          {
            ...
            CGPathRef cgPath =
                CGPathCreateCopyByStrokingPath(path.CGPath, NULL, layout.container.pathLineWidth,
  ...
  for (NSUInteger l = 0, lMax = lines.count; l < lMax; l++) {
    ...
    if (op.CTLineNumberColor) {
      ...
      NSMutableAttributedString *num =
          [[NSMutableAttributedString alloc] initWithString:@(l).description];
    ...
    if (op.CTRunFillColor || op.CTRunBorderColor || op.CTRunNumberColor || op.CGGlyphFillColor ||
      ...
      for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
        ...
        if (isVertical) {
          runTypoBounds =
              CGRectMake(runPosition.x - descent, runPosition.y, ascent + descent, width);
        } else {
          runTypoBounds =
              CGRectMake(runPosition.x, line.position.y - ascent, width, ascent + descent);
        }
        ...
        if (op.CTRunNumberColor) {
          ...
          NSMutableAttributedString *num =
              [[NSMutableAttributedString alloc] initWithString:@(r).description];
