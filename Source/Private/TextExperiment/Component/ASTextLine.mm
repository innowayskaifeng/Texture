  ...
  for (NSUInteger r = 0; r < runCount; r++) {
    ...
    if (attachment) {
      ...
      if (_vertical) {
        ...
        runTypoBounds = CGRectMake(_position.x + runPosition.x - descent, runPosition.y,
                                   ascent + descent, runWidth);
      } else {
        runPosition.x += _position.x;
        runPosition.y = _position.y - runPosition.y;
        runTypoBounds =
            CGRectMake(runPosition.x, runPosition.y - ascent, runWidth, ascent + descent);
      }
...
- (NSString *)description {
  ...
  [desc appendFormat:@"<ASTextLine: %p> row:%ld range:%tu,%tu", self, (long)self.row,
...
+ (instancetype)rangeWithRange:(NSRange)range
                      drawMode:(ASTextRunGlyphDrawMode)mode NS_RETURNS_RETAINED {
