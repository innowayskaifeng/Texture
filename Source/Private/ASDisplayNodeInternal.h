...
#define setFlag(flag, x) \
  (((x ? _atomicFlags.fetch_or(flag) : _atomicFlags.fetch_and(~flag)) & flag) != 0)
...
#define TIME_DISPLAYNODE_OPS \
  0  // If you're using this information frequently, try: (DEBUG || PROFILE)
...
@interface ASDisplayNode () <_ASTransitionContextCompletionDelegate> {
 @package
  ...
  struct ASDisplayNodeFlags {
    ...
    // The layer contents should not be cleared in case the node is wrapping a
    // UIImageView.UIImageView is specifically optimized for performance and does not use the usual
    // way to provide the contents of the CALayer via the CALayerDelegate method that backs the
    // UIImageView.
    unsigned canClearContentsOfLayer : 1;

    // Prevent calling setNeedsDisplay on a layer that backs a UIImageView. Usually calling
    // setNeedsDisplay on a CALayer triggers a recreation of the contents of layer unfortunately
    // calling it on a CALayer that backs a UIImageView it goes through the normal flow to assign
    // the contents to a layer via the CALayerDelegate methods. Unfortunately UIImageView does not
    // do recreate the layer contents the usual way, it actually does not implement some of the
 ...
 @protected
  ...
  // This is the desired contentsScale, not the scale at which the layer's contents should be
  // displayed
...
#if YOGA
  // Only ASDisplayNodes are supported in _yogaChildren currently. This means that it is necessary
  // to create ASDisplayNodes to make a stack layout when using Yoga. However, the implementation is
  // mostly ready for id <ASLayoutElement>, with a few areas requiring updates.
  ...
  /// Sentinel for layout data. Incremented when we get -setNeedsLayout /
  /// -invalidateCalculatedLayout. Starts at 1.
...
// Private API for helper functions / unit tests.  Use ASDisplayNodeDisableHierarchyNotifications()
// to control this.
...
/// Display the node's view/layer immediately on the current thread, bypassing the background thread
/// rendering. Will be deprecated.
- (void)displayImmediately;

/// Refreshes any precomposited or drawn clip corners, setting up state as required to transition
/// radius or rounding type.
- (void)updateCornerRoundingWithType:(ASCornerRoundingType)newRoundingType
                        cornerRadius:(CGFloat)newCornerRadius;

/// Alternative initialiser for backing with a custom view class.  Supports asynchronous display
/// with _ASDisplayView subclasses.
- (instancetype)initWithViewClass:(Class)viewClass;

/// Alternative initialiser for backing with a custom layer class.  Supports asynchronous display
/// with _ASDisplayLayer subclasses.
 ...
 */
- (void)enumerateInterfaceStateDelegates:
    (void(NS_NOESCAPE ^)(id<ASInterfaceStateDelegate> delegate))block;
 ...
 * Removing a node from the hierarchy will cause it to exit the Display state, clearing its
 * contents. For some animations, it's desirable to be able to remove a node without causing it to
 * re-display. Once re-enabled, the interface state will be updated to the same value it would have
 * been.
 *
...
/**
 * This method has proven helpful in a few rare scenarios, similar to a category extension on
 * UIView, but it's considered private API for now and its use should not be encouraged.
 * @param checkViewHierarchy If YES, and no supernode can be found, method will walk up from
 * `self.view` to find a supernode. If YES, this method must be called on the main thread and the
 * node must not be layer-backed.
 */
...
/**
 * Returns the internal style object or creates a new if no exists. Need to be called with lock
 * held.
 */
