...
#define ASFailUpdateValidation(...)                                            \
  _Pragma("clang diagnostic push")                                             \
      _Pragma("clang diagnostic ignored \"-Wdeprecated-declarations\"") if (   \
...
/**
 On return `changes` is sorted according to the change type with changes coalesced by
 animationOptions Assumes: `changes` all have the same changeType
 */
+ (void)sortAndCoalesceSectionChanges:(NSMutableArray<_ASHierarchySectionChange *> *)changes;

/// Returns all the indexes from all the `indexSet`s of the given `_ASHierarchySectionChange`
/// objects.
...
+ (void)ensureItemChanges:(NSArray<_ASHierarchyItemChange *> *)changes
               ofSameType:(_ASHierarchyChangeType)changeType;
@end
...
@property(nonatomic, readonly)
    NSMutableArray<_ASHierarchySectionChange *> *originalInsertSectionChanges;

@property(nonatomic, readonly) NSMutableArray<_ASHierarchySectionChange *> *deleteSectionChanges;
@property(nonatomic, readonly)
    NSMutableArray<_ASHierarchySectionChange *> *originalDeleteSectionChanges;
...
- (NSIndexSet *)indexesForItemChangesOfType:(_ASHierarchyChangeType)changeType
...
- (ASIntegerMap *)sectionMapping {
  ASDisplayNodeAssertNotNil(_deletedSections, @"Cannot call %s before `markCompleted` returns.",
                            sel_getName(_cmd));
  ASDisplayNodeAssertNotNil(_insertedSections, @"Cannot call %s before `markCompleted` returns.",
                            sel_getName(_cmd));
  [self _ensureCompleted];
...
- (NSMutableArray *)itemMappings {
  ...
  if (_itemMappings == nil) {
    ...
    const auto insertMap =
        [_ASHierarchyItemChange sectionToIndexSetMapFromChanges:_originalInsertItemChanges];
    const auto deleteMap =
        [_ASHierarchyItemChange sectionToIndexSetMapFromChanges:_originalDeleteItemChanges];
...
- (void)deleteItems:(NSArray *)indexPaths
...
- (void)reloadItems:(NSArray *)indexPaths
    animationOptions:(ASDataControllerAnimationOptions)options {
  [self _ensureNotCompleted];
  _ASHierarchyItemChange *change =
      [[_ASHierarchyItemChange alloc] initWithChangeType:_ASHierarchyChangeTypeReload
                                              indexPaths:indexPaths
                                        animationOptions:options
                                               presorted:NO];
...
- (void)reloadSections:(NSIndexSet *)sections
  ...
  [self _ensureNotCompleted];
  _ASHierarchySectionChange *change =
      [[_ASHierarchySectionChange alloc] initWithChangeType:_ASHierarchyChangeTypeReload
                                                   indexSet:sections
                                           animationOptions:options];
...
- (void)_sortAndCoalesceChangeArrays {
  ...
  @autoreleasepool {
    ...
    _deletedSections =
        [_ASHierarchySectionChange allIndexesInSectionChanges:_originalDeleteSectionChanges];
    _insertedSections =
        [_ASHierarchySectionChange allIndexesInSectionChanges:_originalInsertSectionChanges];
    ...
    for (_ASHierarchySectionChange *change in _reloadSectionChanges) {
      NSIndexSet *newSections = [change.indexSet as_indexesByMapping:^(NSUInteger idx) {
        ...
        ASDisplayNodeAssert(newSec != NSNotFound, @"Request to reload and delete same section %tu",
                            idx);
        return newSec;
      }];
    ...
    _insertedSections =
        [_ASHierarchySectionChange allIndexesInSectionChanges:_insertSectionChanges];
    ...
    [_ASHierarchyItemChange ensureItemChanges:_insertItemChanges
                                   ofSameType:_ASHierarchyChangeTypeInsert];
    [_ASHierarchyItemChange ensureItemChanges:_deleteItemChanges
                                   ofSameType:_ASHierarchyChangeTypeDelete];

    for (_ASHierarchyItemChange *change in _reloadItemChanges) {
      NSAssert(change.changeType == _ASHierarchyChangeTypeReload,
               @"It must be a reload change to be in here");
      ...
      // We delete the items that needs reload together with other deleted items, at their original
      // index
      ...
      // We insert the items that needs reload together with other inserted items, at their future
      // index
    ...
    [_ASHierarchyItemChange sortAndCoalesceItemChanges:_deleteItemChanges
                             ignoringChangesInSections:_deletedSections];

    // Ignore item inserts in reloaded(new)/inserted sections.
    [_ASHierarchyItemChange sortAndCoalesceItemChanges:_insertItemChanges
                             ignoringChangesInSections:_insertedSections];
  }
}
...
- (void)_validateUpdate {
  ...
  NSIndexSet *allReloadedSections =
      [_ASHierarchySectionChange allIndexesInSectionChanges:_reloadSectionChanges];
  ...
  if (newSectionCount != oldSectionCount + insertedSectionCount - deletedSectionCount) {
    ASFailUpdateValidation(
        @"Invalid number of sections. The number of sections after the update (%ld) must be equal "
  ...
  for (_ASHierarchyItemChange *change in _deleteItemChanges) {
    for (NSIndexPath *indexPath in change.indexPaths) {
      ...
      if (section >= oldSectionCount) {
        ASFailUpdateValidation(@"Attempt to delete item %ld from section %ld, but there are only "
                               @"%ld sections before the update.",
      ...
      if (item >= oldItemCount) {
        ASFailUpdateValidation(@"Attempt to delete item %ld from section %ld, which only contains "
                               @"%ld items before the update.",
  ...
  for (_ASHierarchyItemChange *change in _insertItemChanges) {
    for (NSIndexPath *indexPath in change.indexPaths) {
      ...
      if (section >= newSectionCount) {
        ASFailUpdateValidation(@"Attempt to insert item %ld into section %ld, but there are only "
                               @"%ld sections after the update.",
      ...
      if (item >= newItemCount) {
        ASFailUpdateValidation(@"Attempt to insert item %ld into section %ld, which only contains "
                               @"%ld items after the update.",
                               (long)item, (long)section, (long)newItemCount);
        return;
      }
    }
  }
  ...
  for (NSUInteger oldSection = 0; oldSection < oldSectionCount; oldSection++) {
    ...
    NSIndexSet *originalInsertedItems =
        [self indexesForItemChangesOfType:_ASHierarchyChangeTypeOriginalInsert
                                inSection:newSection];
    NSIndexSet *originalDeletedItems =
        [self indexesForItemChangesOfType:_ASHierarchyChangeTypeOriginalDelete
                                inSection:oldSection];
    NSIndexSet *reloadedItems = [self indexesForItemChangesOfType:_ASHierarchyChangeTypeReload
                                                        inSection:oldSection];

    // Assert that no reloaded items were deleted.
    NSInteger deletedReloadedItem =
        [originalDeletedItems as_intersectionWithIndexes:reloadedItems].firstIndex;
    if (deletedReloadedItem != NSNotFound) {
      ASFailUpdateValidation(@"Attempt to delete and reload the same item at index path %@",
                             [NSIndexPath indexPathForItem:deletedReloadedItem
                                                 inSection:oldSection]);
      return;
    }
    ...
    if (newItemCount != oldItemCount + insertedItemCount - deletedItemCount) {
      ASFailUpdateValidation(
          @"Invalid number of items in section %ld. The number of items after the update (%ld) "
          @"must be equal to the number of items before the update (%ld) plus or minus the number "
          @"of items inserted or deleted (%ld inserted, %ld deleted).",
...
- (NSMutableArray<NSDictionary *> *)propertiesForDescription {
  ...
  if (_reloadSectionChanges.count > 0) {
    [result addObject:@{
      @"reloadSections" :
          [_ASHierarchySectionChange smallDescriptionForSectionChanges:_reloadSectionChanges]
    }];
  }
  if (_reloadItemChanges.count > 0) {
    [result addObject:@{
      @"reloadItems" : [_ASHierarchyItemChange smallDescriptionForItemChanges:_reloadItemChanges]
    }];
  }
  if (_originalDeleteSectionChanges.count > 0) {
    [result addObject:@{
      @"deleteSections" : [_ASHierarchySectionChange
          smallDescriptionForSectionChanges:_originalDeleteSectionChanges]
    }];
  }
  if (_originalDeleteItemChanges.count > 0) {
    [result addObject:@{
      @"deleteItems" :
          [_ASHierarchyItemChange smallDescriptionForItemChanges:_originalDeleteItemChanges]
    }];
  }
  if (_originalInsertSectionChanges.count > 0) {
    [result addObject:@{
      @"insertSections" : [_ASHierarchySectionChange
          smallDescriptionForSectionChanges:_originalInsertSectionChanges]
    }];
  }
  if (_originalInsertItemChanges.count > 0) {
    [result addObject:@{
      @"insertItems" :
          [_ASHierarchyItemChange smallDescriptionForItemChanges:_originalInsertItemChanges]
    }];
  }
  return result;
}
...
- (instancetype)initWithChangeType:(_ASHierarchyChangeType)changeType
  ...
  self = [super init];
  if (self) {
    ASDisplayNodeAssert(indexSet.count > 0,
...
+ (void)sortAndCoalesceSectionChanges:(NSMutableArray<_ASHierarchySectionChange *> *)changes {
  ...
  BOOL reverse =
      type == _ASHierarchyChangeTypeDelete || type == _ASHierarchyChangeTypeOriginalDelete;
                              ...
                              usingBlock:^(NSRange range, BOOL *_Nonnull stop) {
                                ...
                                for (NSInteger i = start; i != limit; i += increment) {
                                  ...
                                  if (options != currentOptions && currentIndexes.count > 0) {
                                    _ASHierarchySectionChange *change =
                                        [[_ASHierarchySectionChange alloc]
                                            initWithChangeType:type
...
- (instancetype)initWithChangeType:(_ASHierarchyChangeType)changeType
  ...
  self = [super init];
  if (self) {
    ASDisplayNodeAssert(indexPaths.count > 0,
                        @"Request to create _ASHierarchyItemChange with no items!");
    _changeType = changeType;
    if (presorted) {
      _indexPaths = indexPaths;
    } else {
      SEL sorting = changeType == _ASHierarchyChangeTypeDelete ? @selector(asdk_inverseCompare:)
                                                               : @selector(compare:);
...
#if ASDISPLAYNODE_ASSERTIONS_ENABLED
  for (_ASHierarchyItemChange *change in changes) {
    NSAssert(change.changeType == changeType,
             @"The map we created must all be of the same changeType as of now");
  }
#endif
}
...
+ (void)sortAndCoalesceItemChanges:(NSMutableArray<_ASHierarchyItemChange *> *)changes
  ...
  SEL sorting =
      type == _ASHierarchyChangeTypeDelete ? @selector(asdk_inverseCompare:) : @selector(compare:);
  ...
  for (NSIndexPath *indexPath in allIndexPaths) {
    ...
    if (options != currentOptions && currentIndexPaths.count > 0) {
      _ASHierarchyItemChange *change =
          [[_ASHierarchyItemChange alloc] initWithChangeType:type
...
+ (NSString *)smallDescriptionForItemChanges:(NSArray<_ASHierarchyItemChange *> *)changes {
  ...
  [map enumerateKeysAndObjectsUsingBlock:^(NSNumber *_Nonnull section,
                                           NSIndexSet *_Nonnull indexSet, BOOL *_Nonnull stop) {
