...
- (CGPoint)targetContentOffsetForProposedContentOffset:(CGPoint)proposedContentOffset {
  // Don't mess around if the user is interacting with the page node. Although if just a rotation
  // happened we should try to use the current index path to not end up setting the target content
  // offset to something in between pages
  if (!self.collectionView.decelerating && !self.collectionView.tracking) {
    ...
    if (indexPath) {
      return [self _targetContentOffsetForItemAtIndexPath:indexPath
...
- (CGPoint)_targetContentOffsetForItemAtIndexPath:(NSIndexPath *)indexPath
  ...
  CGFloat xOffset =
      (CGRectGetWidth(self.collectionView.bounds) - CGRectGetWidth(attributes.frame)) / 2.0;
...
- (UICollectionViewLayoutInvalidationContext *)invalidationContextForBoundsChange:
  ...
  UICollectionViewFlowLayoutInvalidationContext *ctx =
      (UICollectionViewFlowLayoutInvalidationContext *)[super
          invalidationContextForBoundsChange:newBounds];
