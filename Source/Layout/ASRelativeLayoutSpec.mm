...
- (ASLayout *)calculateLayoutThatFits:(ASSizeRange)constrainedSize {
  // If we have a finite size in any direction, pass this so that the child can resolve percentages
  // against it. Otherwise pass ASLayoutElementParentDimensionUndefined as the size will depend on
  // the content
  CGSize size = {ASPointsValidForSize(constrainedSize.max.width) == NO
                     ? ASLayoutElementParentDimensionUndefined
                     : constrainedSize.max.width,
                 ASPointsValidForSize(constrainedSize.max.height) == NO
                     ? ASLayoutElementParentDimensionUndefined
  ...
  ASLayout *sublayout =
      [self.child layoutThatFits:ASSizeRangeMake(minChildSize, constrainedSize.max)
                      parentSize:size];
  ...
  size = ASSizeRangeClamp(
      constrainedSize,
      {MIN(size.width, (_sizingOption & ASRelativeLayoutSpecSizingOptionMinimumWidth) != 0
                           ? sublayout.size.width
                           : size.width),
       MIN(size.height, (_sizingOption & ASRelativeLayoutSpecSizingOptionMinimumHeight) != 0
