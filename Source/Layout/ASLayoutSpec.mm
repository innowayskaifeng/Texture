...
- (void)setChild:(id<ASLayoutElement>)child {
  ...
  ASDisplayNodeAssert(_childrenArray.count < 2,
                      @"This layout spec does not support more than one child. Use the "
                      @"setChildren: or the setChild:AtIndex: API");
...
#pragma mark - ASDescriptionProvider
  ...
  if (NSArray *children = self.children) {
    ...
    const auto tinyDescriptions =
        ASArrayByFlatMapping(children, id object, ASObjectDescriptionMakeTiny(object));
...
- (nullable NSHashTable<id<ASLayoutElement>> *)findDuplicatedElementsInSubtree {
  ...
  NSUInteger count = 0;
  [self _findDuplicatedElementsInSubtreeWithWorkingSet:
 ...
 * @param result The set into which to put the result. This initially points to @c nil to save time
 * if no duplicates exist.
 */
- (void)
    _findDuplicatedElementsInSubtreeWithWorkingSet:(NSHashTable<id<ASLayoutElement>> *)workingSet
                                      workingCount:(NSUInteger *)workingCount
                                            result:(NSHashTable<id<ASLayoutElement>> *_Nullable *)
                                                       result {
...
+ (NSString *)asciiArtStringForChildren:(NSArray *)children parentName:(NSString *)parentName {
  return [self asciiArtStringForChildren:children
