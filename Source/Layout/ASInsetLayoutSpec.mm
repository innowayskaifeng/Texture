...
- (ASLayout *)calculateLayoutThatFits:(ASSizeRange)constrainedSize
  ...
  const CGFloat minWidth =
      (isinf(_insets.left) || isinf(_insets.right)) ? 0 : constrainedSize.min.width;
  // if either y-axis inset is infinite, let child be intrinsic height
  const CGFloat minHeight =
      (isinf(_insets.top) || isinf(_insets.bottom)) ? 0 : constrainedSize.min.height;
  ...
  const CGSize insetParentSize = {MAX(0, parentSize.width - insetsX),
  ...
  const CGFloat x = finite(
      ...
      constrainedSize.max.width -
          (finite(_insets.right, centerInset(constrainedSize.max.width, sublayout.size.width)) +
           sublayout.size.width));

  const CGFloat y = finite(
      _insets.top,
