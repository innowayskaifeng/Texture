 ...
 */
ASDISPLAYNODE_INLINE AS_WARN_UNUSED_RESULT ASLayoutElementSize
ASLayoutElementSizeMakeFromCGSize(CGSize size) {
...
ASLayoutElementSizeEqualToLayoutElementSize(ASLayoutElementSize lhs, ASLayoutElementSize rhs) {
 ...
 * From the given size uses width, height to resolve the exact size constraint, uses the minHeight
 * and minWidth to resolve the min size constraint and the maxHeight and maxWidth to resolve the max
 * size constraint. For every dimension with unit ASDimensionUnitAuto the given autoASSizeRange
 * value will be used. Based on the calculated exact, min and max size constraints the final size
 * range will be calculated.
 */
AS_EXTERN AS_WARN_UNUSED_RESULT ASSizeRange ASLayoutElementSizeResolveAutoSize(
    ASLayoutElementSize size, const CGSize parentSize, ASSizeRange autoASSizeRange);

/**
 * Resolve the given size to a parent size. Uses internally ASLayoutElementSizeResolveAutoSize with
 * {INFINITY, INFINITY} as as autoASSizeRange. For more information look at
 * ASLayoutElementSizeResolveAutoSize.
 */
ASDISPLAYNODE_INLINE AS_WARN_UNUSED_RESULT ASSizeRange
ASLayoutElementSizeResolve(ASLayoutElementSize size, const CGSize parentSize) {
