...
#define ASLayoutElementLayoutCalculationDefaults                                               \
  ...
  -(ASLayout *)calculateLayoutThatFits : (ASSizeRange)constrainedSize restrictedToSize         \
    ...
    const ASSizeRange resolvedRange = ASSizeRangeIntersect(                                    \
        constrainedSize, ASLayoutElementSizeResolve(self.style.size, parentSize));             \
    return [self calculateLayoutThatFits:resolvedRange];                                       \
  }

#pragma mark - ASLayoutElementExtensibility

// Provides extension points for elments that comply to ASLayoutElement like ASLayoutSpec to add
// additional properties besides the default one provided in ASLayoutElementStyle
