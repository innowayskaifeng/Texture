...
ASDISPLAYNODE_INLINE BOOL ASIsCGPositionValidForLayout(CGPoint point) {
  return (ASIsCGPositionPointsValidForLayout(point.x) &&
          ASIsCGPositionPointsValidForLayout(point.y));
}
 ...
 *
 * "Auto" - This indicated "I have no opinion" and may be resolved in whatever way makes most sense
...
typedef NS_ENUM(NSInteger, ASDimensionUnit) {
  /** This indicates "I have no opinion" and may be resolved in whatever way makes most sense given
     the circumstances. */
  ASDimensionUnitAuto,
 ...
 */
ASOVERLOADABLE ASDISPLAYNODE_INLINE ASDimension ASDimensionMake(ASDimensionUnit unit,
                                                                CGFloat value) {
  if (unit == ASDimensionUnitAuto) {
    ...
    ASDisplayNodeCAssert(0 <= value && value <= 1.0,
                         @"ASDimension fraction value (%f) must be between 0 and 1.", value);
  }
 ...
 */
ASOVERLOADABLE ASDISPLAYNODE_INLINE AS_WARN_UNUSED_RESULT ASDimension
ASDimensionMake(CGFloat points) {
 ...
 */
ASDISPLAYNODE_INLINE AS_WARN_UNUSED_RESULT ASDimension
ASDimensionMakeWithFraction(CGFloat fraction) {
  ASDisplayNodeCAssert(0 <= fraction && fraction <= 1.0,
                       @"ASDimension fraction value (%f) must be between 0 and 1.", fraction);
 ...
 */
ASDISPLAYNODE_INLINE AS_WARN_UNUSED_RESULT BOOL ASDimensionEqualToDimension(ASDimension lhs,
                                                                            ASDimension rhs) {
 ...
 */
ASDISPLAYNODE_INLINE AS_WARN_UNUSED_RESULT ASLayoutSize ASLayoutSizeMake(ASDimension width,
                                                                         ASDimension height) {
  ASLayoutSize size;
  size.width = width;
  size.height = height;
 ...
 */
ASDISPLAYNODE_INLINE CGSize ASLayoutSizeResolveSize(ASLayoutSize layoutSize,
                                                    CGSize parentSize,
                                                    CGSize autoSize) {
...
ASDISPLAYNODE_INLINE AS_WARN_UNUSED_RESULT NSString *NSStringFromASLayoutSize(ASLayoutSize size) {
  return [NSString stringWithFormat:@"{%@, %@}", NSStringFromASDimension(size.width),
 ...
 */
ASDISPLAYNODE_INLINE AS_WARN_UNUSED_RESULT BOOL
ASSizeRangeHasSignificantArea(ASSizeRange sizeRange) {
 ...
 */
ASOVERLOADABLE ASDISPLAYNODE_INLINE AS_WARN_UNUSED_RESULT ASSizeRange ASSizeRangeMake(CGSize min,
                                                                                      CGSize max) {
  ...
  ASDisplayNodeCAssert(min.width <= max.width,
                       @"Range min width (%f) must not be larger than max width (%f).", min.width,
                       max.width);
 ...
 */
ASOVERLOADABLE ASDISPLAYNODE_INLINE AS_WARN_UNUSED_RESULT ASSizeRange
ASSizeRangeMake(CGSize exactSize) {
 ...
 */
ASDISPLAYNODE_INLINE AS_WARN_UNUSED_RESULT CGSize ASSizeRangeClamp(ASSizeRange sizeRange,
                                                                   CGSize size) {
...
/**
 * Intersects another size range. If the other size range does not overlap in either dimension, this
 * size range "wins" by returning a single point within its own range that is closest to the
 * non-overlapping range.
 */
AS_EXTERN AS_WARN_UNUSED_RESULT ASSizeRange ASSizeRangeIntersect(ASSizeRange sizeRange,
                                                                 ASSizeRange otherSizeRange);

/**
 * Returns whether two size ranges are equal in min and max size.
 */
ASDISPLAYNODE_INLINE AS_WARN_UNUSED_RESULT BOOL ASSizeRangeEqualToSizeRange(ASSizeRange lhs,
                                                                            ASSizeRange rhs) {
