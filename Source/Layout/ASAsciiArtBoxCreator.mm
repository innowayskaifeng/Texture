...
+ (instancetype)debugbox_stringWithString:(NSString *)stringToRepeat
  ...
  NSMutableString *string =
      [[NSMutableString alloc] initWithCapacity:[stringToRepeat length] * repeatCount];
...
+ (NSString *)horizontalBoxStringForChildren:(NSArray *)children parent:(NSString *)parent {
  ...
  for (NSString *child in children) {
    ...
    for (NSUInteger index = 0; index < topPadding; index++) {
      [lines insertObject:[NSString debugbox_stringWithString:@" " repeatedCount:lineLength]
                  atIndex:0];
    }
  ...
  if (totalLineLength < [parent length]) {
    ...
    for (NSString *line in concatenatedLines) {
      ...
      paddedLine =
          [paddedLine stringByReplacingOccurrencesOfString:@"|"
...
+ (NSString *)verticalBoxStringForChildren:(NSArray *)children parent:(NSString *)parent {
  ...
  NSString *rightPaddingString =
      [NSString debugbox_stringWithString:@" " repeatedCount:rightPadding + kDebugBoxPadding];
  NSString *leftPaddingString = [NSString debugbox_stringWithString:@" "
                                                      repeatedCount:leftPadding + kDebugBoxPadding];
  ...
  for (NSString *child in children) {
    ...
    for (NSString *line in lines) {
      NSString *rightLinePaddingString = [NSString debugbox_stringWithString:@" "
                                                               repeatedCount:rightLinePadding];
      rightLinePaddingString =
          [NSString stringWithFormat:@"%@%@|", rightLinePaddingString, rightPaddingString];

      NSString *leftLinePaddingString = [NSString debugbox_stringWithString:@" "
                                                              repeatedCount:leftLinePadding];
      leftLinePaddingString =
          [NSString stringWithFormat:@"|%@%@", leftLinePaddingString, leftPaddingString];

      NSString *paddingLine = [NSString
          stringWithFormat:@"%@%@%@", leftLinePaddingString, line, rightLinePaddingString];
...
+ (NSMutableArray *)appendTopAndBottomToBoxString:(NSMutableArray *)boxStrings
  ...
  NSString *topLine = [parent debugbox_stringByAddingPadding:@"-"
                                                       count:leftPadding
                                                    location:PIDebugBoxPaddingLocationFront];
  topLine = [topLine debugbox_stringByAddingPadding:@"-"
                                              count:rightPadding
                                           location:PIDebugBoxPaddingLocationEnd];
