...
- (instancetype)initWithLayoutElement:(id<ASLayoutElement>)layoutElement
  ...
  if (self) {
    ...
    for (ASLayout *sublayout in sublayouts) {
      ASDisplayNodeAssert(ASPointIsNull(sublayout.position) == NO,
                          @"Invalid position is not allowed in sublayout.");
    }
#endif
    ...
    if (!ASIsCGSizeValidForSize(size)) {
      ASDisplayNodeFailAssert(@"layoutSize is invalid and unsafe to provide to Core Animation! "
                              @"Release configurations will force to 0, 0.  Size = %@, node = %@",
...
+ (instancetype)layoutWithLayoutElement:(id<ASLayoutElement>)layoutElement
                                   size:(CGSize)size NS_RETURNS_RETAINED {
...
- (BOOL)isFlattened {
  // A layout is flattened if its position is null, and all of its sublayouts are of type
  // displaynode with no sublayouts.
...
- (ASLayout *)filteredNodeLayoutTree NS_RETURNS_RETAINED {
  ...
  while (!queue.empty()) {
    ...
    if (ASLayoutIsDisplayNodeType(layout)) {
      if (sublayoutsCount > 0 ||
          CGPointEqualToPoint(ASCeilPointValues(absolutePosition), layout.position) == NO) {
        // Only create a new layout if the existing one can't be reused, which means it has either
        // some sublayouts or an invalid absolute position.
      ...
      // Fast-reverse-enumerate the sublayouts array by copying it into a C-array and push_front'ing
      // each into the queue.
  ...
  NSArray *array = [NSArray arrayByTransferring:flattenedSublayouts.data()
                                          count:flattenedSublayouts.size()];
...
ASLayout *ASCalculateLayout(id<ASLayoutElement> layoutElement,
...
ASLayout *ASCalculateRootLayout(id<ASLayoutElement> rootLayoutElement,
                                const ASSizeRange sizeRange) {
