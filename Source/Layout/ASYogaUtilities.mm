...
void ASDisplayNodePerformBlockOnEveryYogaChild(ASDisplayNode *node,
...
YGSize ASLayoutElementYogaMeasureFunc(YGNodeRef yogaNode,
  ...
  if (widthMode == YGMeasureModeExactly) {
    ...
    sizeRange.min.width =
        (minWidth.unit == ASDimensionUnitPoints ? yogaDimensionToPoints(minWidth) : 0.0);
  }
  if (heightMode == YGMeasureModeExactly) {
    ...
    sizeRange.min.height =
        (minHeight.unit == ASDimensionUnitPoints ? yogaDimensionToPoints(minHeight) : 0.0);
  }
