...
- (ASLayout *)calculateLayoutThatFits:(ASSizeRange)constrainedSize {
  ...
  // Accessing the style and size property is pretty costly we create layout spec children we use to
  // figure out the layout for each child
  const auto stackChildren =
      AS::map(children, [&](const id<ASLayoutElement> child) -> ASStackLayoutSpecChild {
  ...
  const auto positionedLayout =
      ASStackPositionedLayout::compute(unpositionedLayout, style, constrainedSize);
...
- (NSString *)asciiArtString {
  return [ASLayoutSpec asciiArtStringForChildren:self.children
                                      parentName:[self asciiArtName]
