...
- (void)_initializeInstance {
  ...
  _selfConformsToRangeModeProtocol =
      [self conformsToProtocol:@protocol(ASRangeControllerUpdateRangeProtocol)];
  _nodeConformsToRangeModeProtocol =
      [_node conformsToProtocol:@protocol(ASRangeControllerUpdateRangeProtocol)];
  _automaticallyAdjustRangeModeBasedOnViewEvents =
      _selfConformsToRangeModeProtocol || _nodeConformsToRangeModeProtocol;
  ...
  if (_node.nodeLoaded) {
    ...
    // If the node didn't load yet add ourselves as on did load observer to load the view in case
    // the node gets loaded before the view controller
...
- (void)loadView {
  ...
  // ensure that self.node has a valid trait collection before a subclass's implementation of
  // viewDidLoad. Any subnodes added in viewDidLoad will then inherit the proper environment.
...
- (void)viewWillLayoutSubviews {
  ...
  // Before layout, make sure that our trait collection containerSize actually matches the size of
  // our bounds. If not, we need to update the traits and propagate them.
  ...
  if (CGSizeEqualToSize(self.node.primitiveTraitCollection.containerSize, boundsSize) == NO) {
    ...
    // Call layoutThatFits: to let the node prepare for a layout that will happen shortly in the
    // layout pass of the view. If the node's constrained size didn't change between the last layout
    // pass it's a no-op
...
- (void)viewWillAppear:(BOOL)animated {
  ...
  // into the hierarchy before UIKit applies the scroll view inset adjustments, if automatic subnode
  // management is enabled. Otherwise the insets would not be applied.
...
- (void)visibilityDepthDidChange {
  ...
  as_log_verbose(ASNodeLog(), "Updating visibility of %@ to: %@ (visibility depth: %zd)", self,
                 rangeModeString, self.visibilityDepth);
#endif
...
- (void)setAutomaticallyAdjustRangeModeBasedOnViewEvents:
    (BOOL)automaticallyAdjustRangeModeBasedOnViewEvents {
  if (automaticallyAdjustRangeModeBasedOnViewEvents !=
      _automaticallyAdjustRangeModeBasedOnViewEvents) {
    if (automaticallyAdjustRangeModeBasedOnViewEvents && _selfConformsToRangeModeProtocol == NO &&
        _nodeConformsToRangeModeProtocol == NO) {
      NSLog(@"Warning: automaticallyAdjustRangeModeBasedOnViewEvents set to YES in %@, but range "
            @"mode updating is not possible because neither view controller nor node %@ conform to "
...
- (void)updateCurrentRangeModeWithModeIfPossible:(ASLayoutRangeMode)rangeMode {
  ...
  if (_selfConformsToRangeModeProtocol) {
    id<ASRangeControllerUpdateRangeProtocol> rangeUpdater =
        (id<ASRangeControllerUpdateRangeProtocol>)self;
    [rangeUpdater updateCurrentRangeWithMode:rangeMode];
  }

  if (_nodeConformsToRangeModeProtocol) {
    id<ASRangeControllerUpdateRangeProtocol> rangeUpdater =
        (id<ASRangeControllerUpdateRangeProtocol>)_node;
    [rangeUpdater updateCurrentRangeWithMode:rangeMode];
  }
}
...
- (ASPrimitiveTraitCollection)primitiveTraitCollectionForUITraitCollection:
    (UITraitCollection *)traitCollection {
  if (self.overrideDisplayTraitsWithTraitCollection) {
    ASTraitCollection *asyncTraitCollection =
        self.overrideDisplayTraitsWithTraitCollection(traitCollection);
    return [asyncTraitCollection primitiveTraitCollection];
  }

  ASDisplayNodeAssertMainThread();
  ASPrimitiveTraitCollection asyncTraitCollection =
...
- (void)propagateNewTraitCollection:(ASPrimitiveTraitCollection)traitCollection {
  ...
  if (ASPrimitiveTraitCollectionIsEqualToASPrimitiveTraitCollection(traitCollection,
                                                                    oldTraitCollection) == NO) {
    as_activity_scope_verbose(as_activity_create("Propagate ASViewController trait collection",
    ...
    as_activity_scope_verbose(as_activity_create("Layout ASViewController node with new traits",
                                                 AS_ACTIVITY_CURRENT, OS_ACTIVITY_FLAG_DEFAULT));
    [_node layoutThatFits:[self nodeConstrainedSize]];
  }
}
