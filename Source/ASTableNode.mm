...
- (void)didEnterPreloadState {
  ...
  // Intentionally allocate the view here and trigger a layout pass on it, which in turn will
  // trigger the intial data load. We can get rid of this call later when ASDataController,
  // ASRangeController and ASCollectionLayout can operate without the view.
...
- (void)setInverted:(BOOL)inverted {
  ...
  if ([self pendingState]) {
    ...
    ASDisplayNodeAssert([self isNodeLoaded],
                        @"ASTableNode should be loaded if pendingState doesn't exist");
...
- (void)setLeadingScreensForBatching:(CGFloat)leadingScreensForBatching {
  ...
  if (pendingState) {
    ...
    ASDisplayNodeAssert(self.nodeLoaded,
                        @"ASTableNode should be loaded if pendingState doesn't exist");
...
- (void)selectRowAtIndexPath:(nullable NSIndexPath *)indexPath
  ...
  if (indexPath != nil) {
    ...
    NSLog(@"Failed to select row at index path %@ because the row never reached the view.",
          indexPath);
  }
}
...
- (void)deselectRowAtIndexPath:(NSIndexPath *)indexPath animated:(BOOL)animated {
  ...
  if (indexPath != nil) {
    ...
    NSLog(@"Failed to deselect row at index path %@ because the row never reached the view.",
          indexPath);
  }
}
...
- (void)scrollToRowAtIndexPath:(NSIndexPath *)indexPath
  ...
  if (indexPath != nil) {
    ...
    NSLog(@"Failed to scroll to row at index path %@ because the row never reached the view.",
          indexPath);
  }
}
