...
- (void)setElement:(ASCollectionElement *)element {
  ...
  if (node) {
    ...
    // the following ensures that we clip the entire cell to it's bounds if node.clipsToBounds is
    // set (the default) This is actually a workaround for a bug we are seeing in some rare cases
    // (selected background view overlaps other cells if size of ASCellNode has changed.)
...
- (BOOL)consumesCellNodeVisibilityEvents {
  ...
  return ASSubclassOverridesSelector([ASCellNode class], [node class],
                                     @selector(cellNodeVisibilityEvent:
                                                          inScrollView:withCellFrame:));
}
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder {
  NSLog(@"Warning: AsyncDisplayKit is not designed to be used with Interface Builder.  Table "
        @"properties set in IB will be lost.");
...
- (void)dealloc {
  ...
  ASDisplayNodeCAssert(_batchUpdateCount == 0,
                       @"ASTableView deallocated in the middle of a batch update.");
...
- (void)setDataSource:(id<UITableViewDataSource>)dataSource {
  ...
  // assert for non-nil.
  ASDisplayNodeAssert(dataSource == nil,
...
- (void)setDelegate:(id<UITableViewDelegate>)delegate {
  ...
  // non-nil value here.
  ASDisplayNodeAssert(delegate == nil,
...
- (void)setAsyncDataSource:(id<ASTableDataSource>)asyncDataSource {
  ...
  // dataSource in this case because calls to ASTableViewProxy will start failing and cause crashes.
  NS_VALID_UNTIL_END_OF_SCOPE id oldDataSource = self.dataSource;

  if (asyncDataSource == nil) {
    _asyncDataSource = nil;
    _proxyDataSource =
        _isDeallocating ? nil : [[ASTableViewProxy alloc] initWithTarget:nil interceptor:self];
    ...
    _asyncDataSourceFlags.tableViewNodeForRow =
        [_asyncDataSource respondsToSelector:@selector(tableView:nodeForRowAtIndexPath:)];
    _asyncDataSourceFlags.tableNodeNodeForRow =
        [_asyncDataSource respondsToSelector:@selector(tableNode:nodeForRowAtIndexPath:)];
    ...
    _asyncDataSourceFlags.tableViewCanMoveRow =
        [_asyncDataSource respondsToSelector:@selector(tableView:canMoveRowAtIndexPath:)];
    _asyncDataSourceFlags.tableViewMoveRow =
        [_asyncDataSource respondsToSelector:@selector(tableView:moveRowAtIndexPath:toIndexPath:)];
    _asyncDataSourceFlags.sectionIndexMethods =
        [_asyncDataSource respondsToSelector:@selector(sectionIndexTitlesForTableView:)] &&
        [_asyncDataSource respondsToSelector:@selector(tableView:
    ...
    ASDisplayNodeAssert(_asyncDataSourceFlags.tableViewNodeBlockForRow ||
                            ...
                            _asyncDataSourceFlags.tableNodeNodeBlockForRow ||
                            _asyncDataSourceFlags.tableNodeNodeForRow,
                        @"Data source must implement tableNode:nodeBlockForRowAtIndexPath: or "
                        @"tableNode:nodeForRowAtIndexPath:");
    ASDisplayNodeAssert(_asyncDataSourceFlags.tableNodeNumberOfRowsInSection ||
                            _asyncDataSourceFlags.tableViewNumberOfRowsInSection,
...
- (void)setAsyncDelegate:(id<ASTableDelegate>)asyncDelegate {
  ...
  // _proxyDataSource still exists. It's really important to hold a strong reference to the old
  // delegate in this case because calls to ASTableViewProxy will start failing and cause crashes.
  ...
  if (asyncDelegate == nil) {
    ...
    if (_asyncDelegateFlags.tableViewWillDisplayNodeForRow == NO) {
      _asyncDelegateFlags.tableViewWillDisplayNodeForRowDeprecated = [_asyncDelegate
          respondsToSelector:@selector(tableView:willDisplayNodeForRowAtIndexPath:)];
    }
    _asyncDelegateFlags.tableViewDidEndDisplayingNodeForRow = [_asyncDelegate
    ...
    _asyncDelegateFlags.tableViewWillSelectRow =
        [_asyncDelegate respondsToSelector:@selector(tableView:willSelectRowAtIndexPath:)];
    _asyncDelegateFlags.tableNodeWillSelectRow =
        [_asyncDelegate respondsToSelector:@selector(tableNode:willSelectRowAtIndexPath:)];
    _asyncDelegateFlags.tableViewDidSelectRow =
        [_asyncDelegate respondsToSelector:@selector(tableView:didSelectRowAtIndexPath:)];
    _asyncDelegateFlags.tableNodeDidSelectRow =
        [_asyncDelegate respondsToSelector:@selector(tableNode:didSelectRowAtIndexPath:)];
    _asyncDelegateFlags.tableViewWillDeselectRow =
        [_asyncDelegate respondsToSelector:@selector(tableView:willDeselectRowAtIndexPath:)];
    _asyncDelegateFlags.tableNodeWillDeselectRow =
        [_asyncDelegate respondsToSelector:@selector(tableNode:willDeselectRowAtIndexPath:)];
    _asyncDelegateFlags.tableViewDidDeselectRow =
        [_asyncDelegate respondsToSelector:@selector(tableView:didDeselectRowAtIndexPath:)];
    _asyncDelegateFlags.tableNodeDidDeselectRow =
        [_asyncDelegate respondsToSelector:@selector(tableNode:didDeselectRowAtIndexPath:)];
    ...
    _asyncDelegateFlags.tableViewDidHighlightRow =
        [_asyncDelegate respondsToSelector:@selector(tableView:didHighlightRowAtIndexPath:)];
    _asyncDelegateFlags.tableNodeDidHighlightRow =
        [_asyncDelegate respondsToSelector:@selector(tableNode:didHighlightRowAtIndexPath:)];
    ...
    _asyncDelegateFlags.tableViewCanPerformActionForRow = [_asyncDelegate
        respondsToSelector:@selector(tableView:canPerformAction:forRowAtIndexPath:withSender:)];
    _asyncDelegateFlags.tableNodeCanPerformActionForRow = [_asyncDelegate
        respondsToSelector:@selector(tableNode:canPerformAction:forRowAtIndexPath:withSender:)];
    _asyncDelegateFlags.tableViewPerformActionForRow = [_asyncDelegate
        respondsToSelector:@selector(tableView:performAction:forRowAtIndexPath:withSender:)];
    _asyncDelegateFlags.tableNodePerformActionForRow = [_asyncDelegate
        respondsToSelector:@selector(tableNode:performAction:forRowAtIndexPath:withSender:)];
  }
...
- (void)reloadDataWithCompletion:(void (^)())completion {
  ...
  if (!_dataController.initialReloadDataHasBeenCalled) {
    // If this is the first reload, forward to super immediately to prevent it from triggering more
    // "initial" loads while our data controller is working.
    [super reloadData];
  }
...
- (nullable NSIndexPath *)validateIndexPath:(nullable NSIndexPath *)indexPath {
  ...
  if (item != NSNotFound && item >= [self numberOfRowsInSection:section]) {
    ASDisplayNodeFailAssert(
        @"Table view index path has invalid item %lu in section %lu, item count = %lu",
...
- (nullable NSIndexPath *)indexPathForNode:(ASCellNode *)cellNode waitingIfNeeded:(BOOL)wait {
  ...
  NSIndexPath *indexPath =
      [_dataController.visibleMap indexPathForElement:cellNode.collectionElement];
...
- (void)layoutSubviews {
  ...
  CGFloat constrainedWidth =
      self.bounds.size.width - [self sectionIndexWidth] - contentInset.left - contentInset.right;
  ...
  // To ensure _nodesConstrainedWidth is up-to-date for every usage, this call to super must be done
  // last
  [super layoutSubviews];
...
- (void)endAdjustingContentOffsetAnimated:(BOOL)animated {
  ...
  NSIndexPath *newIndexPathForTopVisibleRow =
      [self indexPathForNode:_contentOffsetAdjustmentTopVisibleNode];
...
- (UITableViewCell *)tableView:(UITableView *)tableView
  ...
  _ASTableViewCell *cell = [self dequeueReusableCellWithIdentifier:kCellReuseIdentifier
...
- (void)tableView:(UITableView *)tableView
  ...
  if (_asyncDataSourceFlags.tableViewMoveRow) {
    [_asyncDataSource tableView:self
...
- (void)tableView:(UITableView *)tableView
  ...
  if (element) {
    ...
    ASDisplayNodeAssert(NO, @"Unexpected nil element for willDisplayCell: %@, %@, %@", cell, self,
                        indexPath);
    return;
  }
...
- (void)tableView:(UITableView *)tableView
  ...
  if (element) {
    ...
    ASDisplayNodeAssert(NO, @"Unexpected nil element for didEndDisplayingCell: %@, %@, %@", cell,
                        self, indexPath);
    return;
  }
...
- (NSIndexPath *)tableView:(UITableView *)tableView
    willDeselectRowAtIndexPath:(nonnull NSIndexPath *)indexPath {
...
- (BOOL)tableView:(UITableView *)tableView
    shouldHighlightRowAtIndexPath:(nonnull NSIndexPath *)indexPath {
...
- (void)tableView:(UITableView *)tableView
    didHighlightRowAtIndexPath:(nonnull NSIndexPath *)indexPath {
...
- (void)tableView:(UITableView *)tableView
    didUnhighlightRowAtIndexPath:(nonnull NSIndexPath *)indexPath {
  if (_asyncDelegateFlags.tableNodeDidHighlightRow) {
    GET_TABLENODE_OR_RETURN(tableNode, (void)0);
    indexPath = [self convertIndexPathToTableNode:indexPath];
    if (indexPath != nil) {
...
- (BOOL)tableView:(UITableView *)tableView
    shouldShowMenuForRowAtIndexPath:(nonnull NSIndexPath *)indexPath {
...
- (BOOL)tableView:(UITableView *)tableView
  ...
  if (_asyncDelegateFlags.tableNodeCanPerformActionForRow) {
    ...
    indexPath = [self convertIndexPathToTableNode:indexPath];
    if (indexPath != nil) {
      return [_asyncDelegate tableNode:tableNode
...
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
    return [_asyncDelegate tableView:self
                    canPerformAction:action
                   forRowAtIndexPath:indexPath
                          withSender:sender];
#pragma clang diagnostic pop
...
- (void)tableView:(UITableView *)tableView
  ...
  if (_asyncDelegateFlags.tableNodePerformActionForRow) {
    ...
    indexPath = [self convertIndexPathToTableNode:indexPath];
    if (indexPath != nil) {
      [_asyncDelegate tableNode:tableNode
...
- (ASScrollDirection)scrollableDirections {
  ...
  CGFloat totalContentWidth =
      self.contentSize.width + self.contentInset.left + self.contentInset.right;
  ...
  if (self.alwaysBounceHorizontal ||
      totalContentWidth > self.bounds.size.width) {  // Can scroll horizontally.
    scrollableDirection |= ASScrollDirectionHorizontalDirections;
  }
  if (self.alwaysBounceVertical ||
      totalContentHeight > self.bounds.size.height) {  // Can scroll vertically.
...
- (void)_beginBatchFetchingIfNeededWithContentOffset:(CGPoint)contentOffset
  ...
  if (ASDisplayShouldFetchBatchForScrollView(self, self.scrollDirection,
                                             ASScrollDirectionVerticalDirections, contentOffset,
                                             velocity)) {
...
- (void)rangeController:(ASRangeController *)rangeController
  ...
  BOOL shouldAdjustContentOffset =
      (_automaticallyAdjustsContentOffset && !changeSet.includesReloadData);
...
- (ASCellNodeBlock)dataController:(ASDataController *)dataController
  ...
  if (_asyncDataSourceFlags.tableNodeNodeBlockForRow) {
    ...
    if ([node isKindOfClass:[ASCellNode class]]) {
      ...
      ASDisplayNodeFailAssert(
          @"Data source returned invalid node from tableNode:nodeForRowAtIndexPath:. Node: %@",
          node);
    }
...
#pragma clang diagnostic pop
    if ([node isKindOfClass:[ASCellNode class]]) {
      ...
      ASDisplayNodeFailAssert(
          @"Data source returned invalid node from tableView:nodeForRowAtIndexPath:. Node: %@",
          node);
    }
  }
  ...
  if (block == nil) {
    ASDisplayNodeFailAssert(@"ASTableNode could not get a node block for row at index path %@",
                            indexPath);
    block = ^{
  ...
  return ^{
    ...
    ASDisplayNodeAssert([node isKindOfClass:[ASCellNode class]],
                        @"ASTableNode provided a non-ASCellNode! %@, %@", node, strongSelf);
...
- (ASSizeRange)dataController:(ASDataController *)dataController
  ...
  if (_asyncDelegateFlags.tableNodeConstrainedSizeForRow) {
    ...
    constrainedSize =
        ASSizeRangeMake(CGSizeMake(_nodesConstrainedWidth, delegateConstrainedSize.min.height),
...
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
    ASSizeRange delegateConstrainedSize = [_asyncDelegate tableView:self
                                   constrainedSizeForRowAtIndexPath:indexPath];
#pragma clang diagnostic pop
    // ignore widths in the returned size range (for TableView)
    constrainedSize =
...
#if TARGET_OS_IOS
  /**
   * Weirdly enough, Apple expects the return value in tableView:heightForRowAtIndexPath: to
   * _include_ the height of the separator, if there is one! So if rectForRow would return 44.0 we
   * need to use 43.5.
   */
...
- (void)didLayoutSubviewsOfTableViewCell:(_ASTableViewCell *)tableViewCell {
  ...
  // Normally the content view width equals to the constrained size width (which equals to the table
  // view width). If there is a mismatch between these values, for example after the table view
  // entered or left editing mode, content view width is preferred and used to re-measure the cell
  // node.
  if (CGSizeEqualToSize(node.calculatedSize, CGSizeZero) == NO &&
      contentViewWidth != constrainedSize.max.width) {
    constrainedSize.min.width = contentViewWidth;
    constrainedSize.max.width = contentViewWidth;

    // Re-measurement is done on main to ensure thread affinity. In the worst case, this is as fast
    // as UIKit's implementation.
    //
    // Unloaded nodes *could* be re-measured off the main thread, but only with the assumption that
    // content view width is the same for all cells (because there is no easy way to get that
    // individual value before the node being assigned to a _ASTableViewCell). Also, in many cases,
    // some nodes may not need to be re-measured at all, such as when user enters and then
    // immediately leaves editing mode. To avoid premature optimization and making such assumption,
    // as well as to keep ASTableView simple, re-measurement is strictly done on main.
    ...
    // After the re-measurement, set the new constrained size to the node's backing colleciton
    // element.
