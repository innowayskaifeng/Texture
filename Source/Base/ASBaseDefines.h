...
/**
 * Create a new ObjectPointerPersonality NSHashTable by mapping `collection` over `work`, ignoring
 * nil.
 *
...
#define ASPointerTableByFlatMapping(collection, decl, work)                                   \
  ({                                                                                          \
    NSHashTable *t = [[NSHashTable alloc] initWithOptions:NSHashTableObjectPointerPersonality \
                                                 capacity:0];                                 \
    for (decl in collection) {                                                                \
      id result = work;                                                                       \
      if (result != nil) {                                                                    \
