...
#define as_activity_create(description, parent_activity, flags) \
  os_activity_create(description, parent_activity, flags)
...
#define as_activity_get_identifier(activity, outParentID) \
  os_activity_get_identifier(activity, outParentID)
...
// Create activities only when verbose enabled. Doesn't materially impact performance, but good if
// we're cluttering up activity scopes and reducing readability.
...
// Convenience for: as_activity_scope(as_activity_create(description, AS_ACTIVITY_CURRENT,
...
/**
 * The logging macros are not guarded by deployment-target checks like the activity macros are, but
 * they are only available on iOS >= 9 at runtime, so just make them conditional.
 */
