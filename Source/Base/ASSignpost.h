...
#define ASSignpost(name, identifier, arg2, color)                                    \
  AS_AT_LEAST_IOS10                                                                  \
      ? kdebug_signpost(name, (uintptr_t)identifier, (uintptr_t)arg2, 0,             \
                        ASSignpostGetColor(name, color))                             \
      : syscall(SYS_kdebug_trace, APPSDBG_CODE(DBG_MACH_CHUD, name) | DBG_FUNC_NONE, \
                (uintptr_t)identifier, (uintptr_t)arg2, 0, ASSignpostGetColor(name, color));

#define ASSignpostStartCustom(name, identifier, arg2)                                 \
  AS_AT_LEAST_IOS10                                                                   \
      ? kdebug_signpost_start(name, (uintptr_t)identifier, (uintptr_t)arg2, 0, 0)     \
...
#define ASSignpostEndCustom(name, identifier, arg2, color)                          \
  AS_AT_LEAST_IOS10                                                                 \
      ? kdebug_signpost_end(name, (uintptr_t)identifier, (uintptr_t)arg2, 0,        \
                            ASSignpostGetColor(name, color))                        \
      : syscall(SYS_kdebug_trace, APPSDBG_CODE(DBG_MACH_CHUD, name) | DBG_FUNC_END, \
                (uintptr_t)identifier, (uintptr_t)arg2, 0, ASSignpostGetColor(name, color));
