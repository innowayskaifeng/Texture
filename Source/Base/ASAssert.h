 ...
 * The construction listed above does not show the user what arguments are required and what are
 * optional.
 */
...
#define ASDisplayNodeAssertNil(condition, desc, ...) \
  ASDisplayNodeAssert((condition) == nil, desc, ##__VA_ARGS__)
#define ASDisplayNodeCAssertNil(condition, desc, ...) \
  ASDisplayNodeCAssert((condition) == nil, desc, ##__VA_ARGS__)

#define ASDisplayNodeAssertNotNil(condition, desc, ...) \
  ASDisplayNodeAssert((condition) != nil, desc, ##__VA_ARGS__)
#define ASDisplayNodeCAssertNotNil(condition, desc, ...) \
  ASDisplayNodeCAssert((condition) != nil, desc, ##__VA_ARGS__)
...
#define ASDisplayNodeAssertNotInstantiable() \
  ASDisplayNodeAssert(NO, nil, @"This class is not instantiable.");
...
#define ASDisplayNodeCAssertNotMainThread()                                                  \
  ASDisplayNodeCAssert(0 == pthread_main_np(), @"This function must be called off the main " \
                                               @"thread")

#define ASDisplayNodeAssertFlag(X, desc, ...) \
  ASDisplayNodeAssert((1 == __builtin_popcount(X)), desc, ##__VA_ARGS__)
#define ASDisplayNodeCAssertFlag(X, desc, ...) \
  ASDisplayNodeCAssert((1 == __builtin_popcount(X)), desc, ##__VA_ARGS__)

#define ASDisplayNodeAssertTrue(condition) \
  ASDisplayNodeAssert((condition), @"Expected %s to be true.", #condition)
#define ASDisplayNodeCAssertTrue(condition) \
  ASDisplayNodeCAssert((condition), @"Expected %s to be true.", #condition)

#define ASDisplayNodeAssertFalse(condition) \
  ASDisplayNodeAssert(!(condition), @"Expected %s to be false.", #condition)
#define ASDisplayNodeCAssertFalse(condition) \
  ASDisplayNodeCAssert(!(condition), @"Expected %s to be false.", #condition)
...
#define ASDisplayNodeCAssertPositiveReal(description, num)                                         \
  ASDisplayNodeCAssert(num >= 0 && num <= CGFLOAT_MAX, @"%@ must be a real positive integer: %f.", \
                       description, (CGFloat)num)
#define ASDisplayNodeCAssertInfOrPositiveReal(description, num)                             \
  ASDisplayNodeCAssert(isinf(num) || (num >= 0 && num <= CGFLOAT_MAX),                      \
                       @"%@ must be infinite or a real positive integer: %f.", description, \
                       (CGFloat)num)

#define ASDisplayNodeCAssertPermanent(object)                                      \
  ASDisplayNodeCAssert(                                                            \
      CFGetRetainCount((__bridge CFTypeRef)(object)) == CFGetRetainCount(kCFNull), \
 ...
 * even if it means violating threading requirements. These functions are used in -debugDescription
 * and let threads decide to suppress/re-enable main thread assertions.
 */
