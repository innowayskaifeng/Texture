...
- (void)setASDKCollectionNode:(ASCollectionNode *)collectionNode {
  ...
  if (dataSource != nil) {
    ASDisplayNodeFailAssert(
        @"Attempt to call %@ multiple times on the same list adapter. Not currently allowed!",
  ...
  [collectionNode onDidLoad:^(__kindof ASCollectionNode *_Nonnull collectionNode) {
    ...
    // We manually set the superclass of ASCollectionView to IGListCollectionView at runtime if
    // needed.
