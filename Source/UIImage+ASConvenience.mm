...
+ (UIImage *)as_resizableRoundedImageWithCornerRadius:(CGFloat)cornerRadius
  ...
  dispatch_once(&onceToken, ^{
    ...
    // UIBezierPath objects are fairly small and these are equally sized. 20 should be plenty for
  ...
  if (cornerColor) {
    contextIsClean = NO;
    [cornerColor setFill];
    // Copy "blend" mode is extra fast because it disregards any value currently in the buffer and
    // overrides directly.
  ...
  if (borderColor) {
    ...
    // It is rarer to have a stroke path, and our cache key only handles rounded rects for the
    // exact-stretchable size calculated by cornerRadius, so we won't bother caching this path.
    // Profiling validates this decision.
