 ...
 * allows checking whether a node is in an interface situation where it is prudent to trigger
 * certain actions: measurement, data loading, display, and visibility (the latter for animations or
 * other onscreen-only effects).
 *
 * The defualt state, ASInterfaceStateNone, means that the element is not predicted to be onscreen
 * soon and preloading should not be performed. Swift: use [] for the default behavior.
 */
typedef NS_OPTIONS(NSUInteger, ASInterfaceState) {
  ...
  /** The element may be added to a view soon that could become visible.  Measure the layout,
     including size calculation. */
  ASInterfaceStateMeasureLayout = 1 << 0,
  /** The element is likely enough to come onscreen that disk and/or network data required for
     display should be fetched. */
  ASInterfaceStatePreload = 1 << 1,
  /** The element is very likely to become visible, and concurrent rendering should be executed for
     any -setNeedsDisplay. */
   ...
   */
  ASInterfaceStateInHierarchy = ASInterfaceStateMeasureLayout | ASInterfaceStatePreload |
 ...
 * @discussion Subclasses may use this to monitor when they become visible, should free cached data,
 * and much more.
 ...
 * @discussion Subclasses may use this to monitor when a node should no longer be rendering its
 * content.
 * @note This method is guaranteed to be called on main.
 */
 ...
 * @discussion Subclasses may use this to monitor data for a node should be preloaded, either from a
 ...
 * @discussion Subclasses may use this to monitor whether preloading data for a node should be
 * canceled.
 * @note This method is guaranteed to be called on main.
 */
 ...
 * This is called after the first display pass even if network image nodes have not downloaded
 * anything (text would be done, and other nodes that are ready to do their final display). Each
 * render of every progressive jpeg network node would cause this to be called, so this hook could
 * be called up to 1 + (pJPEGcount * pJPEGrenderCount) times. The render count depends on how many
 * times the downloader calls the progressImage block.
 * @note This method is guaranteed to be called on main.
 */
