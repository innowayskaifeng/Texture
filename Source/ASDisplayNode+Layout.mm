...
- (ASLayout *)layoutThatFits:(ASSizeRange)constrainedSize parentSize:(CGSize)parentSize {
  ...
  // If one or multiple layout transitions are in flight it still can happen that layout information
  // is requested on other threads. As the pending and calculated layout to be updated in the layout
  // transition in here just a layout calculation wil be performed without side effect
  ...
  if (_calculatedDisplayNodeLayout.isValid(constrainedSize, parentSize, version)) {
    ASDisplayNodeAssertNotNil(_calculatedDisplayNodeLayout.layout,
                              @"-[ASDisplayNode layoutThatFits:parentSize:] "
                              @"_calculatedDisplayNodeLayout.layout should not be nil! %@",
                              self);
    layout = _calculatedDisplayNodeLayout.layout;
  } else if (_pendingDisplayNodeLayout.isValid(constrainedSize, parentSize, version)) {
    ASDisplayNodeAssertNotNil(_pendingDisplayNodeLayout.layout,
                              @"-[ASDisplayNode layoutThatFits:parentSize:] "
                              @"_pendingDisplayNodeLayout.layout should not be nil! %@",
                              self);
    ...
    layout = [self calculateLayoutThatFits:constrainedSize
                          restrictedToSize:self.style.size
                      relativeToParentSize:parentSize];
    as_log_verbose(ASLayoutLog(), "Established pending layout for %@ in %s", self,
                   sel_getName(_cmd));
    _pendingDisplayNodeLayout = ASDisplayNodeLayout(layout, constrainedSize, parentSize, version);
    ASDisplayNodeAssertNotNil(layout,
                              @"-[ASDisplayNode layoutThatFits:parentSize:] newly calculated "
...
- (void)setPrimitiveTraitCollection:(ASPrimitiveTraitCollection)traitCollection {
  if (ASPrimitiveTraitCollectionIsEqualToASPrimitiveTraitCollection(
          traitCollection, _primitiveTraitCollection.load()) == NO) {
...
- (ASTraitCollection *)asyncTraitCollection {
  return [ASTraitCollection
      traitCollectionWithASPrimitiveTraitCollection:self.primitiveTraitCollection];
}
 ...
 *
 * @discussion The size of a root node is determined by each subnode. Calling invalidateSize will
 * let the root node know that the intrinsic size of the receiver node is no longer valid and a
 * resizing of the root node needs to happen.
 */
...
- (void)_u_measureNodeWithBoundsIfNecessary:(CGRect)bounds {
  ...
  // If there is no such _pending, check if _calculated is valid to reuse (avoiding recalculation
  // below).
  ...
  as_log_verbose(ASLayoutLog(), "Node %@, bounds size %@, calculatedSize %@, calculatedIsDirty %d",
                 self, NSStringFromCGSize(boundsSizeForLayout),
...
#define layoutSizeDifferentFromBounds \
  !CGSizeEqualToSize(nextLayout.layout.size, boundsSizeForLayout)

  // nextLayout was likely created by a call to layoutThatFits:, check if it is valid and can be
  // applied. If our bounds size is different than it, or invalid, recalculate.  Use #define to
  // avoid nullptr->
  ...
  // If our new layout's desired size for self doesn't match current size, ask our parent to update
  // it. This can occur for either pre-calculated or newly-calculated layouts.
  if (nextLayout.requestedLayoutFromAbove == NO &&
    ...
    as_log_verbose(ASLayoutLog(),
                   "Layout size doesn't match bounds size. Requesting layout from above.");
    ...
    // particular ASLayout object, and shouldn't loop asking again unless we have a different
    // ASLayout.
    ...
    // Update the layout's version here because _u_setNeedsLayoutFromAbove calls __setNeedsLayout
    // which in turn increases _layoutVersion Failing to do this will cause the layout to be invalid
    // immediately
  ...
  // If a parent is currently executing a layout transition, perform our layout application after
  // it.
...
- (ASSizeRange)_locked_constrainedSizeForLayoutPass {
  ...
  if (_pendingDisplayNodeLayout.requestedLayoutFromAbove ||
    ...
    // We assume the size from the last returned layoutThatFits: layout was applied so use the
    // pending display node layout constrained size
    return _pendingDisplayNodeLayout.constrainedSize;
  } else if (_calculatedDisplayNodeLayout.layout != nil &&
             (_calculatedDisplayNodeLayout.requestedLayoutFromAbove ||
              CGSizeEqualToSize(_calculatedDisplayNodeLayout.layout.size, boundsSizeForLayout))) {
    ...
    // In this case neither the _pendingDisplayNodeLayout or the _calculatedDisplayNodeLayout
    // constrained size can be reused, so the current bounds is used. This is usual the case if a
    // frame was set manually that differs to the one returned from layoutThatFits: or
    // layoutThatFits: was never called
...
- (void)transitionLayoutWithSizeRange:(ASSizeRange)constrainedSize
...

  {
    ...
    if (ASHierarchyStateIncludesLayoutPending(_hierarchyState)) {
      ASDisplayNodeAssert(
          NO, @"Can't start a transition when one of the supernodes is performing one.");
      return;
    }
  }

  // Invalidate calculated layout because this method acts as an animated "setNeedsLayout" for
  // nodes. If the user has reconfigured the node and calls this, we should never return a stale
  // layout for subsequent calls to layoutThatFits: regardless of size range. We choose this method
  // rather than -setNeedsLayout because that method also triggers a CA layout invalidation, which
  // isn't necessary at this time. See https://github.com/TextureGroup/Texture/issues/463
  [self invalidateCalculatedLayout];

  // Every new layout transition has a transition id associated to check in subsequent transitions
  // for cancelling
  ...
  asdisplaynode_iscancelled_block_t isCancelled = ^{
    ...
    if (result) {
      as_log_verbose(ASLayoutLog(), "Transition %d canceled, superseded by %d", transitionID,
                     _transitionID.load());
    }
    return result;
  };
  ...
  void (^transitionBlock)(void) = ^{
    ...
    // Perform a full layout creation pass with passed in constrained size to create the new layout
    // for the transition
    ...
    ASPerformBlockOnMainThread(^{
      ...
      _ASTransitionContext *pendingLayoutTransitionContext;
      {
        ...
        _pendingLayoutTransition = pendingLayoutTransition =
            [[ASLayoutTransition alloc] initWithNode:self
                                       pendingLayout:pendingLayout
                                      previousLayout:previousLayout];
        // Setup context for pending layout transition. we need to hold a strong reference to the
        // context
...
/*
 * Hook for subclasses to perform an animation based on the given ASContextTransitioning. By default
 * a fade in and out animation is provided.
 */
- (void)animateLayoutTransition:(id<ASContextTransitioning>)context {
  ...
  const auto insertedSubnodeContexts =
      [[NSMutableArray<_ASAnimatedTransitionContext *> alloc] init];
  ...
  for (ASDisplayNode *insertedSubnode in insertedSubnodes) {
    [insertedSubnodeContexts
        addObject:[_ASAnimatedTransitionContext contextForNode:insertedSubnode
                                                         alpha:insertedSubnode.alpha]];
  }
  for (ASDisplayNode *removedSubnode in removedSubnodes) {
    [removedSubnodeContexts
        addObject:[_ASAnimatedTransitionContext contextForNode:removedSubnode
...
/**
 * Hook for subclasses to clean up nodes after the transition happened. Furthermore this can be used
 * from subclasses to manually perform deletions.
 */
...
/**
 * Completes the pending layout transition immediately without going through the the Layout
...
/**
 * Can be directly called to commit the given layout transition immediately to complete without
 * calling through to the Layout Transition Animation API
 */
- (void)_completeLayoutTransition:(ASLayoutTransition *)layoutTransition {
  // Layout transition is not supported for nodes that do not have automatic subnode management
  // enabled
  ...
  if (ASDisplayNodeThreadIsMain() || layoutTransition.isSynchronous == NO) {
    // Committing the layout transition will result in subnode insertions and removals, both of
    ...
    // Subnode insertions and removals need to happen always on the main thread if at least one
    // subnode is already loaded
    ASPerformBlockOnMainThread(^{
      [layoutTransition commitTransition];
    });
  }
...
- (void)_assertSubnodeState {
  ...
  // This can occur in rare cases if main thread layout is flushed while a background layout is
  // calculating.
  ...
  if (insertions.count > 0) {
    NSLog(@"Warning: node's layout includes subnode that has not been added: node = %@, subnodes = "
          @"%@, subnodes in layout = %@",
  ...
  if (deletions) {
    for (NSUInteger i = deletions.lastIndex; i != NSNotFound;
         i = [deletions indexLessThanIndex:i]) {
...
- (void)_pendingLayoutTransitionDidComplete {
  // This assertion introduces a breaking behavior for nodes that has ASM enabled but also manually
  // manage some subnodes. Let's gate it behind YOGA flag.
#if YOGA
  ...
  // We generate placeholders at -layoutThatFits: time so that a node is guaranteed to have a
  // placeholder ready to go. This is also because measurement is usually asynchronous, but
  // placeholders need to be set up synchronously. First measurement is guaranteed to be before the
  // node is onscreen, so we can create the image async. but still have it appear sync.
