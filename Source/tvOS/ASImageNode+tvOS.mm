...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
  ...
  if (!self.isDefaultFocusAppearance) {
    ...
    CATransform3D pitchTransform =
        CATransform3DMakeRotation(GLKMathDegreesToRadians(pitch), 1.0, 0.0, 0.0);
    CATransform3D yawTransform =
        CATransform3DMakeRotation(GLKMathDegreesToRadians(yaw), 0.0, 1.0, 0.0);
    CATransform3D transform = CATransform3DConcat(pitchTransform, yawTransform);
    CATransform3D scaleAndTransform =
        CATransform3DConcat(transform, CATransform3DMakeAffineTransform(CGAffineTransformScale(
                                           CGAffineTransformIdentity, 1.25, 1.25)));
...
- (UIView *)getView {
  ...
  // If we are inside a ASCellNode, then we need to apply our focus effects to the ASCellNode
  // view/layer rather than the ASImageNode view/layer.
