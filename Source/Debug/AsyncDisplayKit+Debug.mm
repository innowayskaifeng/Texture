...
- (void)layout {
  ...
  if ([ASControlNode enableHitTestDebug]) {
    // Construct hitTestDebug highlight overlay frame indicating tappable area of a node, which can
    // be restricted by two things:

    // (1) Any parent's tapable area (its own bounds + hitTestSlop) may restrict the desired
    // tappable area expansion using hitTestSlop of a child as UIKit event delivery (hitTest:) will
    // not search sub-hierarchies if one of our parents does not return YES for pointInside:. To
    // circumvent this restriction, a developer will need to set / adjust the hitTestSlop on the
    // limiting parent. This is indicated in the overlay by a dark GREEN edge. This is an ACTUAL
    // restriction.

    // (2) Any parent's .clipToBounds. If a parent is clipping, we cannot show the overlay outside
    // that area (although it still respond to touch). To indicate that the overlay cannot
    // accurately display the true tappable area, the overlay will have an ORANGE edge. This is a
    ...
    // FIXED: Stop climbing hierarchy if UIScrollView is encountered (its offset bounds origin may
    // make it seem like our events will be clipped when scrolling will actually reveal them
    // (because this process will not re-run due to scrolling))
    while (intersectSuperlayer &&
           ![intersectSuperlayer.delegate respondsToSelector:@selector(contentOffset)]) {
      ...
      if (!CGSizeEqualToSize(parentHitRect.size, intersectRectInParentCoordinates.size)) {
        ...
        if (parentClipsToBounds) {
          clipsToBoundsClippedEdges =
              [self setEdgesOfIntersectionForChildRect:intersectRectInParentCoordinates
                                            parentRect:parentHitRect
    ...
    if (clippedEdges == UIRectEdgeNone) {
      ...
      UIColor *clipsBorderColor = [UIColor colorWithRed:30 / 255.0
                                                  green:90 / 255.0
                                                   blue:50 / 255.0
                                                  alpha:0.7];
      ...
      UIEdgeInsets edgeInsets =
          UIEdgeInsetsMake(borderWidth, borderWidth, borderWidth, borderWidth);
      debugOverlay.image =
...
- (void)drawEdgeIfClippedWithEdges:(UIRectEdge)rectEdge
  ...
  // so that the developer is aware that increasing hitTestSlop will not result in an expanded
  // tappable area
...
+ (UIWindow *)keyWindow {
  // hack to work around app extensions not having UIApplication...not sure of a better way to do
  // this?
...
- (instancetype)initWithFrame:(CGRect)frame {
  ...
  if (self) {
    ...
    self.frame =
        CGRectMake(windowSize.width - (windowSize.width / OVERLAY_SCALE) - OVERLAY_INSET,
                   windowSize.height - OVERLAY_INSET, windowSize.width / OVERLAY_SCALE, 0.0);

    UIPanGestureRecognizer *panGR =
        [[UIPanGestureRecognizer alloc] initWithTarget:self
                                                action:@selector(rangeDebugOverlayWasPanned:)];
...
- (void)layoutToFitAllBarsExcept:(NSInteger)barsToClip {
  ...
  CGRect barRect =
      CGRectMake(0, boundsSize.height - BAR_THICKNESS, self.bounds.size.width, BAR_THICKNESS);
  ...
  for (_ASRangeDebugBarView *barView in [_rangeControllerViews copy]) {
    ...
    ASInterfaceState interfaceState = [barView.rangeController.dataSource
        interfaceStateForRangeController:barView.rangeController];
...
- (void)addRangeController:(ASRangeController *)rangeController {
  ...
  _ASRangeDebugBarView *rangeView =
      [[_ASRangeDebugBarView alloc] initWithRangeController:rangeController];
               ...
               interfaceState:(ASInterfaceState)interfaceState;
{
  ...
  if (!((displayTuningParameters.leadingBufferScreenfuls +
         displayTuningParameters.trailingBufferScreenfuls) == 0)) {
    leadingDisplayTuningRatio = displayTuningParameters.leadingBufferScreenfuls /
                                (displayTuningParameters.leadingBufferScreenfuls +
                                 displayTuningParameters.trailingBufferScreenfuls);
  }
  if (!((preloadTuningParameters.leadingBufferScreenfuls +
         preloadTuningParameters.trailingBufferScreenfuls) == 0)) {
    leadingPreloadTuningRatio = preloadTuningParameters.leadingBufferScreenfuls /
                                (preloadTuningParameters.leadingBufferScreenfuls +
                                 preloadTuningParameters.trailingBufferScreenfuls);
  }
...
- (_ASRangeDebugBarView *)barViewForRangeController:(ASRangeController *)controller {
  ...
  for (_ASRangeDebugBarView *rangeView in
       [[_rangeControllerViews reverseObjectEnumerator] allObjects]) {
    // remove barView if its rangeController has been deleted
    if (!rangeView.rangeController) {
      rangeView.destroyOnLayout = YES;
      [self setNeedsLayout];
    }
    ASInterfaceState interfaceState = [rangeView.rangeController.dataSource
        interfaceStateForRangeController:rangeView.rangeController];
...
- (void)layoutSubviews {
  ...
  rect.size =
      [_debugText layoutThatFits:ASSizeRangeMake(CGSizeZero, CGSizeMake(CGFLOAT_MAX, CGFLOAT_MAX))]
          .size;
  ...
  if (ASScrollDirectionContainsLeft(_scrollDirection) ||
      ASScrollDirectionContainsUp(_scrollDirection)) {
  ...
  } else if (ASScrollDirectionContainsRight(_scrollDirection) ||
             ASScrollDirectionContainsDown(_scrollDirection)) {
    if (displayLargerThanPreload) {
  ...
  [UIView animateWithDuration:animate ? 0.3 : 0.0
                   ...
                   animations:^{
                     _visibleRect.frame = CGRectMake(HORIZONTAL_INSET + visiblePoint, rect.origin.y,
                                                     visibleDimension, subCellHeight);
                     _displayRect.frame = CGRectMake(HORIZONTAL_INSET + displayPoint, rect.origin.y,
                                                     displayDimension, subCellHeight);
                     _preloadRect.frame = CGRectMake(HORIZONTAL_INSET + preloadPoint, rect.origin.y,
                                                     preloadDimension, subCellHeight);
                   }
                   completion:^(BOOL finished){
                   }];
...
- (void)setBarDebugLabelsWithSize:(CGFloat)size {
  ...
  if (_debugString) {
    _debugText.attributedText = [_ASRangeDebugBarView whiteAttributedStringFromString:_debugString
                                                                             withSize:size];
  }

  if (ASScrollDirectionContainsVerticalDirection(_scrollDirection)) {
    _leftDebugText.attributedText = [_ASRangeDebugBarView whiteAttributedStringFromString:@"▲"
                                                                                 withSize:size];
    _rightDebugText.attributedText = [_ASRangeDebugBarView whiteAttributedStringFromString:@"▼"
                                                                                  withSize:size];
  } else if (ASScrollDirectionContainsHorizontalDirection(_scrollDirection)) {
    _leftDebugText.attributedText = [_ASRangeDebugBarView whiteAttributedStringFromString:@"◀︎"
                                                                                 withSize:size];
    _rightDebugText.attributedText = [_ASRangeDebugBarView whiteAttributedStringFromString:@"▶︎"
                                                                                  withSize:size];
  }

  _leftDebugText.hidden =
      (_scrollDirection != ASScrollDirectionLeft && _scrollDirection != ASScrollDirectionUp);
  _rightDebugText.hidden =
      (_scrollDirection != ASScrollDirectionRight && _scrollDirection != ASScrollDirectionDown);
}
...
- (ASImageNode *)createRangeNodeWithColor:(UIColor *)color {
  ...
  rangeBarImageNode.image = [UIImage
      as_resizableRoundedImageWithCornerRadius:RANGE_BAR_CORNER_RADIUS
...
+ (NSAttributedString *)whiteAttributedStringFromString:(NSString *)string
                                               withSize:(CGFloat)size NS_RETURNS_RETAINED {
