...
- (void)setLayerBacked:(BOOL)layerBacked {
  ASDisplayNodeAssert(!self.isLiveMap, @"ASMapNode can not be layer backed whilst .liveMap = YES, "
                                       @"set .liveMap = NO to use layer backing.");
...
- (void)takeSnapshot {
  // If our size is zero, we want to avoid calling a default sized snapshot. Set
  // _snapshotAfterLayout to YES so if layout changes in the future, we'll try snapshotting again.
  ...
  [_snapshotter
         startWithQueue:dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0)
          ...
          if (annotations.count > 0) {
            ...
            for (id<MKAnnotation> annotation in annotations) {
              if (strongSelf.imageForStaticMapAnnotationBlock) {
                ...
                pinImage =
                    strongSelf.imageForStaticMapAnnotationBlock(annotation, &pinCenterOffset);
                if (!pinImage) {
...
- (MKCoordinateRegion)regionToFitAnnotations:(NSArray<id<MKAnnotation>> *)annotations {
  ...
  for (id<MKAnnotation> annotation in annotations) {
    topLeftCoord = CLLocationCoordinate2DMake(
        std::fmax(topLeftCoord.latitude, annotation.coordinate.latitude),
        std::fmin(topLeftCoord.longitude, annotation.coordinate.longitude));
    bottomRightCoord = CLLocationCoordinate2DMake(
        std::fmin(bottomRightCoord.latitude, annotation.coordinate.latitude),
...
- (void)setSnapshotSizeWithReloadIfNeeded:(CGSize)snapshotSize {
  if (snapshotSize.height > 0 && snapshotSize.width > 0 &&
      !CGSizeEqualToSize(self.options.size, snapshotSize)) {
...
- (CGSize)calculateSizeThatFits:(CGSize)constrainedSize {
  // FIXME: Need a better way to allow maps to take up the right amount of space in a layout
  // (sizeRange, etc) These fallbacks protect against inheriting a constrainedSize that contains a
  // CGFLOAT_MAX value.
...
// -layout isn't usually needed over -layoutSpecThatFits, but this way we can avoid a needless node
...
- (void)layout {
  ...
  if (self.isLiveMap) {
    ...
    // If our bounds.size is different from our current snapshot size, then let's request a new
    // image from MKMapSnapshotter.
    if (_needsMapReloadOnBoundsChange) {
      [self setSnapshotSizeWithReloadIfNeeded:self.bounds.size];
      // FIXME: Adding a check for Preload here seems to cause intermittent map load failures, but
      // shouldn't. if (ASInterfaceStateIncludesPreload(self.interfaceState)) {
    }
  }
}
