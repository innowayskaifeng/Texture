...
// TODO: It would be nice to remove this dependency; it's the only subclass using more than
// +FrameworkSubclasses.h
...
- (BOOL)isEqual:(id)object {
  ...
  // Optimization opportunity: The `isKindOfClass` call here could be avoided by not using the
  // NSObject `isEqual:` convention and instead using a custom comparison function that assumes all
  // items are heterogeneous. However, profiling shows that our entire `isKindOfClass` expression is
  // only ~1/40th of the total overheard of our caching, so it's likely not high-impact.
  if ([object isKindOfClass:[ASImageNodeContentsKey class]]) {
    ...
    return [_image isEqual:other.image] && CGSizeEqualToSize(_backingSize, other.backingSize) &&
           ...
           _willDisplayNodeContentWithRenderingContext ==
               other.willDisplayNodeContentWithRenderingContext &&
           _didDisplayNodeContentWithRenderingContext ==
               other.didDisplayNodeContentWithRenderingContext &&
...
- (instancetype)init {
  ...
  // If no backgroundColor is set to the image node and it's a subview of UITableViewCell,
  // UITableView is setting the opaque value of all subviews to YES if highlighting / selection is
  // happening and does not set it back to the initial value. With setting a explicit
  // backgroundColor we can prevent that change.
...
- (void)_locked_setImage:(UIImage *)image {
  ...
  if (image != nil) {
    // We explicitly call setNeedsDisplay in this case, although we know setNeedsDisplay will be
    // called with lock held. Therefore we have to be careful in methods that are involved with
    // setNeedsDisplay to not run into a deadlock
    ...
    if ([ASImageNode shouldShowImageScalingOverlay] && _debugLabelNode == nil) {
      // do not use ASPerformBlockOnMainThread here, if it performs the block synchronously it will
      // continue holding the lock while calling addSubnode.
  ...
  BOOL shouldReleaseImageOnBackgroundThread =
      oldImageSize.width > kMinReleaseImageOnBackgroundSize.width ||
...
- (NSObject *)drawParametersForAsyncLayer:(_ASDisplayLayer *)layer {
  ...
  drawParameters->_willDisplayNodeContentWithRenderingContext =
      _willDisplayNodeContentWithRenderingContext;
  drawParameters->_didDisplayNodeContentWithRenderingContext =
      _didDisplayNodeContentWithRenderingContext;
...
+ (UIImage *)displayWithParameters:(id<NSObject>)parameter
  ...
  CGSize imageSizeInPixels =
      CGSizeMake(imageSize.width * image.scale, imageSize.height * image.scale);
  CGSize boundsSizeInPixels = CGSizeMake(std::floor(bounds.size.width * contentsScale),
                                         std::floor(bounds.size.height * contentsScale));
  ...
  if (boundsSizeInPixels.width * contentsScale < 1.0f ||
      boundsSizeInPixels.height * contentsScale < 1.0f || imageSizeInPixels.width < 1.0f ||
  ...
  if (!cropEnabled || !contentModeSupported || stretchable) {
    ...
    ASCroppedImageBackingSizeAndDrawRectInBounds(imageSizeInPixels, boundsSizeInPixels, contentMode,
                                                 cropRect, forceUpscaling, forcedSize, &backingSize,
                                                 &imageDrawRect);
  }
  ...
  contentsKey.willDisplayNodeContentWithRenderingContext =
...
+ (ASWeakMapEntry *)contentsForkey:(ASImageNodeContentsKey *)key
  ...
  // cache miss
  UIImage *contents = [self createContentsForkey:key
...
+ (UIImage *)createContentsForkey:(ASImageNodeContentsKey *)key
                   drawParameters:(id)drawParameters
                      isCancelled:(asdisplaynode_iscancelled_block_t)isCancelled {
  // The following `ASGraphicsBeginImageContextWithOptions` call will sometimes take take longer
  // than 5ms on an A5 processor for a 400x800 backingSize. Check for cancellation before we call
  // it.
  if (isCancelled()) {
    return nil;
  }
  ...
  // ASCroppedImageBackingSizeAndDrawRectInBounds will do its rounding on pixel instead of point
  // boundaries
  ...
  // The issue is present in Mac OS X El Capitan and has been seen hanging Pro apps like Adobe
  // Premiere, as well as iOS games, and a small number of ASDK apps that provide the same image
  // reference to many separate ASImageNodes.  A workaround is to set .displaysAsynchronously = NO
  // for the nodes that may get the same pointer for a given UI asset image, etc.
  // FIXME: We should replace @synchronized here, probably using a global, locked NSMutableSet, and
  // only if the object already exists in the set we should create a semaphore to signal waiting
  // threads upon removal of the object from the set when the operation completes. Another option is
  // to have ASDisplayNode+AsyncDisplay coordinate these cases, and share the decoded buffer.
  // Details tracked in https://github.com/facebook/AsyncDisplayKit/issues/1068

  UIImage *image = key.image;
  BOOL canUseCopy =
      (contextIsClean || ASImageAlphaInfoIsOpaque(CGImageGetAlphaInfo(image.CGImage)));
...
- (void)displayDidFinish {
  ...
  if (hasDebugLabel) {
    ...
    CGSize boundsSizeInPixels =
        CGSizeMake(std::floor(self.bounds.size.width * self.contentsScale),
                   std::floor(self.bounds.size.height * self.contentsScale));
    CGFloat pixelCountRatio = (imageSizeInPixels.width * imageSizeInPixels.height) /
                              (boundsSizeInPixels.width * boundsSizeInPixels.height);
    if (pixelCountRatio != 1.0) {
      NSString *scaleString = [NSString stringWithFormat:@"%.2fx", pixelCountRatio];
      _debugLabelNode.attributedText =
          [[NSAttributedString alloc] initWithString:scaleString
...
- (void)setCropEnabled:(BOOL)cropEnabled
     recropImmediately:(BOOL)recropImmediately
              inBounds:(CGRect)cropBounds {
  __instanceLock__.lock();
...
- (void)layout {
  ...
  if (_debugLabelNode) {
    CGSize boundsSize = self.bounds.size;
    CGSize debugLabelSize =
        [_debugLabelNode layoutThatFits:ASSizeRangeMake(CGSizeZero, boundsSize)].size;
...
asimagenode_modification_block_t ASImageNodeRoundBorderModificationBlock(CGFloat borderWidth,
                                                                         UIColor *borderColor) {
  return ^(UIImage *originalImage) {
    ASGraphicsBeginImageContextWithOptions(originalImage.size, NO, originalImage.scale);
    UIBezierPath *roundOutline =
        [UIBezierPath bezierPathWithOvalInRect:(CGRect){CGPointZero, originalImage.size}];
