...
- (instancetype)initWithCache:(id<ASImageCacheProtocol>)cache
  ...
  _downloaderFlags.downloaderImplementsSetProgress = [downloader
      respondsToSelector:@selector(setProgressImageBlock:callbackQueue:withDownloadIdentifier:)];
  _downloaderFlags.downloaderImplementsSetPriority =
      [downloader respondsToSelector:@selector(setPriority:withDownloadIdentifier:)];
  _downloaderFlags.downloaderImplementsAnimatedImage =
      [downloader respondsToSelector:@selector(animatedImageWithData:)];
  _downloaderFlags.downloaderImplementsCancelWithResume = [downloader
      respondsToSelector:@selector(cancelImageDownloadWithResumePossibilityForIdentifier:)];

  _cacheFlags.cacheSupportsClearing =
      [cache respondsToSelector:@selector(clearFetchedImageFromCacheWithURL:)];
...
/// Setter for public image property. It has the side effect of setting an internal
/// _imageWasSetExternally that prevents setting an image internally. Setting an image internally
/// should happen with the _setImage: method
...
- (void)_locked_setImage:(UIImage *)image {
  ...
  BOOL shouldCancelAndClear =
      imageWasSetExternally && (imageWasSetExternally != _imageWasSetExternally);
  _imageWasSetExternally = imageWasSetExternally;
  if (shouldCancelAndClear) {
    ASDisplayNodeAssertNil(_URL, @"Directly setting an image on an ASNetworkImageNode causes it to "
                                 @"behave like an ASImageNode instead of an ASNetworkImageNode. If "
                                 @"this is what you want, set the URL to nil first.");
    _URL = nil;
...
- (void)setURL:(NSURL *)URL resetToDefault:(BOOL)reset {
  {
    ...
    ASDisplayNodeAssert(_imageWasSetExternally == NO,
                        @"Setting a URL to an ASNetworkImageNode after setting an image changes "
                        @"its behavior from an ASImageNode to an ASNetworkImageNode. If this is "
                        @"what you want, set the image to nil first.");
    ...
    // If URL is nil and URL was not equal to _URL (checked at the top), then we previously had a
    // URL but it's been nil'd out.
    BOOL hadURL = (URL == nil);
    if (reset || hadURL) {
 ...
 * so we always have to dispatch to the main thread to ensure that we queue the operations in the
 * correct order. (see comment in displayDidFinish)
 */
...
- (void)setDelegate:(id<ASNetworkImageNodeDelegate>)delegate {
  ...
  _delegateFlags.delegateDidStartFetchingData =
      [delegate respondsToSelector:@selector(imageNodeDidStartFetchingData:)];
  _delegateFlags.delegateDidFailWithError = [delegate respondsToSelector:@selector(imageNode:
                                                                             didFailWithError:)];
  _delegateFlags.delegateDidFinishDecoding =
      [delegate respondsToSelector:@selector(imageNodeDidFinishDecoding:)];
  _delegateFlags.delegateDidLoadImage = [delegate respondsToSelector:@selector(imageNode:
                                                                            didLoadImage:)];
  _delegateFlags.delegateDidLoadImageFromCache =
      [delegate respondsToSelector:@selector(imageNodeDidLoadImageFromCache:)];
  _delegateFlags.delegateDidLoadImageWithInfo =
      [delegate respondsToSelector:@selector(imageNode:didLoadImage:info:)];
}
...
/* displayWillStartAsynchronously: in ASMultiplexImageNode has a very similar implementation.
 Changes here are likely necessary in ASMultiplexImageNode as well. */
...
/* visibileStateDidChange in ASMultiplexImageNode has a very similar implementation. Changes here
 are likely necessary in ASMultiplexImageNode as well. */
...
- (void)handleProgressImage:(UIImage *)progressImage
  ...
  as_log_verbose(ASImageLoadingLog(), "Received progress image for %@ q: %.2g id: %@", self,
...
- (void)_updateProgressImageBlockOnDownloaderIfNeeded {
  ...
  [self lock];
  BOOL shouldRender =
      _shouldRenderProgressImages && ASInterfaceStateIncludesVisible(_interfaceState);
  ...
  if (oldDownloadIDForProgressBlock != nil) {
    as_log_verbose(ASImageLoadingLog(), "Disabled progress images for %@ id: %@", self,
  ...
  if (newDownloadIDForProgressBlock != nil) {
    ...
    as_log_verbose(ASImageLoadingLog(), "Enabled progress images for %@ id: %@", self,
                   newDownloadIDForProgressBlock);
    [_downloader
         setProgressImageBlock:^(UIImage *_Nonnull progressImage, CGFloat progress,
                                 id _Nullable downloadIdentifier) {
           [weakSelf handleProgressImage:progressImage
                                progress:progress
  ...
  {
    ASLockScopeSelf();
    // Check if the oldDownloadIDForProgressBlock still is the same as the
    ...
    if (_downloadIdentifierForProgressBlock == oldDownloadIDForProgressBlock) {
      ...
      // If this is not the case another thread did change the _downloadIdentifierForProgressBlock
      // already so we have to deregister the newDownloadIDForProgressBlock that we registered above
      clearAndReattempt = YES;
    }
  }

  if (clearAndReattempt) {
    // In this case another thread changed the _downloadIdentifierForProgressBlock before we
    // finished registering the new progress block for newDownloadIDForProgressBlock ID. Let's clear
    // it now and reattempt to register
...
- (void)_locked_cancelImageDownloadWithResumePossibility:(BOOL)storeResume {
  ...
  if (_downloadIdentifier) {
    if (storeResume && _downloaderFlags.downloaderImplementsCancelWithResume) {
      as_log_verbose(ASImageLoadingLog(), "Canceling image download w resume for %@ id: %@", self,
                     _downloadIdentifier);
...
- (void)_downloadImageWithCompletion:(void (^)(id<ASImageContainerProtocol> imageContainer,
                                               NSError *,
                                               id downloadIdentifier,
                                               id userInfo))finished {
  ASPerformBlockOnBackgroundThread(^{
    NSURL *url;
    id downloadIdentifier;
    BOOL cancelAndReattempt = NO;

    // Below, to avoid performance issues, we're calling downloadImageWithURL without holding the
    // lock. This is a bit ugly because We need to reobtain the lock after and ensure that the task
    // we've kicked off still matches our URL. If not, we need to cancel it and try again.
    {
      ASLockScopeSelf();
      url = self->_URL;
    }

    downloadIdentifier = [self->_downloader
        downloadImageWithURL:url
               callbackQueue:[self callbackQueue]
            downloadProgress:NULL
                  completion:^(id<ASImageContainerProtocol> _Nullable imageContainer,
                               NSError *_Nullable error, id _Nullable downloadIdentifier,
...

    {
      ...
      if (ASObjectIsEqual(self->_URL, url)) {
        ...
        // The URL changed since we kicked off our download task. This shouldn't happen often so
        // we'll pay the cost and cancel that request and kick off a new one.
    ...
    if (cancelAndReattempt) {
      if (downloadIdentifier != nil) {
        as_log_verbose(ASImageLoadingLog(), "Canceling image download no resume for %@ id: %@",
                       self, downloadIdentifier);
...
- (void)_lazilyLoadImageIfNecessary {
  ...
  if (!isImageLoaded && URL != nil && currentDownloadIdentifier == nil) {
    ...
    if (URL.isFileURL) {
      dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (self->_shouldCacheImage) {
          ...
          if (nonAnimatedImage == nil) {
            // If we couldn't find it, execute an -imageNamed:-like search so we can find resources
            // even if the extension is not provided in the path.  This allows the same path to work
            // regardless of shouldCacheImage.
            NSString *filename = [[NSBundle mainBundle] pathForResource:URL.path.lastPathComponent
                                                                 ofType:nil];
            if (filename != nil) {
      ...
      const auto finished = ^(id<ASImageContainerProtocol> imageContainer, NSError *error,
                              id downloadIdentifier, ASNetworkImageSourceType imageSource,
                              id userInfo) {
        ASPerformBlockOnBackgroundThread(^{
          ...
          // No longer in preload range, no point in setting the results (they won't be cleared in
          // exit preload range)
          ...
          if (imageContainer != nil) {
            ...
            if (animatedImageData &&
                strongSelf->_downloaderFlags.downloaderImplementsAnimatedImage) {
      ...
      if (_cache != nil) {
        ...
        ASImageCacherCompletion completion = ^(id<ASImageContainerProtocol> imageContainer) {
          ...
          if ([imageContainer asdk_image] == nil && self->_downloader != nil) {
            ...
            [self
                _downloadImageWithCompletion:^(id<ASImageContainerProtocol> imageContainer,
        ...
        [self _downloadImageWithCompletion:^(id<ASImageContainerProtocol> imageContainer,
                                             NSError *error, id downloadIdentifier, id userInfo) {
          finished(imageContainer, error, downloadIdentifier, ASNetworkImageSourceDownload,
                   userInfo);
        }];
...
- (void)displayDidFinish {
...

  {
    ...
    if (_delegateFlags.delegateDidFinishDecoding && self.layer.contents != nil) {
      /* We store the image quality in _currentImageQuality whenever _image is set. On the following
       displayDidFinish, we'll know that _currentImageQuality is the quality of the image that has
       just finished rendering. In order for this to be accurate, we need to be sure we are on main
       thread when we set _currentImageQuality. Otherwise, it is possible for _currentImageQuality
       to be modified at a point where it is too late to cancel the main thread's previous display
       (the final sentinel check has passed), but before the displayDidFinish of the previous
       display pass is called. In this situation, displayDidFinish would be called and we would set
       _renderedImageQuality to the new _currentImageQuality, but the actual quality of the rendered
       image should be the previous value stored in _currentImageQuality. */
