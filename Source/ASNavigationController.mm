...
- (NSInteger)visibilityDepthOfChildViewController:(UIViewController *)childViewController {
  NSUInteger viewControllerIndex =
      [self.viewControllers indexOfObjectIdenticalTo:childViewController];
  if (viewControllerIndex == NSNotFound) {
    // If childViewController is not actually a child, return NSNotFound which is also a really
    // large number.
    return NSNotFound;
  }
...
- (NSArray *)popToRootViewControllerAnimated:(BOOL)animated {
  ...
  as_log_info(ASNodeLog(), "Popped view controllers %@ from %@",
              ASGetDescriptionValueString(viewControllers), self);
...
- (void)setViewControllers:(NSArray *)viewControllers {
  // NOTE: As of now this method calls through to setViewControllers:animated: so no need to
  // log/activity here.
