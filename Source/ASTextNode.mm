...
- (BOOL)isEqual:(ASTextNodeRendererKey *)object {
  ...
  return _attributes == object->_attributes &&
...
/**
 The concept here is that neither the node nor layout should ever have a strong reference to the
 renderer object. This is to reduce memory load when loading thousands and thousands of text nodes
 into memory at once. Instead we maintain a LRU renderer cache that is queried via a unique key
 based on text kit attributes and constrained size.
 */

static ASTextKitRenderer *rendererForAttributes(ASTextKitAttributes attributes,
                                                CGSize constrainedSize) {
  ...
  if (renderer == nil) {
    renderer = [[ASTextKitRenderer alloc] initWithTextKitAttributes:attributes
...
- (NSString *)_plainStringForDescription {
  NSString *plainString = [[self.attributedText string]
...
- (void)setTextContainerInset:(UIEdgeInsets)textContainerInset {
  if (ASLockedSelfCompareAssignCustom(_textContainerInset, textContainerInset,
...
// Returns the ascender of the first character in attributedString by also including the line height
// if specified in paragraph style.
...
- (void)setAttributedText:(NSAttributedString *)attributedText {
...

  {
    ...
    NSAttributedString *cleanedAttributedString =
        ASCleanseAttributedStringOfCoreTextAttributes(attributedText);

    // Invalidating the truncation text must be done while we still hold the lock. Because after we
    // release it, another thread may set a new truncation text that will then be cleared by this
    // thread, other may draw this soon-to-be-invalidated text.
  ...
  self.isAccessibilityElement =
      (currentAttributedText.length !=
       0);  // We're an accessibility element by default if there is a string.
...
- (NSObject *)drawParametersForAsyncLayer:(_ASDisplayLayer *)layer {
  ...
  return
      [[ASTextNodeDrawParameter alloc] initWithRendererAttributes:[self _locked_rendererAttributes]
...
+ (void)drawRect:(CGRect)bounds
  ...
  UIColor *backgroundColor =
      (isRasterizing || drawParameter == nil) ? nil : drawParameter->_backgroundColor;
  UIEdgeInsets textContainerInsets =
      drawParameter ? drawParameter->_textContainerInsets : UIEdgeInsetsZero;
...
- (id)_linkAttributeValueAtPoint:(CGPoint)point
  ...
  [renderer
      enumerateTextIndexesAtPosition:point
                          usingBlock:^(NSUInteger characterIndex, CGRect glyphBoundingRect,
                                       BOOL *stop) {
                            CGPoint glyphLocation = CGPointMake(CGRectGetMidX(glyphBoundingRect),
                                                                CGRectGetMidY(glyphBoundingRect));
                            CGFloat currentDistance =
                                std::sqrt(std::pow(point.x - glyphLocation.x, 2.f) +
                            ...
                            // Check if it's outside the visible range, if so, then we mark this
                            // touch as inside the truncation message, because in at least one of
                            // the touch points it was.
                            ...
                            // truncation message may be beyond the scope of the actual input
                            // string, we have to make sure that we don't start asking for
                            // attributes on it.
                            ...
                            for (NSString *attributeName in self->_linkAttributeNames) {
                              ...
                              // If highlighting, check with delegate first. If not implemented,
                              // assume YES.
                              if (highlighting &&
                                  [self->_delegate
                              ...
                              if (value != nil || name != nil) {
                                ...
                                if (rangeOut != NULL && value != nil) {
                                  *rangeOut = range;
                                  // Limit to only the visible range, because the attributed string
                                  // will return values outside the visible range.
                                  if (NSMaxRange(*rangeOut) > NSMaxRange(visibleRange)) {
                                    (*rangeOut).length =
                                        MAX(NSMaxRange(visibleRange) - (*rangeOut).location, 0);
                                  }
                                }
...
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gestureRecognizer {
  ...
  if (gestureRecognizer == _longPressGestureRecognizer) {
    ...
    if ([self _pendingLinkTap] &&
        [_delegate respondsToSelector:@selector(textNode:
  ...
  if (!NSEqualRanges(highlightRange, _highlightRange) &&
    ...
    if (_activeHighlightLayer) {
      if (animated) {
        ...
        CABasicAnimation *possibleFadeIn =
            (CABasicAnimation *)[weakHighlightLayer animationForKey:@"opacity"];
        if (possibleFadeIn) {
        ...
        fadeOut.timingFunction =
            [CAMediaTimingFunction functionWithName:kCAMediaTimingFunctionEaseInEaseOut];
        fadeOut.fromValue =
            possibleFadeIn.toValue ?: @(((CALayer *)weakHighlightLayer.presentationLayer).opacity);
        fadeOut.toValue = @0.0;
    ...
    if (0 != highlightRange.length) {
      ...
      if (highlightTargetLayer != nil) {
        ...
        for (NSValue *rectValue in highlightRects) {
          ...
          CGRect rendererRect =
              ASTextNodeAdjustRenderRectForShadowPadding(rectValue.CGRectValue, shadowPadding);
          ...
          CGRect highlightedRect = [self.layer convertRect:rendererRect
                                                   toLayer:highlightTargetLayer];
        ...
        ASHighlightOverlayLayer *overlayLayer =
            [[ASHighlightOverlayLayer alloc] initWithRects:converted];
...
+ (CGColorRef)_highlightColorForStyle:(ASTextNodeHighlightStyle)style {
  return [UIColor colorWithWhite:(style == ASTextNodeHighlightStyleLight ? 0.0 : 1.0) alpha:1.0]
      .CGColor;
}

+ (CGFloat)_highlightOpacityForStyle:(ASTextNodeHighlightStyle)style {
  return (style == ASTextNodeHighlightStyleLight) ? ASTextNodeHighlightLightOpacity
                                                  : ASTextNodeHighlightDarkOpacity;
}

#pragma mark - Text rects

static CGRect ASTextNodeAdjustRenderRectForShadowPadding(CGRect rendererRect,
...
- (NSArray *)_rectsForTextRange:(NSRange)textRange
                  measureOption:(ASTextKitRendererMeasureOption)measureOption {
  ASLockScopeSelf();

  NSArray *rects = [[self _locked_renderer] rectsForTextRange:textRange
...
- (UIImage *)placeholderImage {
  ...
  NSArray *lineRects = [self _rectsForTextRange:visibleRange
...
- (BOOL)pointInside:(CGPoint)point withEvent:(UIEvent *)event {
  ...
  BOOL linkCrossesVisibleRange =
      (lastCharIndex > range.location) && (lastCharIndex < NSMaxRange(range) - 1);

  if (range.length > 0 && !linkCrossesVisibleRange && linkAttributeValue != nil &&
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
  ...
  if (inAdditionalTruncationMessage) {
    ...
    NSRange truncationMessageRange =
        [self _additionalTruncationMessageRangeWithVisibleRange:visibleRange];
    [self _setHighlightRange:truncationMessageRange
            forAttributeName:ASTextNodeTruncationTokenAttributeName
                       value:nil
                    animated:YES];
  } else if (range.length > 0 && !linkCrossesVisibleRange && linkAttributeValue != nil &&
             linkAttributeName != nil) {
    [self _setHighlightRange:range
            forAttributeName:linkAttributeName
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
  ...
  // on 3D Touch enabled phones, this gets fired with changes in force, and usually will get fired
  // immediately after touchesBegan:withEvent:
...
- (void)_handleLongPress:(UILongPressGestureRecognizer *)longPressRecognizer {
  ...
  if (longPressRecognizer.state == UIGestureRecognizerStateBegan) {
    if ([self _pendingLinkTap] &&
        [_delegate respondsToSelector:@selector(textNode:
                                          longPressedLinkAttribute:value:atPoint:textRange:)]) {
...
static NSAttributedString *DefaultTruncationAttributedString() {
  ...
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    defaultTruncationAttributedString = [[NSAttributedString alloc]
        initWithString:NSLocalizedString(@"\u2026", @"Default truncation string")];
  });
...
- (BOOL)shouldTruncateForConstrainedSize:(ASSizeRange)constrainedSize {
  return ASLockedSelf(
      [[self _locked_rendererWithBounds:{.size = constrainedSize.max}] isTruncated]);
}
...
- (NSRange)_additionalTruncationMessageRangeWithVisibleRange:(NSRange)visibleRange {
  ...
  return NSMakeRange(truncationTokenIndex + _truncationAttributedText.length + 1,
                     additionalTruncationMessageLength);
}
...
- (NSAttributedString *)_locked_composedTruncationText {
  ...
  if (_composedTruncationText == nil) {
    ...
    _composedTruncationText =
        [self _locked_prepareTruncationStringForDrawing:_composedTruncationText];
  }
 ...
 * of the body text
 */
- (NSAttributedString *)_locked_prepareTruncationStringForDrawing:
  ...
  if (_attributedText.length > 0) {
    ...
    NSDictionary *originalStringAttributes =
        [originalString attributesAtIndex:originalStringLength - 1 effectiveRange:NULL];
    [truncationString
        enumerateAttributesInRange:NSMakeRange(0, truncationString.length)
                           options:0
                        usingBlock:^(NSDictionary *attributes, NSRange range, BOOL *stop) {
                          ...
                          [truncationMutableString setAttributes:futureTruncationAttributes
                                                           range:range];
                        }];
  }
...
+ (void)_registerAttributedText:(NSAttributedString *)str {
  ...
  if (array.count == 2000) {
    NSString *path =
        [NSTemporaryDirectory() stringByAppendingPathComponent:@"AttributedStrings.plist"];
