...
- (void)didEnterPreloadState {
  ...
  // If we enter preload state we apply the pending asset to load to the video node so it can start
  // and fetch the asset
...
- (void)_locked_createPlaybackButton {
  ...
  if (_playbackButtonNode == nil) {
    ...
    [_cachedControls setObject:_playbackButtonNode
                        forKey:@(ASVideoPlayerNodeControlTypePlaybackButton)];
  }
...
- (void)_locked_createFullScreenButton {
  ...
  if (_fullScreenButtonNode == nil) {
    ...
    [_fullScreenButtonNode addTarget:self
                    ...
                    forControlEvents:ASControlNodeEventTouchUpInside];
    [_cachedControls setObject:_fullScreenButtonNode
                        forKey:@(ASVideoPlayerNodeControlTypeFullScreenButton)];
  }
...
- (void)_locked_createElapsedTextField {
  ...
  if (_elapsedTextNode == nil) {
    ...
    _elapsedTextNode.attributedText =
        [self timeLabelAttributedStringForString:@"00:00"
...
- (void)_locked_createDurationTextField {
  ...
  if (_durationTextNode == nil) {
    ...
    [_cachedControls setObject:_durationTextNode
                        forKey:@(ASVideoPlayerNodeControlTypeDurationText)];
  }
    ...
    _scrubberNode = [[ASDisplayNode alloc] initWithViewBlock:^UIView *_Nonnull {
      ...
      if (_delegateFlags.delegateScrubberMinimumTrackTintColor) {
        slider.minimumTrackTintColor =
            [strongSelf.delegate videoPlayerNodeScrubberMinimumTrackTint:strongSelf];
      }

      if (_delegateFlags.delegateScrubberMaximumTrackTintColor) {
        slider.maximumTrackTintColor =
            [strongSelf.delegate videoPlayerNodeScrubberMaximumTrackTint:strongSelf];
      }
      ...
      [slider addTarget:strongSelf
                    action:@selector(seekTimeDidChange:)
...
- (void)_locked_createControlFlexGrowSpacer {
  ...
  [_cachedControls setObject:_controlFlexGrowSpacerSpec
                      forKey:@(ASVideoPlayerNodeControlTypeFlexGrowSpacer)];
}
...
- (void)updateDurationTimeLabel {
  ...
  _durationTextNode.attributedText =
      [self timeLabelAttributedStringForString:formattedDuration
                                forControlType:ASVideoPlayerNodeControlTypeDurationText];
}
...
- (void)updateElapsedTimeLabel:(NSTimeInterval)seconds {
  ...
  NSString *formattedElapsed = [self
      timeStringForCMTime:CMTimeMakeWithSeconds(seconds, _videoNode.periodicTimeObserverTimescale)
         forTimeLabelType:ASVideoPlayerNodeControlTypeElapsedText];
  _elapsedTextNode.attributedText =
      [self timeLabelAttributedStringForString:formattedElapsed
                                forControlType:ASVideoPlayerNodeControlTypeElapsedText];
}

- (NSAttributedString *)timeLabelAttributedStringForString:(NSString *)string
                                            forControlType:
                                                (ASVideoPlayerNodeControlType)controlType {
...
- (void)videoNode:(ASVideoNode *)videoNode didPlayToTimeInterval:(NSTimeInterval)timeInterval {
  ...
  if (_scrubberNode) {
    [(UISlider *)_scrubberNode.view setValue:(timeInterval / CMTimeGetSeconds(_duration))
                                    animated:NO];
  }
}
  ...
  if (!_spinnerNode) {
    ...
    _spinnerNode = [[ASDisplayNode alloc] initWithViewBlock:^UIView * {
      ...
      if (_delegateFlags.delegateSpinnerStyle) {
        spinnnerView.activityIndicatorViewStyle =
            [_delegate videoPlayerNodeSpinnerStyle:strongSelf];
      }
...
- (void)seekToTime:(CGFloat)percentComplete {
  ...
  [_videoNode.player
      seekToTime:CMTimeMakeWithSeconds(seconds, _videoNode.periodicTimeObserverTimescale)];
...
- (ASLayoutSpec *)layoutSpecThatFits:(ASSizeRange)constrainedSize {
  ...
  if (_delegateFlags.delegateLayoutSpecForControls) {
    layoutSpec = [_delegate videoPlayerNodeLayoutSpec:self
                                          forControls:_cachedControls
                                       forMaximumSize:maxSize];
  } else {
  ...
  ASOverlayLayoutSpec *overlaySpec = [ASOverlayLayoutSpec overlayLayoutSpecWithChild:_videoNode
                                                                             overlay:layoutSpec];
...
- (ASLayoutSpec *)defaultLayoutSpecThatFits:(CGSize)maxSize {
  ...
  ASStackLayoutSpec *controlbarSpec =
  ...
  ASInsetLayoutSpec *controlbarInsetSpec =
      [ASInsetLayoutSpec insetLayoutSpecWithInsets:insets child:controlbarSpec];
...
- (void)setDelegate:(id<ASVideoPlayerNodeDelegate>)delegate {
  ...
  if (_delegate == nil) {
    ...
    _delegateFlags.delegateCustomControls =
        [_delegate respondsToSelector:@selector(videoPlayerNodeCustomControls:)];
    _delegateFlags.delegateSpinnerTintColor =
        [_delegate respondsToSelector:@selector(videoPlayerNodeSpinnerTint:)];
    _delegateFlags.delegateSpinnerStyle =
        [_delegate respondsToSelector:@selector(videoPlayerNodeSpinnerStyle:)];
    ...
    _delegateFlags.delegateLayoutSpecForControls = [_delegate
        respondsToSelector:@selector(videoPlayerNodeLayoutSpec:forControls:forMaximumSize:)];
    _delegateFlags.delegateVideoNodeDidPlayToTime =
        [_delegate respondsToSelector:@selector(videoPlayerNode:didPlayToTime:)];
    _delegateFlags.delegateVideoNodeWillChangeState = [_delegate
        respondsToSelector:@selector(videoPlayerNode:willChangeVideoNodeState:toVideoNodeState:)];
    ...
    _delegateFlags.delegateTimeLabelAttributedString = [_delegate
        respondsToSelector:@selector(videoPlayerNode:timeStringForTimeLabelType:forTime:)];
    ...
    _delegateFlags.delegateDidTapVideoPlayerNode =
        [_delegate respondsToSelector:@selector(didTapVideoPlayerNode:)];
...
- (NSString *)timeStringForCMTime:(CMTime)time forTimeLabelType:(ASVideoPlayerNodeControlType)type {
  ...
  if (dHours > 0) {
    videoDurationText =
        [NSString stringWithFormat:@"%i:%02i:%02i", (int)dHours, (int)dMinutes, (int)dSeconds];
  } else {
