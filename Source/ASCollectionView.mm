...
@interface ASCollectionView () <ASRangeControllerDataSource,
  ...
  /**
   * If YES, the `UICollectionView` will reload its data on next layout pass so we should not
   forward any updates to it.

   * Rationale:
   * In `reloadData`, a collection view invalidates its data and marks itself as needing reload, and
   waits until `layoutSubviews` to requery its data source.
   * This can lead to data inconsistency problems.
   * Say you have an empty collection view. You call `reloadData`, then immediately insert an item
   into your data source and call `insertItemsAtIndexPaths:[0,0]`.
   * You will get an assertion failure saying `Invalid number of items in section 0.
   * The number of items after the update (1) must be equal to the number of items before the update
   (1) plus or minus the items added and removed (1 added, 0 removed).`
   * The collection view never queried your data source before the update to see that it actually
   had 0 items.
   */
   ...
   * (0 sections) we always check at least once after each update (initial reload is the first
   * update.)
   */
...
- (instancetype)_initWithFrame:(CGRect)frame
  ...
  _dataController = [[ASDataController alloc] initWithDataSource:self
                                                            node:owningNode
                                                        eventLog:eventLog];
...
- (void)dealloc {
  ...
  ASDisplayNodeCAssert(_batchUpdateCount == 0,
                       @"ASCollectionView deallocated in the middle of a batch update.");

  // Sometimes the UIKit classes can call back to their delegate even during deallocation, due to
  // animation completion blocks etc.
  _isDeallocating = YES;
...
- (void)reloadData {
  ...
  // UICollectionView calls -reloadData during first layoutSubviews and when the data source
  // changes. This fires off the first load of cell nodes.
...
- (void)waitUntilAllUpdatesAreCommitted {
  ...
  if (_batchUpdateCount > 0) {
    ...
    //    ASDisplayNodeFailAssert(@"Should not call %@ during batch update",
...
- (void)setDataSource:(id<UICollectionViewDataSource>)dataSource {
  // UIKit can internally generate a call to this method upon changing the asyncDataSource; only
  // assert for non-nil. We also allow this when we're doing interop.
  ASDisplayNodeAssert(
      _asyncDelegateFlags.interop || dataSource == nil,
      @"ASCollectionView uses asyncDataSource, not UICollectionView's dataSource property.");
}

- (void)setDelegate:(id<UICollectionViewDelegate>)delegate {
  // Our UIScrollView superclass sets its delegate to nil on dealloc. Only assert if we get a
  // non-nil value here. We also allow this when we're doing interop.
  ASDisplayNodeAssert(
      _asyncDelegateFlags.interop || delegate == nil,
...
- (void)setAsyncDataSource:(id<ASCollectionDataSource>)asyncDataSource {
  // Changing super.dataSource will trigger a setNeedsLayout, so this must happen on the main
  // thread.
  ASDisplayNodeAssertMainThread();

  // Note: It's common to check if the value hasn't changed and short-circuit but we aren't doing
  // that here to handle the (common) case of nilling the asyncDataSource in the ViewController's
  // dealloc. In this case our _asyncDataSource will return as nil (ARC magic) even though the
  // _proxyDataSource still exists. It's really important to hold a strong reference to the old
  // dataSource in this case because calls to ASCollectionViewProxy will start failing and cause
  // crashes.
  ...
  if (asyncDataSource == nil) {
    ...
    _proxyDataSource =
        _isDeallocating ? nil : [[ASCollectionViewProxy alloc] initWithTarget:nil interceptor:self];
    _asyncDataSourceFlags = {};

  } else {
    _asyncDataSource = asyncDataSource;
    _proxyDataSource = [[ASCollectionViewProxy alloc] initWithTarget:_asyncDataSource
                                                         interceptor:self];
    ...
    _asyncDataSourceFlags.collectionViewNodeBlockForItem = [_asyncDataSource
        respondsToSelector:@selector(collectionView:nodeBlockForItemAtIndexPath:)];
    ...
    _asyncDataSourceFlags.collectionViewNodeForSupplementaryElement =
        [_asyncDataSource respondsToSelector:@selector(collectionView:
    ...
    _asyncDataSourceFlags.collectionNodeNodeBlockForItem = [_asyncDataSource
        respondsToSelector:@selector(collectionNode:nodeBlockForItemAtIndexPath:)];
    ...
    _asyncDataSourceFlags.collectionNodeNodeForSupplementaryElement =
        [_asyncDataSource respondsToSelector:@selector(collectionNode:
                                                 nodeForSupplementaryElementOfKind:atIndexPath:)];
    _asyncDataSourceFlags.collectionNodeNodeBlockForSupplementaryElement = [_asyncDataSource
        respondsToSelector:@selector(collectionNode:
                               nodeBlockForSupplementaryElementOfKind:atIndexPath:)];
    _asyncDataSourceFlags.collectionNodeSupplementaryElementKindsInSection = [_asyncDataSource
        respondsToSelector:@selector(collectionNode:supplementaryElementKindsInSection:)];
    _asyncDataSourceFlags.nodeModelForItem = [_asyncDataSource
        respondsToSelector:@selector(collectionNode:nodeModelForItemAtIndexPath:)];
    _asyncDataSourceFlags.collectionNodeCanMoveItem =
        [_asyncDataSource respondsToSelector:@selector(collectionNode:canMoveItemWithNode:)];
    _asyncDataSourceFlags.collectionNodeMoveItem = [_asyncDataSource
        respondsToSelector:@selector(collectionNode:moveItemAtIndexPath:toIndexPath:)];

    _asyncDataSourceFlags.interop =
        [_asyncDataSource conformsToProtocol:@protocol(ASCollectionDataSourceInterop)];
    if (_asyncDataSourceFlags.interop) {
      id<ASCollectionDataSourceInterop> interopDataSource =
          (id<ASCollectionDataSourceInterop>)_asyncDataSource;
      _asyncDataSourceFlags.interopAlwaysDequeue =
          [[interopDataSource class]
              respondsToSelector:@selector(dequeuesCellsForNodeBackedItems)] &&
    ...
    ASDisplayNodeAssert(_asyncDataSourceFlags.collectionNodeNodeBlockForItem ||
                            ...
                            _asyncDataSourceFlags.collectionViewNodeForItem,
                        @"Data source must implement collectionNode:nodeBlockForItemAtIndexPath: "
                        @"or collectionNode:nodeForItemAtIndexPath:");
  }

  _dataController.validationErrorSource = asyncDataSource;
  super.dataSource = (id<UICollectionViewDataSource>)_proxyDataSource;

  // Cache results of layoutInspector to ensure flags are up to date if getter lazily loads a new
  // one.
...
- (void)setAsyncDelegate:(id<ASCollectionDelegate>)asyncDelegate {
  ...
  // Note: It's common to check if the value hasn't changed and short-circuit but we aren't doing
  // that here to handle the (common) case of nilling the asyncDelegate in the ViewController's
  // dealloc. In this case our _asyncDelegate will return as nil (ARC magic) even though the
  // _proxyDataSource still exists. It's really important to hold a strong reference to the old
  // delegate in this case because calls to ASCollectionViewProxy will start failing and cause
  // crashes.
  ...
  if (asyncDelegate == nil) {
    ...
    _asyncDelegateFlags.scrollViewDidScroll =
        [_asyncDelegate respondsToSelector:@selector(scrollViewDidScroll:)];
    _asyncDelegateFlags.scrollViewWillEndDragging = [_asyncDelegate
    ...
    _asyncDelegateFlags.collectionViewWillDisplayNodeForItem = [_asyncDelegate
        respondsToSelector:@selector(collectionView:willDisplayNode:forItemAtIndexPath:)];
    if (_asyncDelegateFlags.collectionViewWillDisplayNodeForItem == NO) {
      _asyncDelegateFlags.collectionViewWillDisplayNodeForItemDeprecated = [_asyncDelegate
          respondsToSelector:@selector(collectionView:willDisplayNodeForItemAtIndexPath:)];
    }
    _asyncDelegateFlags.collectionViewDidEndDisplayingNodeForItem = [_asyncDelegate
        respondsToSelector:@selector(collectionView:didEndDisplayingNode:forItemAtIndexPath:)];
    _asyncDelegateFlags.collectionViewWillBeginBatchFetch = [_asyncDelegate
        respondsToSelector:@selector(collectionView:willBeginBatchFetchWithContext:)];
    ...
    _asyncDelegateFlags.collectionViewShouldDeselectItem = [_asyncDelegate
        respondsToSelector:@selector(collectionView:shouldDeselectItemAtIndexPath:)];
    ...
    _asyncDelegateFlags.collectionViewShouldHighlightItem = [_asyncDelegate
        respondsToSelector:@selector(collectionView:shouldHighlightItemAtIndexPath:)];
    ...
    _asyncDelegateFlags.collectionViewDidUnhighlightItem = [_asyncDelegate
        respondsToSelector:@selector(collectionView:didUnhighlightItemAtIndexPath:)];
    _asyncDelegateFlags.collectionViewShouldShowMenuForItem = [_asyncDelegate
        respondsToSelector:@selector(collectionView:shouldShowMenuForItemAtIndexPath:)];
    _asyncDelegateFlags.collectionViewCanPerformActionForItem = [_asyncDelegate
        respondsToSelector:@selector(collectionView:
                                   canPerformAction:forItemAtIndexPath:withSender:)];
    _asyncDelegateFlags.collectionViewPerformActionForItem = [_asyncDelegate
    ...
    _asyncDelegateFlags.collectionNodeWillBeginBatchFetch = [_asyncDelegate
        respondsToSelector:@selector(collectionNode:willBeginBatchFetchWithContext:)];
    ...
    _asyncDelegateFlags.collectionNodeShouldDeselectItem = [_asyncDelegate
        respondsToSelector:@selector(collectionNode:shouldDeselectItemAtIndexPath:)];
    ...
    _asyncDelegateFlags.collectionNodeShouldHighlightItem = [_asyncDelegate
        respondsToSelector:@selector(collectionNode:shouldHighlightItemAtIndexPath:)];
    ...
    _asyncDelegateFlags.collectionNodeDidUnhighlightItem = [_asyncDelegate
        respondsToSelector:@selector(collectionNode:didUnhighlightItemAtIndexPath:)];
    _asyncDelegateFlags.collectionNodeShouldShowMenuForItem = [_asyncDelegate
        respondsToSelector:@selector(collectionNode:shouldShowMenuForItemAtIndexPath:)];
    _asyncDelegateFlags.collectionNodeCanPerformActionForItem = [_asyncDelegate
        respondsToSelector:@selector(collectionNode:canPerformAction:forItemAtIndexPath:sender:)];
    _asyncDelegateFlags.collectionNodePerformActionForItem = [_asyncDelegate
        respondsToSelector:@selector(collectionNode:performAction:forItemAtIndexPath:sender:)];
    _asyncDelegateFlags.collectionNodeWillDisplaySupplementaryElement = [_asyncDelegate
        respondsToSelector:@selector(collectionNode:willDisplaySupplementaryElementWithNode:)];
    _asyncDelegateFlags.collectionNodeDidEndDisplayingSupplementaryElement = [_asyncDelegate
        respondsToSelector:@selector(collectionNode:didEndDisplayingSupplementaryElementWithNode:)];
    _asyncDelegateFlags.interop =
        [_asyncDelegate conformsToProtocol:@protocol(ASCollectionDelegateInterop)];
    if (_asyncDelegateFlags.interop) {
      id<ASCollectionDelegateInterop> interopDelegate =
          (id<ASCollectionDelegateInterop>)_asyncDelegate;
      _asyncDelegateFlags.interopWillDisplayCell = [interopDelegate
          respondsToSelector:@selector(collectionView:willDisplayCell:forItemAtIndexPath:)];
      _asyncDelegateFlags.interopDidEndDisplayingCell = [interopDelegate
          respondsToSelector:@selector(collectionView:didEndDisplayingCell:forItemAtIndexPath:)];
      _asyncDelegateFlags.interopWillDisplaySupplementaryView = [interopDelegate
          respondsToSelector:@selector(collectionView:
                                 willDisplaySupplementaryView:forElementKind:atIndexPath:)];
      _asyncDelegateFlags.interopdidEndDisplayingSupplementaryView = [interopDelegate
          respondsToSelector:@selector(collectionView:
                                 didEndDisplayingSupplementaryView:forElementOfKind:atIndexPath:)];
    }
  }
...
- (void)_asyncDelegateOrDataSourceDidChange {
  ...
  if (_asyncDataSource == nil && _asyncDelegate == nil &&
      !ASActivateExperimentalFeature(ASExperimentalSkipClearData)) {
...
- (void)setLayoutInspector:(id<ASCollectionViewLayoutInspecting>)layoutInspector {
  ...
  _layoutInspectorFlags.constrainedSizeForSupplementaryNodeOfKindAtIndexPath = [_layoutInspector
      respondsToSelector:@selector(collectionView:
                             constrainedSizeForSupplementaryNodeOfKind:atIndexPath:)];
  _layoutInspectorFlags.supplementaryNodesOfKindInSection = [_layoutInspector
      respondsToSelector:@selector(collectionView:supplementaryNodesOfKind:inSection:)];
...
- (void)setTuningParameters:(ASRangeTuningParameters)tuningParameters
  ...
  [_rangeController setTuningParameters:tuningParameters
                           forRangeMode:ASLayoutRangeModeFull
                              rangeType:rangeType];
}
...
- (CGSize)sizeForElement:(ASCollectionElement *)element {
  ...
  if (useUIKitCell) {
    ...
    if (wrapperNode.sizeForItemBlock) {
      ...
      // In this case, it is possible the model indexPath for this element will be nil. Attempt to
      // convert it, and call out to the delegate directly. If it has been deleted from the model,
      // the size returned will be the layout's default.
      NSIndexPath *indexPath = [_dataController.visibleMap indexPathForElement:element];
      return [self _sizeForUIKitCellWithKind:element.supplementaryElementKind
...
- (NSIndexPath *)convertIndexPathFromCollectionNode:(NSIndexPath *)indexPath
  ...
  NSIndexPath *viewIndexPath =
      [_dataController.visibleMap convertIndexPath:indexPath fromMap:_dataController.pendingMap];
...
- (nullable NSIndexPath *)validateIndexPath:(nullable NSIndexPath *)indexPath {
  ...
  if (section >= self.numberOfSections) {
    ASDisplayNodeFailAssert(
        @"Collection view index path has invalid section %lu, section count = %lu",
  ...
  if (item != NSNotFound && item >= [self numberOfItemsInSection:section]) {
    ASDisplayNodeFailAssert(
        @"Collection view index path has invalid item %lu in section %lu, item count = %lu",
...
- (ASCellNode *)supplementaryNodeForElementKind:(NSString *)elementKind
                                    atIndexPath:(NSIndexPath *)indexPath {
  return [_dataController.visibleMap supplementaryElementOfKind:elementKind atIndexPath:indexPath]
      .node;
}
...
- (void)_configureCollectionViewLayout:(nonnull UICollectionViewLayout *)layout {
  _hasDataControllerLayoutDelegate =
      [layout conformsToProtocol:@protocol(ASDataControllerLayoutDelegate)];
...
/**
 This method is called only for UIKit Passthrough cells - either regular Items or Supplementary
 elements. It checks if the delegate implements the UICollectionViewFlowLayout methods that provide
 sizes, and if not, uses the default values set on the flow layout. If a flow layout is not in use,
 UICollectionView Passthrough cells must be sized by logic in the Layout object, and Texture does
 not participate in these paths.
*/
- (CGSize)_sizeForUIKitCellWithKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath {
  ...
  if (kind == nil) {
    ...
    if (indexPath && [_asyncDelegate respondsToSelector:sizeForHeader]) {
      size = [(id)_asyncDelegate collectionView:self
                                         layout:l
 ...
 */
- (void)_superPerformBatchUpdates:(void (^)())updates
...
- (void)beginUpdates {
  ...
  if (_batchUpdateCount == 0) {
    _changeSet =
        [[_ASHierarchyChangeSet alloc] initWithOldData:[_dataController itemCountsFromDataSource]];
    _changeSet.rootActivity = as_activity_create("Perform async collection update",
                                                 AS_ACTIVITY_CURRENT, OS_ACTIVITY_FLAG_DEFAULT);
    _changeSet.submitActivity = as_activity_create(
        "Submit changes for collection update", _changeSet.rootActivity, OS_ACTIVITY_FLAG_DEFAULT);
  }
  _batchUpdateCount++;
}
...
- (void)endUpdatesAnimated:(BOOL)animated completion:(nullable void (^)(BOOL))completion {
  ...
  if (_batchUpdateCount == 0) {
    ...
    // Nil out _changeSet before forwarding to _dataController to allow the change set to cause
    // subsequent batch updates on the same run loop
    _changeSet = nil;
...
- (void)performBatchUpdates:(NS_NOESCAPE void (^)())updates completion:(void (^)(BOOL))completion {
  // We capture the current state of whether animations are enabled if they don't provide us with
  // one.
...
- (void)moveSection:(NSInteger)section toSection:(NSInteger)newSection {
  ...
  [self
      performBatchUpdates:^{
        [_changeSet moveSection:section
...
- (CGFloat)collectionView:(UICollectionView *)cv
  ...
  section = [self delegateIndexForSection:section withSelector:_cmd];
  if (section != NSNotFound) {
    return [(id)_asyncDelegate collectionView:cv
                                          layout:l
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView
  ...
  ASCollectionElement *element = [_dataController.visibleMap supplementaryElementOfKind:kind
                                                                            atIndexPath:indexPath];
  ASCellNode *node = element.node;
  ...
  if (_ASCollectionReusableView *reusableView =
          ASDynamicCastStrict(view, _ASCollectionReusableView)) {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView
  ...
  ASDisplayNodeAssert(element != nil,
                      @"Element should exist. indexPath = %@, collectionDataSource = %@", indexPath,
                      self);
  ASDisplayNodeAssert(
      cell != nil,
      @"UICollectionViewCell must not be nil. indexPath = %@, collectionDataSource = %@", indexPath,
      self);
...
- (void)collectionView:(UICollectionView *)collectionView
  ...
  if (element) {
    ASDisplayNodeAssertTrue([_dataController.visibleMap elementForItemAtIndexPath:indexPath] ==
                            element);
    [_visibleElements addObject:element];
  } else {
    ASDisplayNodeAssert(NO, @"Unexpected nil element for willDisplayCell: %@, %@, %@", rawCell,
                        self, indexPath);
    return;
  }
  ...
  // Update the selected background view in collectionView:willDisplayCell:forItemAtIndexPath:
  // otherwise it could be too early e.g. if the selectedBackgroundView was set in didLoad()
  ...
  ASDisplayNodeAssertNotNil(
      cellNode,
      @"Expected node associated with cell that will be displayed not to be nil. indexPath: %@",
      indexPath);
...
- (void)collectionView:(UICollectionView *)collectionView
  ...
  // Retrieve the element from cell instead of visible map because at this point visible map could
  // have been updated and no longer holds the element.
  ASCollectionElement *element = cell.element;
  if (element) {
    [_visibleElements removeObject:element];
  } else {
    ASDisplayNodeAssert(NO, @"Unexpected nil element for didEndDisplayingCell: %@, %@, %@", rawCell,
                        self, indexPath);
    return;
  }
...
- (void)collectionView:(UICollectionView *)collectionView
  ...
  if (element) {
    ASDisplayNodeAssertTrue([_dataController.visibleMap
                                supplementaryElementOfKind:elementKind
                                               atIndexPath:indexPath] == view.element);
    [_visibleElements addObject:element];
  } else {
    ASDisplayNodeAssert(NO, @"Unexpected nil element for willDisplaySupplementaryView: %@, %@, %@",
                        rawView, self, indexPath);
    return;
  }
  ...
  if (view.layoutAttributes == nil) {
    view.layoutAttributes =
        [collectionView layoutAttributesForSupplementaryElementOfKind:elementKind
                                                          atIndexPath:indexPath];
  }
  ...
  if (_asyncDelegateFlags.collectionNodeWillDisplaySupplementaryElement) {
    ...
    ASDisplayNodeAssert([node.supplementaryElementKind isEqualToString:elementKind],
                        ...
                        elementKind, node.supplementaryElementKind);
...
- (void)collectionView:(UICollectionView *)collectionView
  ...
  if (element) {
    ...
    ASDisplayNodeAssert(NO,
                        @"Unexpected nil element for didEndDisplayingSupplementaryView: %@, %@, %@",
                        rawView, self, indexPath);
    return;
  }
  ...
  if (_asyncDelegateFlags.collectionNodeDidEndDisplayingSupplementaryElement) {
    ...
    ASDisplayNodeAssert([node.supplementaryElementKind isEqualToString:elementKind],
                        @"Expected node for supplementary element to have kind '%@', got '%@'.",
                        elementKind, node.supplementaryElementKind);
    [_asyncDelegate collectionNode:collectionNode
        didEndDisplayingSupplementaryElementWithNode:node];
  }
}
...
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
    return [_asyncDelegate collectionView:self
                         canPerformAction:action
                       forItemAtIndexPath:indexPath
                               withSender:sender];
#pragma clang diagnostic pop
...
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
    [_asyncDelegate collectionView:self
                     performAction:action
                forItemAtIndexPath:indexPath
                        withSender:sender];
#pragma clang diagnostic pop
...
- (BOOL)collectionView:(UICollectionView *)collectionView
  ...
  // Currently we do not support interactive moves when using async layout. The reason is, we do not
  // have a mechanism to propagate the "presentation data" element map (containing the speculative
  // in-progress moves) to the layout delegate, and this can cause exceptions to be thrown from
  // UICV. For example, if you drag an item out of a section, the element map will still contain N
  // items in that section, even though there's only N-1 shown, and UICV will throw an exception
  // that you specified an element that doesn't exist.
  //
  // In iOS >= 11, this is made much easier by the UIDataSourceTranslating API. In previous versions
  // of iOS our best bet would be to capture the invalidation contexts that are sent during
  // interactive moves and make our own data source translator.
  if ([self.collectionViewLayout isKindOfClass:[ASCollectionLayout class]]) {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
      as_log_debug(ASCollectionLog(),
                   "Collection node item interactive movement is not supported when using a layout "
                   "delegate. This message will only be logged once. Node: %@",
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
  ...
  for (_ASCollectionViewCell *cell in _cellsForVisibilityUpdates) {
    // _cellsForVisibilityUpdates only includes cells for ASCellNode subclasses with overrides of
    // the visibility method.
    [cell cellNodeVisibilityEvent:ASCellNodeVisibilityEventVisibleRectChanged
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView
  ...
  _deceleratingVelocity =
      CGPointMake(contentOffset.x - ((targetContentOffset != NULL) ? targetContentOffset->x : 0),
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView {
  ...
  for (_ASCollectionViewCell *cell in _cellsForVisibilityUpdates) {
    [cell cellNodeVisibilityEvent:ASCellNodeVisibilityEventWillBeginDragging
                     inScrollView:scrollView];
  }
...
- (ASScrollDirection)_scrollDirectionForVelocity:(CGPoint)scrollVelocity {
  ...
  if (ASScrollDirectionContainsHorizontalDirection(
          scrollableDirections)) {  // Can scroll horizontally.
...
- (void)layoutSubviews {
  ...
  // To ensure _maxSizeForNodesConstrainedSize is up-to-date for every usage, this call to super
  // must be done last
...
- (void)_scheduleCheckForBatchFetchingForNumberOfChanges:(NSUInteger)changes {
  // Prevent fetching will continually trigger in a loop after reaching end of content and no new
...
- (void)_beginBatchFetchingIfNeededWithContentOffset:(CGPoint)contentOffset
  ...
  if (ASDisplayShouldFetchBatchForScrollView(self, self.scrollDirection, self.scrollableDirections,
...
- (void)_beginBatchFetching {
  ...
  if (_asyncDelegateFlags.collectionNodeWillBeginBatchFetch) {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
      GET_COLLECTIONNODE_OR_RETURN(collectionNode, (void)0);
      as_log_debug(ASCollectionLog(), "Beginning batch fetch for %@ with context %@",
                   collectionNode, _batchContext);
...
- (BOOL)dataController:(ASDataController *)dataController
    shouldEagerlyLayoutNode:(ASCellNode *)node {
...
- (id)dataController:(ASDataController *)dataController
    nodeModelForItemAtIndexPath:(NSIndexPath *)indexPath {
...
#pragma clang diagnostic pop
  ...
  if (block == nil) {
    if (cell == nil || ASDynamicCast(cell, ASCellNode) == nil) {
      // In this case, either the client is expecting a UIKit passthrough cell to be created
      // automatically, or it is an error.
      if (_asyncDataSourceFlags.interop) {
        cell = [[ASWrapperCellNode alloc] init];
      } else {
        ASDisplayNodeFailAssert(
            @"ASCollection could not get a node block for item at index path %@: %@, %@. If you "
    ...
    // This condition is intended to run for either cells received from the datasource, or created
    // just above.
...
- (BOOL)dataController:(ASDataController *)dataController
  ...
  if (indexPath == nil) {
    ASDisplayNodeFailAssert(
        @"Data controller should not ask for presented size for element that is not presented.");
    return YES;
  }
...
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
    cell = [_asyncDataSource collectionView:self
          nodeForSupplementaryElementOfKind:kind
                                atIndexPath:indexPath];
#pragma clang diagnostic pop
  }
  ...
  if (block == nil) {
    if (cell == nil || ASDynamicCast(cell, ASCellNode) == nil) {
      ...
      // Otherwise the CGSizeZero default will cause UIKit to not show it (so this isn't an error
      // like the cellForItem case).
  ...
  return ^{
    ...
    // TODO: ASRangeController doesn't currently support managing interfaceState for supplementary
    // nodes. For now, we allow the standard ASInterfaceStateInHierarchy behavior by ensuring we do
    // not inform the node that it should expect external management of interfaceState.
    /*
    if (!disableRangeController) {
      [node enterHierarchyState:ASHierarchyStateRangeManaged];
    }
...
- (ASSizeRange)dataController:(ASDataController *)dataController
  ...
  ASDisplayNodeAssert(
      NO, @"To support supplementary nodes in ASCollectionView, it must have a layoutInspector for "
...
- (id<ASSectionContext>)dataController:(ASDataController *)dataController
                     contextForSection:(NSInteger)section {
  ASDisplayNodeAssertMainThread();
...
- (NSHashTable<ASCollectionElement *> *)visibleElementsForRangeController:
    (ASRangeController *)rangeController {
...
- (NSString *)nameForRangeControllerDataSource {
  return self.asyncDataSource ? NSStringFromClass([self.asyncDataSource class])
...
- (void)rangeController:(ASRangeController *)rangeController
  ...
  if (!self.asyncDataSource || _superIsPendingDataLoad || _updatingInResponseToInteractiveMove) {
    ...
    return;  // if the asyncDataSource has become invalid while we are processing, ignore this
             // request to avoid crashes
  }

  // TODO Do we need to notify _layoutFacilitator before reloadData?
  for (_ASHierarchyItemChange *change in
       [changeSet itemChangesOfType:_ASHierarchyChangeTypeDelete]) {
  ...
  for (_ASHierarchySectionChange *change in
       [changeSet sectionChangesOfType:_ASHierarchyChangeTypeDelete]) {
    [_layoutFacilitator collectionViewWillEditSectionsAtIndexSet:change.indexSet batched:YES];
  }

  for (_ASHierarchySectionChange *change in
       [changeSet sectionChangesOfType:_ASHierarchyChangeTypeInsert]) {
    [_layoutFacilitator collectionViewWillEditSectionsAtIndexSet:change.indexSet batched:YES];
  }

  for (_ASHierarchyItemChange *change in
       [changeSet itemChangesOfType:_ASHierarchyChangeTypeInsert]) {
    [_layoutFacilitator collectionViewWillEditCellsAtIndexPaths:change.indexPaths batched:YES];
  }

  ASPerformBlockWithoutAnimation(!changeSet.animated, ^{
    as_activity_scope(as_activity_create("Commit collection update", changeSet.rootActivity,
    ...
    if (changeSet.includesReloadData) {
      ...
      const auto completion = ^(BOOL finished) {
        as_activity_scope(as_activity_create("Handle collection update completion",
                                             changeSet.rootActivity, OS_ACTIVITY_FLAG_DEFAULT));
        as_log_verbose(ASCollectionLog(), "Update animation finished %{public}@",
                       self.collectionNode);
      ...
      if (shouldReloadData) {
        ...
        [self
            _superPerformBatchUpdates:^{
              ...
              for (_ASHierarchyItemChange *change in
                   [changeSet itemChangesOfType:_ASHierarchyChangeTypeReload]) {
                [super reloadItemsAtIndexPaths:change.indexPaths];
                numberOfUpdates++;
              }

              for (_ASHierarchySectionChange *change in
                   [changeSet sectionChangesOfType:_ASHierarchyChangeTypeReload]) {
...
- (void)nodesDidRelayout:(NSArray<ASCellNode *> *)nodes {
  ...
  const auto uikitIndexPaths =
      ASArrayByFlatMapping(nodes, ASCellNode * node, [self indexPathForNode:node]);
  ...
  for (ASCellNode *node in nodes) {
    if (invalidationStyle == ASCollectionViewInvalidationStyleNone) {
      // We nodesDidRelayout also while we are in layoutSubviews. This should be no problem as CA
      // will ignore this call while be in a layout pass
      [self setNeedsLayout];
    ...
    if (invalidationStyle == ASCollectionViewInvalidationStyleWithAnimation) {
      // TODO: Incorporate `shouldAnimateSizeChanges` into ASEnvironmentState for performance
      // benefit.
      static dispatch_once_t onceToken;
...
// Need these to drive interfaceState so we know when we are visible, if not nested in another
// range-managing element. Because our superclass is a true UIKit class, we cannot also subclass
// _ASDisplayView.
...
- (void)didMoveToWindow {
  ...
  if (!visible && node.inHierarchy) {
    if (rangeControllerNeedsUpdate) {
      ...
      // Although we have not yet cleared the interfaceState's Visible bit (this  happens in
      // __exitHierarchy), the ASRangeController will get the correct value from
  ...
  // Updating the visible node index paths only for not range managed nodes. Range managed nodes
  // will get their their update in the layout pass
  if (rangeControllerNeedsUpdate) {
    [_rangeController updateRanges];
  }
...
- (void)layer:(CALayer *)layer
    didChangeBoundsWithOldValue:(CGRect)oldBounds
                       newValue:(CGRect)newBounds {
  ...
  if (_hasDataControllerLayoutDelegate || self.collectionViewLayout == nil) {
    // Let the layout delegate handle bounds changes if it's available. If no layout, it will init
    // in the new state.
    return;
  }
  ...
  // For a more permanent fix to the unsafety mentioned above, see
  // https://github.com/facebook/AsyncDisplayKit/pull/2182
  ...
  BOOL changedInNonScrollingDirection =
      (fixedHorizontally && newSize.width != lastUsedSize.width) ||
