...
@interface ASControlNode () {
 @private
  ...
  NSMutableDictionary<id<NSCopying>, NSMutableArray<ASControlTargetAction *> *>
      *_controlEventDispatchTable;
}
...
/**
  @abstract Returns a key to be used in _controlEventDispatchTable that identifies the control
  event.
...
/**
 @abstract Returns the expanded bounds used to determine if a touch is considered 'inside' during
 tracking.
...
- (instancetype)init {
  ...
  // As we have no targets yet, we start off with user interaction off. When a target is added,
...
- (void)didLoad {
  ...
  // On tvOS all controls, such as buttons, interact with the focus system even if they don't have a
  // target set on them. Here we add our own internal tap gesture to handle this behaviour.
  self.userInteractionEnabled = YES;
  UITapGestureRecognizer *tapGestureRec =
      [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(_pressDown)];
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
  ...
  // Additionally, if we're already tracking a touch, a second touch beginning is cause for
  // cancellation.
  if (touches.count > 1 || self.tracking) {
    ...
    // No need to check bounds on touchesBegan as we wouldn't get the call if it wasn't in our
    // bounds.
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
  ...
  [self
      sendActionsForControlEvents:(touchUpIsInsideExpandedBounds ? ASControlNodeEventTouchUpInside
...
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gestureRecognizer {
  // If we're interested in touches, this is a tap (the only gesture we care about) and passed
  // -hitTest for us, then no, you may not begin. Sir.
...
- (void)addTarget:(id)target
              action:(SEL)action
    forControlEvents:(ASControlNodeEvent)controlEventMask {
  ...
  if (!_controlEventDispatchTable) {
    _controlEventDispatchTable = [[NSMutableDictionary alloc]
        initWithCapacity:
            kASControlNodeEventDispatchTableInitialCapacity];  // enough to handle common types
                                                               // without re-hashing the dictionary
                                                               // when adding entries.
  ...
  // Enumerate the events in the mask, adding the target-action pair for each control event included
  // in controlEventMask
  _ASEnumerateControlEventsIncludedInMaskWithBlock(
      controlEventMask, ^(ASControlNodeEvent controlEvent) {
...
- (NSArray *)actionsForTarget:(id)target forControlEvent:(ASControlNodeEvent)controlEvent {
  ...
  for (ASControlTargetAction *targetAction in eventTargetActionArray) {
    if ((target == nil && targetAction.createdWithNoTarget) ||
        (target != nil && target == targetAction.target)) {
...
- (void)removeTarget:(id)target
  ...
  // Enumerate the events in the mask, removing the target-action pair for each control event
  // included in controlEventMask.
  _ASEnumerateControlEventsIncludedInMaskWithBlock(
      controlEventMask, ^(ASControlNodeEvent controlEvent) {
        ...
        NSPredicate *filterPredicate = [NSPredicate
            predicateWithBlock:^BOOL(ASControlTargetAction *_Nullable evaluatedObject,
...
- (void)sendActionsForControlEvents:(ASControlNodeEvent)controlEvents withEvent:(UIEvent *)event {
  ASDisplayNodeAssertMainThread();  // We access self.view below, it's not safe to call this off of
                                    // main.
  NSParameterAssert(controlEvents != 0);

  NSMutableArray *resolvedEventTargetActionArray =
      [[NSMutableArray<ASControlTargetAction *> alloc] init];

  {
    ...
    _ASEnumerateControlEventsIncludedInMaskWithBlock(controlEvents, ^(
      ...
      for (ASControlTargetAction *targetAction in
               _controlEventDispatchTable[_ASControlNodeEventKeyForControlEvent(controlEvent)]) {
        ...
        if (!resolvedTargetAction.target && targetAction.createdWithNoTarget) {
          // if the target cannot perform the action, travel the responder chain to try to find
          // something that does
          resolvedTargetAction.target = [self.view targetForAction:resolvedTargetAction.action
                                                        withSender:self];
        }
  ...
  // We don't want to hold the lock while calling out, we could potentially walk up the ownership
  // tree causing a deadlock.
#pragma clang diagnostic push
...
void _ASEnumerateControlEventsIncludedInMaskWithBlock(ASControlNodeEvent mask,
  ...
  // Start with our first event (touch down) and work our way up to the last event
