  ...
  for (Ivar ivar : ivars) {
    ...
    if ([object_getClass(value) needsMainThreadDeallocation]) {
      as_log_debug(ASMainThreadDeallocationLog(),
                   "%@: Trampolining ivar '%s' value %@ for main deallocation.", self,
      ...
      as_log_debug(ASMainThreadDeallocationLog(), "%@: Not trampolining ivar '%s' value %@.", self,
                   ivar_getName(ivar), value);
    }
  }
}
  ...
  for (NSUInteger i = 0; i < allMyIvarsCount; i++) {
    ...
    if (type != NULL && strcmp(type, @encode(id)) == 0) {
      ...
      as_log_verbose(ASMainThreadDeallocationLog(),
                     "%@: Marking ivar '%s' for possible main deallocation due to type id", self,
                     ivar_getName(ivar));
    } else {
      ...
      if ([c needsMainThreadDeallocation]) {
        ...
        resultCount += 1;
        as_log_verbose(ASMainThreadDeallocationLog(),
                       "%@: Marking ivar '%s' for main deallocation due to class %@", self,
                       ivar_getName(ivar), c);
      } else {
        as_log_verbose(ASMainThreadDeallocationLog(),
                       "%@: Skipping ivar '%s' for main deallocation.", self, ivar_getName(ivar));
      }
    }
  }
  free(allMyIvars);
