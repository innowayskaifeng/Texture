 ...
 */
- (CGSize)galleryLayoutDelegate:(ASCollectionGalleryLayoutDelegate *)delegate
                sizeForElements:(ASElementMap *)elements;
 ...
 * @discussion This method will only be called on main thread.
 *
 * @discussion For a vertically scrolling layout, this value represents the minimum spacing between
 * rows. For a horizontally scrolling one, it represents the minimum spacing between columns. It is
 * not applied between the first line and the header, or between the last line and the footer. This
 * is the same behavior as UICollectionViewFlowLayout's minimumLineSpacing.
 *
 * @param delegate The calling object.
 *
...
/**
 * Returns the minumum spacing to use between items in the same row or column, depending on the
 * scroll directions.
 *
 * @discussion This method will only be called on main thread.
 *
 * @discussion For a vertically scrolling layout, this value represents the minimum spacing between
 * items in the same row. For a horizontally scrolling one, it represents the minimum spacing
 * between items in the same column. It is considered while fitting items into lines, but the actual
 * final spacing between some items might be larger. This is the same behavior as
 * UICollectionViewFlowLayout's minimumInteritemSpacing.
 *
 * @param delegate The calling object.
 *
 * @param elements All elements in the layout.
 ...
 *
 * @param scrollableDirections The scrollable directions of this layout. Must be either vertical or
