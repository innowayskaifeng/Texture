...
ASPageCoordinate ASPageCoordinateForPageThatContainsPoint(CGPoint point, CGSize pageSize) {
  return ASPageCoordinateMake((MAX(0.0, point.x) / pageSize.width),
                              (MAX(0.0, point.y) / pageSize.height));
}
...
CGRect ASPageCoordinateGetPageRect(ASPageCoordinate pageCoordinate, CGSize pageSize) {
  ...
  return CGRectMake(ASPageCoordinateGetX(pageCoordinate) * pageWidth,
                    ASPageCoordinateGetY(pageCoordinate) * pageHeight, pageWidth, pageHeight);
}

NSPointerArray *ASPageCoordinatesForPagesThatIntersectRect(CGRect rect,
                                                           CGSize contentSize,
                                                           CGSize pageSize) {
  ...
  NSPointerArray *result =
      [NSPointerArray pointerArrayWithOptions:(NSPointerFunctionsIntegerPersonality |
                                               NSPointerFunctionsOpaqueMemory)];

  ASPageCoordinate minPage = ASPageCoordinateForPageThatContainsPoint(
      CGPointMake(CGRectGetMinX(rect), CGRectGetMinY(rect)), pageSize);
  ASPageCoordinate maxPage = ASPageCoordinateForPageThatContainsPoint(
      CGPointMake(CGRectGetMaxX(rect), CGRectGetMaxY(rect)), pageSize);
  if (minPage == maxPage) {
...
+ (instancetype)pageTableWithValuePointerFunctions:(NSPointerFunctions *)valueFuncs
  ...
  dispatch_once(&onceToken, ^{
    pageCoordinatesFuncs =
        [NSPointerFunctions pointerFunctionsWithOptions:NSPointerFunctionsIntegerPersonality |
                                                        NSPointerFunctionsOpaqueMemory];
  });
...
+ (ASPageTable *)pageTableForStrongObjectPointers NS_RETURNS_RETAINED {
  ...
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    strongObjectPointerFuncs =
        [NSPointerFunctions pointerFunctionsWithOptions:NSPointerFunctionsStrongMemory];
  });
...
+ (ASPageTable *)pageTableForWeakObjectPointers NS_RETURNS_RETAINED {
  ...
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    weakObjectPointerFuncs =
        [NSPointerFunctions pointerFunctionsWithOptions:NSPointerFunctionsWeakMemory];
  });
...
+ (ASPageToLayoutAttributesTable *)pageTableWithLayoutAttributes:
  ...
  for (UICollectionViewLayoutAttributes *attrs in layoutAttributesEnumerator) {
    ...
    NSPointerArray *pages =
        ASPageCoordinatesForPagesThatIntersectRect(attrs.frame, contentSize, pageSize);
