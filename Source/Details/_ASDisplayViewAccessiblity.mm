...
typedef NSComparisonResult (^SortAccessibilityElementsComparator)(
    id<ASAccessibilityElementPositioning>, id<ASAccessibilityElementPositioning>);
...
static void SortAccessibilityElements(NSMutableArray *elements) {
  ...
  dispatch_once(&onceToken, ^{
    comparator = ^NSComparisonResult(id<ASAccessibilityElementPositioning> a,
                                     id<ASAccessibilityElementPositioning> b) {
...
- (CGRect)accessibilityFrame {
  ...
  accessibilityFrame = UIAccessibilityConvertFrameToScreenCoordinates(accessibilityFrame,
                                                                      self.accessibilityContainer);
...
@interface ASAccessibilityCustomAction
    : UIAccessibilityCustomAction <ASAccessibilityElementPositioning>
...
- (CGRect)accessibilityFrame {
  CGRect accessibilityFrame = [self.containerNode convertRect:self.node.bounds fromNode:self.node];
  accessibilityFrame =
      UIAccessibilityConvertFrameToScreenCoordinates(accessibilityFrame, self.container);
  return accessibilityFrame;
}
...
static void CollectAccessibilityElementsForContainer(ASDisplayNode *container,
                                                     UIView *view,
                                                     NSMutableArray *elements) {
    ...
    if (node.accessibilityLabel.length > 0) {
      if (node.accessibilityTraits & InteractiveAccessibilityTraitsMask()) {
        ASAccessibilityCustomAction *action = [[ASAccessibilityCustomAction alloc]
            initWithName:node.accessibilityLabel
                  target:node
        ...
        // Even though not surfaced to UIKit, create a non-interactive element for purposes of
        // building sorted aggregated label.
  ...
  if (AS_AVAILABLE_IOS_TVOS(11, 11)) {
    ...
    [attributedLabels enumerateObjectsUsingBlock:^(id _Nonnull obj, NSUInteger idx,
                                                   BOOL *_Nonnull stop) {
      if (idx != 0) {
...
static void CollectAccessibilityElementsForView(UIView *view, NSMutableArray *elements) {
  ...
  for (ASDisplayNode *subnode in node.subnodes) {
    if (subnode.isAccessibilityElement) {
      ...
      // Go down the hierarchy of the layer backed subnode and collect all of the
...
- (NSArray *)accessibilityElements {
  ...
  if (_accessibilityElements == nil ||
      ASActivateExperimentalFeature(ASExperimentalDisableAccessibilityCache)) {
