...
- (void)setDelegate:(id)delegate {
  ...
  _delegateFlags.delegateDidChangeBounds =
      [delegate respondsToSelector:@selector(layer:didChangeBoundsWithOldValue:newValue:)];
}
...
- (void)setDisplaySuspended:(BOOL)displaySuspended {
  ...
  if (_displaySuspended != displaySuspended) {
    ...
    if (!displaySuspended) {
      ...
      // If suspending display, cancel any current async display so that we don't have contents set
      // on us when it's finished.
...
- (void)setBounds:(CGRect)bounds {
  ...
  if (_delegateFlags.delegateDidChangeBounds) {
    ...
    [(id<ASCALayerExtendedDelegate>)self.delegate layer:self
                            didChangeBoundsWithOldValue:oldBounds
                                               newValue:bounds];
  ...
  if (_attemptedDisplayWhileZeroSized && CGRectIsEmpty(bounds) == NO &&
...
#if DEBUG  // These override is strictly to help detect application-level threading errors.  Avoid
           // method overhead in release.
...
+ (dispatch_queue_t)displayQueue {
  ...
  dispatch_once(&onceToken, ^{
    displayQueue = dispatch_queue_create("org.AsyncDisplayKit.ASDisplayLayer.displayQueue",
...
// e.g. <MYTextNodeLayer: 0xFFFFFF; node = <MYTextNode: 0xFFFFFFE; name = "Username node for user
// 179">>
