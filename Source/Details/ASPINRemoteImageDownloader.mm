...
+ (void)setSharedImageManagerWithConfiguration:(nullable NSURLSessionConfiguration *)configuration {
  NSAssert(sharedDownloader == nil,
           @"Singleton has been created and session can no longer be configured.");
...
+ (void)setSharedPreconfiguredRemoteImageManager:
    (PINRemoteImageManager *)preconfiguredPINRemoteImageManager {
  ...
  NSAssert1(sharedPINRemoteImageManager == nil,
            @"An instance of %@ has been set. Either configuration or preconfigured image manager "
            @"can be set at a time and only once.",
...
+ (PINRemoteImageManager *)
    PINRemoteImageManagerWithConfiguration:(nullable NSURLSessionConfiguration *)configuration
                                imageCache:(nullable id<PINRemoteImageCaching>)imageCache {
  PINRemoteImageManager *manager = nil;
#if DEBUG
  // Check that Carthage users have linked both PINRemoteImage & PINCache by testing for one file
  // each
  if (!(NSClassFromString(@"PINRemoteImageManager"))) {
    NSException *e =
        [NSException exceptionWithName:@"FrameworkSetupException"
...
- (PINRemoteImageManager *)sharedPINRemoteImageManager {
  dispatch_once(&shared_init_predicate, ^{
    sharedPINRemoteImageManager =
        [ASPINRemoteImageDownloader PINRemoteImageManagerWithConfiguration:nil imageCache:nil];
  });
...
- (BOOL)sharedImageManagerSupportsMemoryRemoval {
  ...
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    sharedImageManagerSupportsMemoryRemoval = [[[self sharedPINRemoteImageManager] cache]
        respondsToSelector:@selector(removeObjectForKeyFromMemory:)];
  });
...
{
  PINRemoteImageManagerProgressDownload progressDownload =
      ^(int64_t completedBytes, int64_t totalBytes) {
  ...
  PINRemoteImageManagerImageCompletion imageCompletion =
      ^(PINRemoteImageManagerResult *_Nonnull result) {
        [ASPINRemoteImageDownloader
            _performWithCallbackQueue:callbackQueue
                                 work:^{
#if PIN_ANIMATED_AVAILABLE
                                   if (result.alternativeRepresentation) {
                                     completion(result.alternativeRepresentation, result.error,
                                                result.UUID, result);
                                   } else {
  ...
  // add "IgnoreCache" option since we have a caching API so we already checked it, not worth
  // checking again. PINRemoteImage is responsible for coalescing downloads, and even if it wasn't,
  // the tiny probability of extra downloads isn't worth the effort of rechecking caches every
  // single time. In order to provide feedback to the consumer about whether images are cached, we
  // can't simply make the cache a no-op and check the cache as part of this download.
  return [[self sharedPINRemoteImageManager]
...
- (void)cancelImageDownloadForIdentifier:(id)downloadIdentifier {
  ASDisplayNodeAssert([downloadIdentifier isKindOfClass:[NSUUID class]],
                      @"downloadIdentifier must be NSUUID");
...
- (void)setProgressImageBlock:(ASImageDownloaderProgressImage)progressBlock
  ...
  if (progressBlock) {
    ...
    [[self sharedPINRemoteImageManager] setProgressImageCallback:nil
                                                  ofTaskWithUUID:downloadIdentifier];
  }
}
...
- (id)alternateRepresentationWithData:(NSData *)data
