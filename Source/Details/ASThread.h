...
#define ASLockScope(nsLocking)                                            \
  id<NSLocking> __lockToken __attribute__((cleanup(_ASLockScopeCleanup))) \
      NS_VALID_UNTIL_END_OF_SCOPE = nsLocking;                            \
  [__lockToken lock];

/// Same as ASLockScope(1) but lock isn't retained (be careful).
#define ASLockScopeUnowned(nsLocking)                                   \
  __unsafe_unretained id<NSLocking> __lockToken                         \
      __attribute__((cleanup(_ASLockScopeUnownedCleanup))) = nsLocking; \
  [__lockToken lock];
...
ASDISPLAYNODE_INLINE void _ASLockScopeUnownedCleanup(
    id<NSLocking> __unsafe_unretained *const lockPtr) {
  [*lockPtr unlock];
}
...
#define ASUnlockScope(nsLocking)                                            \
  id<NSLocking> __lockToken __attribute__((cleanup(_ASUnlockScopeCleanup))) \
      NS_VALID_UNTIL_END_OF_SCOPE = nsLocking;                              \
  [__lockToken unlock];
