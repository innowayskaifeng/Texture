...
/**
 * Returns the section index into the receiver that corresponds to the same element in @c map at @c
 * sectionIndex. Fast O(N).
 *
 ...
 *
 * NOTE: This method only regards the category, kind, and index path of the attributes object.
 * Elements do not have any concept of size/position.
 */
- (nullable ASCollectionElement *)elementForLayoutAttributes:
...
typedef NSDictionary<NSString *, NSDictionary<NSIndexPath *, ASCollectionElement *> *>
    ASSupplementaryElementDictionary;

/**
 * Create a new element map for this dataset. You probably don't need to use this – ASDataController
 * is the only one who creates these.
 *
