...
- (void)dealloc {
  NSAssert(_operationCompletionBlock == nil,
           @"Should have been called and released before -dealloc");
}
...
- (NSString *)description {
  return [NSString
      stringWithFormat:@"<ASAsyncTransactionOperation: %p - value = %@>", self, self.value];
}
...
class ASAsyncTransactionQueue {
 public:
  ...
  class Group {
   public:
    // call when group is no longer needed; after last scheduled operation the group will delete
    // itself
 ...
 private:
  ...
  class GroupImpl : public Group {
   public:
    GroupImpl(ASAsyncTransactionQueue &queue)
        : _pendingOperations(0), _releaseCalled(false), _queue(queue) {}
  ...
  struct DispatchEntry  // entry for each dispatch queue
  {
    ...
    typedef std::list<OperationQueue::iterator>
        OperationIteratorList;  // each item points to operation queue
...
ASAsyncTransactionQueue::Operation ASAsyncTransactionQueue::DispatchEntry::popNextOperation(
    bool respectPriority) {
  NSCAssert(!_operationQueue.empty() && !_operationPriorityMap.empty(),
            @"No scheduled operations available");
...
void ASAsyncTransactionQueue::DispatchEntry::pushOperation(
    ASAsyncTransactionQueue::Operation operation) {
...
void ASAsyncTransactionQueue::GroupImpl::schedule(NSInteger priority,
  ...
  if (entry._threadCount < maxThreads) {  // we need to spawn another thread
    ...
    // first thread will take operations in queue order (regardless of priority), other threads will
    // respect priority
    ...
    dispatch_async(queue, ^{
      ...
      if (entry._threadCount == 0) {
        NSCAssert(
            entry._operationQueue.empty() || entry._operationPriorityMap.empty(),
...
- (void)dealloc {
  // Uncommitted transactions break our guarantees about releasing completion blocks on
  // callbackQueue.
  NSAssert(self.state != ASAsyncTransactionStateOpen,
           @"Uncommitted ASAsyncTransactions are not allowed");
  if (_group) {
    _group->release();
  }
}
...
- (void)addOperationWithBlock:(asyncdisplaykit_async_transaction_operation_block_t)block
                   ...
                   completion:
                       (asyncdisplaykit_async_transaction_operation_completion_block_t)completion {
  ASDisplayNodeAssertMainThread();
  NSAssert(self.state == ASAsyncTransactionStateOpen,
...
- (void)commit {
  ...
  if ([_operations count] == 0) {
    // Fast path: if a transaction was opened, but no operations were added, execute completion
    // block synchronously.
    if (_completionBlock) {
...
- (void)completeTransaction {
  ...
  if (state != ASAsyncTransactionStateComplete) {
    ...
    // (e.g. if we needed to force one in this runloop with -waitUntilComplete, but another was
    // already scheduled)
  ...
  if (self.state != ASAsyncTransactionStateComplete) {
    if (_group) {
      ...
      if (self.state == ASAsyncTransactionStateOpen) {
        ...
        NSAssert(self.state != ASAsyncTransactionStateOpen,
                 @"Transaction should not be open after committing group");
      }
...
- (void)_ensureTransactionData {
  // Lazily initialize _group and _operations to avoid overhead in the case where no operations are
...
- (NSString *)description {
  return [NSString
      stringWithFormat:@"<_ASAsyncTransaction: %p - _state = %lu, _group = %p, _operations = %@>",
                       self, (unsigned long)self.state, _group, _operations];
}
