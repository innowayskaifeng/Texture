 ...
 *   on our node when one is next assigned to us in @c setNode: . Since there may be cases when we
 * _do_ already have our node assigned e.g. during a layout update for existing cells, we also
 * attempt to update it now.
 */
