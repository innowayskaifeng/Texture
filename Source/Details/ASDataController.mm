...
@interface ASDataController () {
  ...
  dispatch_queue_t _editingTransactionQueue;  // Serial background queue.  Dispatches concurrent
                                              // layout and manages _editingNodes.
  dispatch_group_t
      _editingTransactionGroup;  // Group of all edit transaction blocks. Useful for waiting.
...
- (instancetype)initWithDataSource:(id<ASDataControllerSource>)dataSource
  ...
  _dataSourceFlags.supplementaryNodesOfKindInSection = [_dataSource
      respondsToSelector:@selector(dataController:supplementaryNodesOfKind:inSection:)];
  _dataSourceFlags.supplementaryNodeBlockOfKindAtIndexPath = [_dataSource
      respondsToSelector:@selector(dataController:
                             supplementaryNodeBlockOfKind:atIndexPath:shouldAsyncLayout:)];
  ...
  _dataSourceFlags.constrainedSizeForSupplementaryNodeOfKindAtIndexPath =
      [_dataSource respondsToSelector:@selector(dataController:
                                          constrainedSizeForSupplementaryNodeOfKind:atIndexPath:)];
  _dataSourceFlags.contextForSection = [_dataSource respondsToSelector:@selector(dataController:
                                                                              contextForSection:)];
  ...
  const char *queueName = [[NSString
      stringWithFormat:@"org.AsyncDisplayKit.ASDataController.editingTransactionQueue:%p", self]
...
- (NSArray<NSIndexPath *> *)_allIndexPathsForItemsOfKind:(NSString *)kind
                                              inSections:(NSIndexSet *)sections {
  ASDisplayNodeAssertMainThread();
  ...
  if ([kind isEqualToString:ASDataControllerRowNodeKind]) {
    ...
    [sections enumerateRangesUsingBlock:^(NSRange range, BOOL *_Nonnull stop) {
      for (NSUInteger sectionIndex = range.location; sectionIndex < NSMaxRange(range);
           sectionIndex++) {
    ...
    [sections enumerateRangesUsingBlock:^(NSRange range, BOOL *_Nonnull stop) {
      for (NSUInteger sectionIndex = range.location; sectionIndex < NSMaxRange(range);
           sectionIndex++) {
        NSUInteger itemCount = [dataSource dataController:self
...
/**
 * Agressively repopulates supplementary nodes of all kinds for sections that contains some given
 * index paths.
 *
 * @param map The element map into which to apply the change.
 * @param indexPaths The index paths belongs to sections whose supplementary nodes need to be
 * repopulated.
...
- (void)_updateSupplementaryNodesIntoMap:(ASMutableElementMap *)map
  ...
  if (sectionCount > 0) {
    NSIndexSet *sectionIndexes =
        [NSIndexSet indexSetWithIndexesInRange:NSMakeRange(0, sectionCount)];
    ASSizeRange newSizeRange = ASSizeRangeZero;
    for (NSString *kind in [self supplementaryKindsInSections:sectionIndexes]) {
      NSArray<NSIndexPath *> *indexPaths = [self _allIndexPathsForItemsOfKind:kind
                                                                   inSections:sectionIndexes];
      ...
      for (NSIndexPath *indexPath in indexPaths) {
        ASCollectionElement *previousElement = [previousMap supplementaryElementOfKind:kind
...
- (void)_insertElementsIntoMap:(ASMutableElementMap *)map
  ...
  for (NSIndexPath *indexPath in indexPaths) {
    ...
    ASCollectionElement *element =
        [[ASCollectionElement alloc] initWithNodeModel:nodeModel
                                             nodeBlock:nodeBlock
...
- (ASSizeRange)constrainedSizeForNodeOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath {
  ...
  if (_dataSourceFlags.constrainedSizeForSupplementaryNodeOfKindAtIndexPath) {
    return [dataSource dataController:self
        constrainedSizeForSupplementaryNodeOfKind:kind
                                      atIndexPath:indexPath];
  }

  ASDisplayNodeAssert(NO, @"Unknown constrained size for node of kind %@ by data source %@", kind,
                      dataSource);
  return ASSizeRangeZero;
}
...
- (void)onDidFinishSynchronizing:(void (^)())completion {
  ...
  if ([self isSynchronized]) {
    ...
    // Hang on to the completion block so that it gets called the next time view is synchronized to
    // data.
...
- (void)updateWithChangeSet:(_ASHierarchyChangeSet *)changeSet {
  ...
  if (changeSet.includesReloadData) {
    if (_initialReloadDataHasBeenCalled) {
      as_log_debug(ASCollectionLog(), "reloadData %@",
  ...
  // If the initial reloadData has not been called, just bail because we don't have our old data
  // source counts. See
  // ASUICollectionViewTests.testThatIssuingAnUpdateBeforeInitialReloadIsUnacceptable for the issue
  // that UICollectionView has that we're choosing to workaround.
  if (!_initialReloadDataHasBeenCalled) {
  ...
  // Attempt to mark the update completed. This is when update validation will occur inside the
  // changeset. If an invalid update exception is thrown, we catch it and inject our
  // "validationErrorSource" object, which is the table/collection node's data source, into the
  // exception reason to help debugging.
  @try {
    ...
    if (e.name == ASCollectionInvalidUpdateException && responsibleDataSource != nil) {
      [NSException raise:ASCollectionInvalidUpdateException
                  format:@"%@: %@", [responsibleDataSource class], e.reason];
    } else {
      @throw e;
    }
  }
  ...
  {
    as_activity_scope(as_activity_create("Latch new data for collection update",
  ...
  dispatch_group_async(_editingTransactionGroup, _editingTransactionQueue, ^{
    __block __unused os_activity_scope_state_s
        preparationScope = {};  // unused if deployment target < iOS10
    as_activity_scope_enter(as_activity_create("Prepare nodes for collection update",
    ...
    if (canDelegate) {
      ...
      for (ASCollectionElement *element in newMap) {
        ...
        if (nodeIfAllocated.shouldUseUIKitCell) {
          // If the node exists and we know it is a passthrough cell, we know it will never have a
          // .calculatedLayout.
          continue;
      ...
      [_delegate dataController:self
            updateWithChangeSet:changeSet
                        updates:^{
                          ...
                          //
                          // Note that since the backing collection view might be busy responding to
                          // user events (e.g scrolling), it will not consume the batch update
                          // blocks immediately. As a result, in a short intermidate time, the view
                          // will still be relying on the old data source state. Thus, we can't just
                          // swap the new map immediately before step 4, but until this update block
                          // is executed. (https://github.com/TextureGroup/Texture/issues/378)
 ...
 */
- (void)_updateSectionsInMap:(ASMutableElementMap *)map
                   changeSet:(_ASHierarchyChangeSet *)changeSet {
  ASDisplayNodeAssertMainThread();
...
- (void)relayoutNodes:(id<NSFastEnumeration>)nodes
     nodesSizeChanged:(NSMutableArray<ASCellNode *> *)nodesSizesChanged {
  ...
  for (ASCellNode *node in nodes) {
    ...
    // then we can't check the presented size. If it's not in the pending map, we can't get the
    // constrained size. This will only happen if the element has been deleted, so the specifics of
    // this behavior aren't important.
    ...
    ASSizeRange constrainedSize = [self constrainedSizeForNodeOfKind:kind
                                                         atIndexPath:indexPathInPendingMap];
    [self _layoutNode:node withConstrainedSize:constrainedSize];

    BOOL matchesSize = [dataSource dataController:self
                          presentedSizeForElement:element
                                      matchesSize:node.frame.size];
    if (!matchesSize) {
...
- (void)relayoutAllNodesWithInvalidationBlock:(nullable void (^)())invalidationBlock {
  ...
  // i.e there might be some nodes that were measured using the old constrained size but haven't
  // been added to _visibleMap
  LOG(@"Edit Command - relayoutRows");
  [self _scheduleBlockOnMainSerialQueue:^{
    // Because -invalidateLayout doesn't trigger any operations by itself, and we answer queries
    // from UICollectionView using layoutThatFits:, we invalidate the layout before we have updated
    // all of the cells. Any cells that the collection needs the size of immediately will get
    // -layoutThatFits: with a new constraint, on the main thread, and synchronously calculate them.
    // Meanwhile, relayoutAllNodes will update the layout of any remaining nodes on background
    // threads (and fast-return for any nodes that the UICV got to first).
...
- (void)_relayoutAllNodes {
  ...
  // Assuming this method is run on the main serial queue, _pending and _visible maps are synced and
  // can be manipulated directly.
  ASDisplayNodeAssert(_visibleMap == _pendingMap,
                      @"Expected visible and pending maps to be synchronized: %@", self);
  ...
  for (ASCollectionElement *element in _visibleMap) {
    // Ignore this element if it is no longer in the latest data. It is still recognized in the
    // UIKit world but will be deleted soon.
    ...
    NSString *kind = element.supplementaryElementKind ?: ASDataControllerRowNodeKind;
    ASSizeRange newConstrainedSize = [self constrainedSizeForNodeOfKind:kind
                                                            atIndexPath:indexPathInPendingMap];
...
- (void)environmentDidChange {
  ASPerformBlockOnMainThread(^{
    ...
    // i.e there might be some elements that were allocated using the old trait collection but
    // haven't been added to _visibleMap
    [self _scheduleBlockOnMainSerialQueue:^{
