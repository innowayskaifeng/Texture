...
@property(nonatomic, readonly)
    NSMapTable<ASCollectionElement *, NSIndexPath *> *elementToIndexPathMap;
...
- (instancetype)initWithSections:(NSArray<ASSection *> *)sections
  ...
  if (self = [super init]) {
    ...
    _supplementaryElements = [[NSDictionary alloc] initWithDictionary:supplementaryElements
                                                            copyItems:YES];

    // Setup our index path map
    _elementToIndexPathMap = [NSMapTable
    ...
    for (NSDictionary *supplementariesForKind in [_supplementaryElements objectEnumerator]) {
      [supplementariesForKind enumerateKeysAndObjectsUsingBlock:^(
                                  NSIndexPath *_Nonnull indexPath,
                                  ASCollectionElement *_Nonnull element, BOOL *_Nonnull stop) {
        [_elementToIndexPathMap setObject:indexPath forKey:element];
      }];
    }
  }
...
- (ASCollectionElement *)elementForLayoutAttributes:
...
// NSMutableCopying conformance is declared in ASMutableElementMap.h, so that most consumers of
// ASElementMap don't bother with it.
...
- (BOOL)sectionIndexIsValid:(NSInteger)section assert:(BOOL)assert {
  ...
  if (section >= sectionCount || section < 0) {
    if (assert) {
      ASDisplayNodeFailAssert(@"Invalid section index %ld when there are only %ld sections!",
                              (long)section, (long)sectionCount);
    }
    return NO;
  } else {
    return YES;
  ...
  if (item >= itemCount || item < 0) {
    if (assert) {
      ASDisplayNodeFailAssert(@"Invalid item index %ld in section %ld which only has %ld items!",
                              (long)item, (long)section, (long)itemCount);
    }
    return NO;
  }
  *outItem = item;
