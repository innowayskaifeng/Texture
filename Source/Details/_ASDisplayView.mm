...
// Keep the node alive while its view is active.  If you create a view, add its layer to a layer
// hierarchy, then release the view, the layer retains the view to prevent a crash.  This replicates
// this behaviour for the node abstraction.
...
- (NSString *)description {
  ...
  if (node != nil) {
    ...
    // Remove layer description – it never contains valuable info and it duplicates the node info.
    // Noisy.
    NSRange layerDescriptionRange = [description rangeOfString:@"; layer = <.*>"
                                                       options:NSRegularExpressionSearch];
    if (layerDescriptionRange.location != NSNotFound) {
      [description replaceCharactersInRange:layerDescriptionRange withString:@""];
      // Our regex will grab the closing angle bracket and I'm not clever enough to come up with a
      // better one, so re-add it if needed.
...
- (void)willMoveToSuperview:(UIView *)newSuperview {
  // Keep the node alive while the view is in a view hierarchy.  This helps ensure that
  // async-drawing views can always display their contents as long as they are visible somewhere,
  // and aids in lifecycle management because the lifecycle of the node can be treated as the same
  // as the lifecycle of the view (let the view hierarchy own the view).
  ASDisplayNode *node = _asyncdisplaykit_node;  // Create strong reference to weak ivar.
    ...
    if (supernode) {
      if (supernodeLoaded) {
        if (supernode.layerBacked) {
          // See comment in -didMoveToSuperview.  This case should be avoided, but is possible with
          // app-level coding errors.
          needsSupernodeUpdate = (supernode.layer != newSuperview.layer);
        } else {
          // If we have a supernode, compensate for users directly messing with views by hitching up
          // to any new supernode.
      ...
      // If we have no supernode and we are now in a view hierarchy, check to see if we can hook up
      // to a supernode.
...
- (void)didMoveToSuperview {
  ...
  if (superview == nil) {
    // Clearing keepalive_node may cause deallocation of the node.  In this case, __exitHierarchy
    // may not have an opportunity (e.g. _node will be cleared by the time -didMoveToWindow occurs
    // after this) to clear the Visible interfaceState, which we need to do before deallocation to
    // meet an API guarantee.
    if (node.inHierarchy) {
      [node __exitHierarchy];
    }
...
#if DEBUG
  // This is only to help detect issues when a root-of-view-controller node is reused separately
  // from its view controller. Avoid overhead in release.
  if (superview && node.viewControllerRoot) {
    UIViewController *vc = [node closestViewController];

    ASDisplayNodeAssert(vc != nil && [vc isKindOfClass:[ASViewController class]] &&
                            ((ASViewController *)vc).node == node,
                        @"This node was once used as a view controller's node. You should not "
                        @"reuse it without its view controller.");
  }
#endif

  ASDisplayNode *supernode = node.supernode;
  ASDisplayNodeAssert(!supernode.isLayerBacked,
                      @"Shouldn't be possible for superview's node to be layer-backed.");
  ...
  if (supernode) {
    ...
    if (superview) {
      // If our new superview is not the same as the supernode's view, or the supernode has no view,
      // disconnect.
      if (supernodeLoaded) {
        if (supernode.layerBacked) {
          // As asserted at the top, this shouldn't be possible, but in production with assertions
          // disabled it can happen. We try to make such code behave as well as feasible because
          // it's not that hard of an error to make if some deep child node of a layer-backed node
          // happens to be view-backed, but it is not supported and should be avoided.
      ...
      // If supernode is loaded but our superview is nil, the user likely manually removed us, so
      // disconnect supernode. The unlikely alternative: we are in __unloadNode, with
      // shouldRasterizeSubnodes just having been turned on. In the latter case, we don't want to
      // disassemble the node hierarchy because all views are intentionally being destroyed.
      BOOL nodeIsRasterized =
          ((node.hierarchyState & ASHierarchyStateRasterized) == ASHierarchyStateRasterized);
      needsSupernodeRemoval = (supernodeLoaded && !nodeIsRasterized);
    }

    if (needsSupernodeRemoval) {
      // The node will only disconnect from its supernode, not removeFromSuperview, in this
      // condition.
      // FIXME: Needs rethinking if automaticallyManagesSubnodes=YES
...
- (void)setContentMode:(UIViewContentMode)contentMode {
  ...
  // Do our own mapping so as not to call super and muck up needsDisplayOnBoundsChange. If we're in
  // a production build, fall back to resize if we see redraw
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event {
  // REVIEW: We should optimize these types of messages by setting a boolean in the associated
  // ASDisplayNode subclass if they actually override the method.  Same goes for
  // -pointInside:withEvent: below.  Many UIKit classes use that pattern for meaningful reductions
  // of message send overhead in hot code (especially event handling).

  // Set boolean so this method can be re-entrant.  If the node subclass wants to default to / make
  // use of UIView hitTest:, it will call it on the view, which is _ASDisplayView.  After calling
  // into the node, any additional calls should use the UIView implementation of hitTest:
  ASDisplayNode *node = _asyncdisplaykit_node;  // Create strong reference to weak ivar.
  if (!_inHitTest) {
    _inHitTest = YES;
...
#define IMPLEMENT_RESPONDER_METHOD(__sel, __nodeMethodOverride, __viewMethodOverride)              \
  ...
  /* All __ prefixed methods are called from ASDisplayNode to let the view decide in what          \
  UIResponder state they are not overridden by a ASDisplayNode subclass */                                                 \
  -(BOOL)__##__sel {                                                                               \
    return [super __sel];                                                                          \
  }
...
- (BOOL)canPerformAction:(SEL)action withSender:(id)sender {
  // We forward responder-chain actions to our node if we can't handle them ourselves. See
...
- (id)forwardingTargetForSelector:(SEL)aSelector {
  // Ideally, we would implement -targetForAction:withSender: and simply return the node where we
  // don't respond personally. Unfortunately UIResponder's default implementation of
  // -targetForAction:withSender: doesn't follow its own documentation. It doesn't call
  // -targetForAction:withSender: up the responder chain when -canPerformAction:withSender: fails,
  // but instead merely calls -canPerformAction:withSender: on itself and then up the chain.
  // rdar://20111500. Consequently, to forward responder-chain actions to our node, we override
  // -canPerformAction:withSender: (used by the chain) to indicate support for responder
  // chain-driven actions that our node supports, and then provide the node as a forwarding target
  // here.
  ASDisplayNode *node = _asyncdisplaykit_node;  // Create strong reference to weak ivar.
  return node;
}
