...
/**
 Fetch the ASCellNode block for specific index path. This block should return the ASCellNode for the
...
- (nullable id)dataController:(ASDataController *)dataController
    nodeModelForItemAtIndexPath:(NSIndexPath *)indexPath;

/**
 * Called just after dispatching ASCellNode allocation and layout to the concurrent background
 * queue. In some cases, for example on the first content load for a screen, it may be desirable to
 * call -waitUntilAllUpdatesAreProcessed at this point.
 *
...
/**
 The constrained size range for layout. Called only if no data controller layout delegate is
 provided.
 */
- (ASSizeRange)dataController:(ASDataController *)dataController
...
- (nullable id<ASSectionContext>)dataController:(ASDataController *)dataController
...
/**
 ...
 These methods will be invoked from main thread right now, but it may be moved to background thread
 in the future.
 */
 ...
 *
 * @discussion This method is called ahead of time, i.e before the underlying collection/table view
 * is aware of the provided elements. As a result, clients must solely rely on the given context and
 * should not reach out to other objects for information not available in the context.
 *
 * This method will be called on background theads. It must be thread-safe and should not change any
 * internal state of the conforming object. It must block the calling thread but can dispatch to
 * other theads to reduce total blocking time.
 */
 ...
 *
 * All operations are asynchronous and thread safe. You can call it from background thread (it is
 * recommendated) and the data will be updated asynchronously. The dataSource must be updated to
 * reflect the changes before these methods has been called. For each data updating, the
 * corresponding methods in delegate will be called.
 */
 ...
 *
 * NOTE: Soon we will drop support for using ASTableView/ASCollectionView without the node, so this
 * will be non-null.
 */
 ...
 *
 * The invalidationBlock is called after flushing the ASMainSerialQueue, which ensures that any
 * in-progress layout calculations have been applied. The block will not be called if data hasn't
 * been loaded.
 */
...
/**
 * Notifies the data controller object that its environment has changed. The object will request its
 * environment delegate for new information and propagate the information to all visible elements,
 * including ones that are being prepared in background.
 *
 * @discussion If called before the initial @c reloadData, this method will do nothing and the trait
 * collection of the initial load will be requested from the environment delegate.
 *
...
/**
 * Reset visibleMap and pendingMap when asyncDataSource and asyncDelegate of collection view become
 * nil.
 */
- (void)clearData;
