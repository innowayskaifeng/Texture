...
NSString *const kASBasicImageDownloaderContextCallbackQueue =
    @"kASBasicImageDownloaderContextCallbackQueue";
NSString *const kASBasicImageDownloaderContextProgressBlock =
    @"kASBasicImageDownloaderContextProgressBlock";
NSString *const kASBasicImageDownloaderContextCompletionBlock =
    @"kASBasicImageDownloaderContextCompletionBlock";
...
- (void)performProgressBlocks:(CGFloat)progress {
  ...
  for (NSDictionary *callbackData in self.callbackDatas) {
    ASImageDownloaderProgress progressBlock =
        callbackData[kASBasicImageDownloaderContextProgressBlock];
...
- (void)completeWithImage:(UIImage *)image error:(NSError *)error {
  ...
  for (NSDictionary *callbackData in self.callbackDatas) {
    ASImageDownloaderCompletion completionBlock =
        callbackData[kASBasicImageDownloaderContextCompletionBlock];
    dispatch_queue_t callbackQueue = callbackData[kASBasicImageDownloaderContextCallbackQueue];
...
- (NSURLSessionTask *)createSessionTaskIfNecessaryWithBlock:
    (NSURLSessionTask * (^)())creationBlock {
  {
    ASDN::MutexLocker l(__instanceLock__);
...
- (void)setAsyncdisplaykit_context:(ASBasicImageDownloaderContext *)asyncdisplaykit_context {
  objc_setAssociatedObject(self, ContextKey(), asyncdisplaykit_context,
...
- (id)downloadImageWithURL:(NSURL *)URL
  ...
  // NSURLSessionDownloadTask will do file I/O to create a temp directory. If called on the main
  // thread this will cause significant performance issues.
  dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
    // associate metadata with it
    const auto callbackData = [[NSMutableDictionary alloc] init];
    callbackData[kASBasicImageDownloaderContextCallbackQueue] =
    ...
    NSURLSessionDownloadTask *task =
        (NSURLSessionDownloadTask *)[context createSessionTaskIfNecessaryWithBlock:^() {
