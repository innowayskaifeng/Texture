...
- (BOOL)respondsToSelector:(SEL)aSelector {
  if ([self interceptsSelector:aSelector]) {
    ...
    // Also return NO if _target has become nil due to zeroing weak reference (or placeholder
    // initialization).
...
- (id)forwardingTargetForSelector:(SEL)aSelector {
  if ([self interceptsSelector:aSelector]) {
    ...
    if (target) {
      ...
      // The _interceptor needs to be nilled out in this scenario. For that a strong reference needs
      // to be created to be able to nil out the _interceptor but still let it know that the proxy
      // target has deallocated We have to hold a strong reference to the interceptor as we have to
      // nil it out and call the proxyTargetHasDeallocated The reason that the interceptor needs to
      // be nilled out is that there maybe a change of a infinite loop, for example if a method will
      // be called in the proxyTargetHasDeallocated: that again would trigger a whole new forwarding
      // cycle
...
- (NSMethodSignature *)methodSignatureForSelector:(SEL)aSelector {
  ...
  // Unfortunately, in order to get this object to work properly, the use of a method which creates
  // an NSMethodSignature from a C string. -methodSignatureForSelector is called when a compiled
  // definition for the selector cannot be found. This is the place where we have to create our own
  // dud NSMethodSignature. This is necessary because if this method returns nil, a selector not
  // found exception is raised. The string argument to -signatureWithObjCTypes: outlines the return
  // type and arguments to the message. To return a dud NSMethodSignature, pretty much any signature
  // will suffice. Since the -forwardInvocation call will do nothing if the delegate does not
  // respond to the selector, the dud NSMethodSignature simply gets us around the exception.
  return methodSignature ?: [NSMethodSignature signatureWithObjCTypes:"@^v^c"];
}

- (void)forwardInvocation:(NSInvocation *)invocation {
  // If we are down here this means _interceptor and _target where nil. Just don't do anything to
  // prevent a crash
}
