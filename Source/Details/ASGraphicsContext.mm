...
void ASGraphicsBeginImageContextWithOptions(CGSize size, BOOL opaque, CGFloat scale) {
  ...
  dispatch_once(&onceToken, ^{
    ...
    ASDisplayNodeCAssert(CGBitmapContextGetBytesPerRow(refCtxOpaque) == 32,
                         @"Expected bytes per row to be aligned to 32. Has "
                         @"CGBitmapGetAlignedBytesPerRow implementation changed?");
  ...
  CGContextRef context =
      CGBitmapContextCreate(buffer.mutableBytes, intWidth, intHeight, bitsPerComponent, bytesPerRow,
...
UIImage *_Nullable ASGraphicsGetImageAndEndCurrentContext() NS_RETURNS_RETAINED {
  ...
  ASCGImageBuffer *buffer =
      objc_getAssociatedObject((__bridge id)context, &__contextDataAssociationKey);
  ...
  CGImageRef cgImg = CGImageCreate(
      ...
      CGBitmapContextGetBytesPerRow(context), imageColorSpace,
  ...
  UIImage *result = [[UIImage alloc] initWithCGImage:cgImg
                                               scale:scale
