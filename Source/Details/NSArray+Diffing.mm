...
- (void)asdk_diffWithArray:(NSArray *)array
  ...
  NSMutableIndexSet *commonIndexes = [self _asdk_commonIndexesWithArray:array
                                                           compareBlock:comparison];
    ...
    for (NSUInteger i = 0, j = 0; j < array.count; j++) {
      ...
      if (i < commonObjects.count && j < array.count && comparison(commonObjects[i], array[j])) {
        ...
        if (movedFrom != NSNotFound) {
          // moves will coalesce a delete / insert - the insert is just not done, and here we remove
          // the delete:
...
// https://github.com/raywenderlich/swift-algorithm-club/tree/master/Longest%20Common%20Subsequence
// is not exactly this code (obviously), but is a good commentary on the algorithm.
- (NSMutableIndexSet *)_asdk_commonIndexesWithArray:(NSArray *)array
