...
- (instancetype)init {
  ...
  _displayLink = [CADisplayLink displayLinkWithTarget:self
                                             selector:@selector(_updateCountDisplayLinkDidFire)];
...
- (ASInterfaceState)interfaceState {
  ...
  if (__ApplicationState == UIApplicationStateBackground) {
    // If the app is background, pretend to be invisible so that we inform each cell it is no longer
...
- (void)_updateVisibleNodeIndexPaths {
  ...
  // TODO: Consider if we need to use this codepath, or can rely on something more similar to the
  // data & display ranges Example: ... = [_layoutController indexPathsForScrolling:scrollDirection
  // rangeType:ASLayoutRangeTypeVisible];
  auto visibleElements = [_dataSource visibleElementsForRangeController:self];
  NSHashTable *newVisibleNodes =
  ...
  if (visibleElements.count ==
    ...
    // This allows us to avoid thrashing through -didExitVisibleState in the case of -reloadData,
    // since that generates didEndDisplayingCell calls. Those didEndDisplayingCell calls result in
    // items being removed from the visibleElements returned by the _dataSource, even though the
    ...
    return;  // don't do anything for this update, but leave _rangeIsValid == NO to make sure we
             // update it later
  }
  ...
  // If we've never scrolled before, we never update the range mode, so it doesn't jump into Full
  // too early. This can happen if we have multiple, noisy updates occurring from application code
  // before the user has engaged. If the range mode is explicitly set via
  // updateCurrentRangeWithMode:, we'll preserve that for at least one update cycle. Once the user
  // has scrolled and the range is visible, we'll always resume managing the range mode
  // automatically.
  if ((updateRangeMode && ASInterfaceStateIncludesVisible(selfInterfaceState)) ||
      [[self class] isFirstRangeUpdateForRangeMode:rangeMode]) {
    rangeMode = [ASRangeController rangeModeForInterfaceState:selfInterfaceState
                                             currentRangeMode:_currentRangeMode];
  }

  ASRangeTuningParameters parametersPreload =
      [_layoutController tuningParametersForRangeMode:rangeMode rangeType:ASLayoutRangeTypePreload];
  ASRangeTuningParameters parametersDisplay =
      [_layoutController tuningParametersForRangeMode:rangeMode rangeType:ASLayoutRangeTypeDisplay];

  // Preload can express the ultra-low-memory state with 0, 0 returned for its tuningParameters
  // above, and will match Visible. However, in this rangeMode, Display is not supposed to contain
  // *any* paths -- not even the visible bounds. TuningParameters can't express this.
  BOOL emptyDisplayRange = (rangeMode == ASLayoutRangeModeLowMemory);
  BOOL equalDisplayPreload =
      ASRangeTuningParametersEqualToRangeTuningParameters(parametersDisplay, parametersPreload);
  BOOL equalDisplayVisible = (ASRangeTuningParametersEqualToRangeTuningParameters(
                                  parametersDisplay, ASRangeTuningParametersZero) &&
                              emptyDisplayRange == NO);

  // Check if both Display and Preload are unique. If they are, we load them with a single fetch
  // from the layout controller for performance.
  ...
  if (optimizedLoadingOfBothRanges) {
    ...
    if (equalDisplayVisible == YES) {
      ...
      // Calculating only the Display range means the Preload range is either the same as Display or
      // Visible.
    ...
    BOOL equalPreloadVisible = ASRangeTuningParametersEqualToRangeTuningParameters(
        parametersPreload, ASRangeTuningParametersZero);
  ...
  NSSet<NSIndexPath *> *visibleIndexPaths = ASSetByFlatMapping(
      visibleElements, ASCollectionElement * element, [map indexPathForElementIfCell:element]);
  NSSet<NSIndexPath *> *displayIndexPaths = ASSetByFlatMapping(
      displayElements, ASCollectionElement * element, [map indexPathForElementIfCell:element]);
  NSSet<NSIndexPath *> *preloadIndexPaths = ASSetByFlatMapping(
      preloadElements, ASCollectionElement * element, [map indexPathForElementIfCell:element]);

  // Prioritize the order in which we visit each.  Visible nodes should be updated first so they are
  // enqueued on the network or display queues before preloading (offscreen) nodes are enqueued.
  NSMutableOrderedSet<NSIndexPath *> *allIndexPaths =
      [[NSMutableOrderedSet alloc] initWithSet:visibleIndexPaths];

  // Typically the preloadIndexPaths will be the largest, and be a superset of the others, though it
  // may be disjoint. Because allIndexPaths is an NSMutableOrderedSet, this adds the non-duplicate
  // items /after/ the existing items. This means that during iteration, we will first visit
  // visible, then display, then preload nodes.
  [allIndexPaths unionSet:displayIndexPaths];
  [allIndexPaths unionSet:preloadIndexPaths];

  // Add anything we had applied interfaceState to in the last update, but is no longer in range, so
  // we can clear any range flags it still has enabled.  Most of the time, all but a few elements
  // are equal; a large programmatic scroll or major main thread stall could cause entirely disjoint
  // sets.  In either case we must visit all. Calling "-set" on NSMutableOrderedSet just references
  // the underlying mutable data store, so we must copy it.
  ...
  NSMutableArray<NSIndexPath *> *modifiedIndexPaths =
      (ASRangeControllerLoggingEnabled ? [NSMutableArray array] : nil);
#endif

  for (NSIndexPath *indexPath in allIndexPaths) {
    // Before a node / indexPath is exposed to ASRangeController, ASDataController should have
    // already measured it. For consistency, make sure each node knows that it should measure itself
    ...
    if (ASInterfaceStateIncludesVisible(selfInterfaceState)) {
      if ([visibleIndexPaths containsObject:indexPath]) {
        interfaceState |=
            (ASInterfaceStateVisible | ASInterfaceStateDisplay | ASInterfaceStatePreload);
      } else {
      ...
      // If selfInterfaceState isn't visible, then visibleIndexPaths represents what /will/ be
      // immediately visible at the instant we come onscreen.  So, preload and display all of those
      // things, but don't waste resources preloading yet. We handle this as a separate case to
      // minimize set operations for offscreen preloading, including containsObject:.
      ...
      if ([allCurrentIndexPaths containsObject:indexPath]) {
        ...
        // our overall container object is itself not visible yet.  The moment it becomes visible,
        // we will run the condition above
        ...
        if (rangeMode != ASLayoutRangeModeLowMemory) {
          // Add Display.
          // We might be looking at an indexPath that was previously in-range, but now we need to
          // clear it. In that case we'll just set it back to MeasureLayout.  Only set Display |
          // Preload if in allCurrentIndexPaths.
          interfaceState |= ASInterfaceStateDisplay;
        }
      }
    }
    ...
    if (node != nil) {
      ASDisplayNodeAssert(node.hierarchyState & ASHierarchyStateRangeManaged,
                          @"All nodes reaching this point should be range-managed, or "
                          @"interfaceState may be incorrectly reset.");
      if (ASInterfaceStateIncludesVisible(interfaceState)) {
        [newVisibleNodes addObject:node];
      }
      // Skip the many method calls of the recursive operation if the top level cell node already
      // has the right interfaceState.
      if (node.pendingInterfaceState != interfaceState) {
#if ASRangeControllerLoggingEnabled
        [modifiedIndexPaths addObject:indexPath];
#endif

        BOOL nodeShouldScheduleDisplay =
            [node shouldScheduleDisplayWithNewInterfaceState:interfaceState];
  ...
  // TODO: This code is for debugging only, but would be great to clean up with a delegate method
  // implementation.
  if (ASDisplayNode.shouldShowRangeDebugOverlay) {
...
#pragma clang diagnostic ignored "-Wundeclared-selector"
      scrollableDirections =
          (ASScrollDirection)[_dataSource performSelector:@selector(scrollableDirections)];
 ...
 * go ahead and schedule the transition as soon as all the currently-scheduled rendering is done
 * #1163.
 */
- (void)registerForNodeDisplayNotificationsForInterfaceStateIfNeeded:
    (ASInterfaceState)interfaceState {
  ...
  if (!_didRegisterForNodeDisplayNotifications && _currentRangeMode != ASLayoutRangeModeFull) {
    ASLayoutRangeMode nextRangeMode =
        [ASRangeController rangeModeForInterfaceState:interfaceState
                                     currentRangeMode:_currentRangeMode];
...
- (void)scheduledNodesDidDisplay:(NSNotification *)notification {
  CFAbsoluteTime notificationTimestamp =
      ((NSNumber *)notification.userInfo[ASRenderingEngineDidDisplayNodesScheduledBeforeTimestamp])
          .doubleValue;
  if (_pendingDisplayNodesTimestamp < notificationTimestamp) {
    // The rendering engine has processed all the nodes this range controller scheduled. Let's
    // schedule a range update
    [[NSNotificationCenter defaultCenter]
        removeObserver:self
                  name:ASRenderingEngineDidDisplayScheduledNodesNotification
                object:nil];
...
- (void)setTuningParameters:(ASRangeTuningParameters)tuningParameters
  ...
  [_layoutController setTuningParameters:tuningParameters
...
// Skip the many method calls of the recursive operation if the top level cell node already has the
...
+ (void)setRangeModeForMemoryWarnings:(ASLayoutRangeMode)rangeMode {
  ASDisplayNodeAssert(
      rangeMode == ASLayoutRangeModeVisibleOnly || rangeMode == ASLayoutRangeModeLowMemory,
      @"It is highly inadvisable to engage a larger range mode when a memory warning occurs, as "
      @"this will almost certainly cause app eviction");
...
+ (void)didReceiveMemoryWarning:(NSNotification *)notification {
  ...
  for (ASRangeController *rangeController in allRangeControllers) {
    ...
    [rangeController updateCurrentRangeWithMode:isDisplay ? ASLayoutRangeModeVisibleOnly
                                                          : __rangeModeForMemoryWarnings];
...
+ (void)didEnterBackground:(NSNotification *)notification {
  NSArray *allRangeControllers = [[self allRangeControllersWeakSet] allObjects];
  for (ASRangeController *rangeController in allRangeControllers) {
    // We do not want to fully collapse the Display ranges of any visible range controllers so that
    // flashes can be avoided when the app is resumed.  Non-visible controllers can be more
    // aggressively culled to the LowMemory state (see definitions for documentation)
    BOOL isVisible = ASInterfaceStateIncludesVisible([rangeController interfaceState]);
    [rangeController updateCurrentRangeWithMode:isVisible ? ASLayoutRangeModeVisibleOnly
                                                          : ASLayoutRangeModeLowMemory];
  }

  // Because -interfaceState checks __ApplicationState and always clears the "visible" bit if
  // Backgrounded, we must set this after updating the range mode.
  __ApplicationState = UIApplicationStateBackground;
  for (ASRangeController *rangeController in allRangeControllers) {
    // Trigger a range update immediately, as we may not be allowed by the system to run the update
    // block scheduled by changing range mode. There's no need to call needs update as
    // updateCurrentRangeWithMode sets this if necessary.
    [rangeController updateIfNeeded];
  }

#if ASRangeControllerLoggingEnabled
  NSLog(@"+[ASRangeController didEnterBackground] with controllers, after backgrounding: %@",
        allRangeControllers);
#endif
}
...
+ (void)willEnterForeground:(NSNotification *)notification {
  ...
  for (ASRangeController *rangeController in allRangeControllers) {
    BOOL isVisible = ASInterfaceStateIncludesVisible([rangeController interfaceState]);
    [rangeController updateCurrentRangeWithMode:isVisible ? ASLayoutRangeModeMinimum
...
#if ASRangeControllerLoggingEnabled
  NSLog(@"+[ASRangeController willEnterForeground] with controllers, after foregrounding: %@",
        allRangeControllers);
#endif
}
...
- (void)_updateCountDisplayLinkDidFire {
  if (_updateCountThisFrame > 1) {
    NSLog(@"ASRangeController %p updated %lu times this frame.", self,
          (unsigned long)_updateCountThisFrame);
  }
...
- (NSString *)descriptionWithIndexPaths:(NSArray<NSIndexPath *> *)indexPaths {
  NSMutableString *description = [NSMutableString
      stringWithFormat:@"%@ %@", [super description], @" allPreviousIndexPaths:\n"];
  for (NSIndexPath *indexPath in indexPaths) {
    ASDisplayNode *node =
        [[_dataSource elementMapForRangeController:self] elementForItemAtIndexPath:indexPath]
            .nodeIfAllocated;
