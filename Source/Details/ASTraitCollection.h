 ...
 *
 * @discussion This struct is for internal use only. Framework users should always use
 * ASTraitCollection.
 *
 * If you use ASPrimitiveTraitCollection, please do make sure to initialize it with
 * ASPrimitiveTraitCollectionMakeDefault() or
 * ASPrimitiveTraitCollectionFromUITraitCollection(UITraitCollection*).
 */
 ...
 */
AS_EXTERN BOOL ASPrimitiveTraitCollectionIsEqualToASPrimitiveTraitCollection(
    ASPrimitiveTraitCollection lhs, ASPrimitiveTraitCollection rhs);
...
/**
 * This function will walk the layout element hierarchy and updates the layout element trait
 * collection for every layout element within the hierarchy.
 */
AS_EXTERN void ASTraitCollectionPropagateDown(id<ASLayoutElement> element,
 ...
 *
 * @discussion This only exists as an internal convenience method. Users should access the trait
 * collections through the NSObject based asyncTraitCollection API
 */
 ...
 *
 * @discussion This only exists as an internal convenience method. Users should not override trait
 * collection using it. Use [ASViewController overrideDisplayTraitsWithTraitCollection] block
 * instead.
 */
...
#define ASLayoutElementCollectionTableSetTraitCollection(lock)                                     \
  -(void)setPrimitiveTraitCollection : (ASPrimitiveTraitCollection)traitCollection {               \
    ...
    /* Extra Trait Collection Handling */                                                          \
                                                                                                   \
    /* If the node is not loaded  yet don't do anything as otherwise the access of the view will   \
     * trigger a load */                                                                           \
    ...
    if (ASPrimitiveTraitCollectionIsEqualToASPrimitiveTraitCollection(currentTraits, oldTraits) == \
        NO) {                                                                                      \
...
+ (ASTraitCollection *)traitCollectionWithASPrimitiveTraitCollection:
    (ASPrimitiveTraitCollection)traits NS_RETURNS_RETAINED;
