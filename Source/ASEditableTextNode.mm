...
- (instancetype)init {
  return [self initWithTextKitComponents:[ASTextKitComponents
                                             componentsWithAttributedSeedString:nil
...
- (void)didLoad {
  ...
  void (^configureTextView)(UITextView *) = ^(UITextView *textView) {
    if (!_displayingPlaceholder || textView != _textKitComponents.textView) {
      // If showing the placeholder, don't propagate backgroundColor/opaque to the editable
      // textView.  It is positioned over the placeholder to accept taps to begin editing, and if
      // it's opaque/colored then it'll obscure the placeholder.
      textView.backgroundColor = self.backgroundColor;
      textView.opaque = self.opaque;
    } else if (_displayingPlaceholder && textView == _textKitComponents.textView) {
      // The default backgroundColor for a textView is white.  Due to the reason described above,
      // make sure the editable textView starts out transparent.
  ...
  _placeholderTextKitComponents.textView = [[ASTextKitComponentsTextView alloc]
  ...
  _textKitComponents.textView =
      [[ASPanningOverriddenUITextView alloc] initWithFrame:CGRectZero
...
- (CGSize)calculateSizeThatFits:(CGSize)constrainedSize {
  ...
  CGFloat height =
      std::ceil(textSize.height + _textContainerInset.top + _textContainerInset.bottom);
  return CGSizeMake(std::fmin(width, constrainedSize.width),
                    std::fmin(height, constrainedSize.height));
}
...
- (void)setBackgroundColor:(UIColor *)backgroundColor {
  ...
  // If showing the placeholder, don't propagate backgroundColor/opaque to the editable textView. It
  // is positioned over the placeholder to accept taps to begin editing, and if it's opaque/colored
  // then it'll obscure the placeholder. The backgroundColor/opaque will be propagated to the
  // editable textView when editing begins.
...
- (void)setLayerBacked:(BOOL)layerBacked {
  ASDisplayNodeAssert(
      !layerBacked,
      @"Cannot set layerBacked to YES on ASEditableTextNode – instances must be view-backed in "
      @"order to ensure touch events can be passed to the internal UITextView during editing.");
...
- (void)setAttributedPlaceholderText:(NSAttributedString *)attributedPlaceholderText {
  ...
  [_placeholderTextKitComponents.textStorage
      setAttributedString:attributedPlaceholderText
...
- (void)setAttributedText:(NSAttributedString *)attributedText {
  ...
  // If we (_cmd) are called while the text view itself is updating (-textViewDidUpdate:), you
  // cannot update the text storage and expect perfect propagation to the text view. Thus, we always
  // update the textview directly if it's been created already.
  if (ASObjectIsEqual(
          (_textKitComponents.textView.attributedText ?: _textKitComponents.textStorage),
          attributedText))
    return;

  // If the cursor isn't at the end of the text, we need to preserve the selected range to avoid
  // moving the cursor.
  ...
  // Otherwise, note that we don't simply nil out attributed text. Because the insertion point is
  // guided by the attributes at index 0, we need to attribute an empty string to ensure the insert
  // point obeys our typing attributes.
  else
    attributedStringToDisplay = [[NSAttributedString alloc] initWithString:@""
  ...
  if (preserveSelectedRange) {
    _isPreservingSelection = YES;  // Used in -textViewDidChangeSelection: to avoid informing our
                                   // delegate about our preservation.
...
- (void)_updateDisplayingPlaceholder {
  ...
  // If hiding the placeholder, propagate backgroundColor/opaque to the editable textView.  It is
  // positioned over the placeholder to accept taps to begin editing, and was kept transparent so it
  // doesn't obscure the placeholder text.  Now that we're editing it and the placeholder is hidden,
  // we can make it opaque to avoid unnecessary blending.
  if (!_displayingPlaceholder) {
...
- (void)_layoutTextView {
  ...
  // Note that both of these won't be necessary once we can disable scrolling, pending
  // rdar://14729288 When we resize to fit (above) the prior layout becomes invalid. For whatever
  // reason, UITextView doesn't invalidate its layout when its frame changes on its own, so we have
  // to do so ourselves.
  ...
  // When you type beyond UITextView's bounds it scrolls you down a line. We need to remain at the
  // top.
...
- (void)textViewDidChange:(UITextView *)textView {
  ...
  // This is used by _delegateDidChangeSelectionFromSelectedRange:toSelectedRange: to distinguish
  // between selection changes that happen because of editing or pure selection changes.
...
- (void)textViewDidChangeSelection:(UITextView *)textView {
  // Typing attributes get reset when selection changes. Reapply them so they actually obey our
  // header.
  _textKitComponents.textView.typingAttributes = _typingAttributes;
  ...
  // Delegateify.
  [self _delegateDidChangeSelectionFromSelectedRange:fromSelectedRange
...
- (CGRect)frameForTextRange:(NSRange)textRange {
  ...
  NSRange glyphRange = [_textKitComponents.layoutManager glyphRangeForCharacterRange:textRange
                                                                actualCharacterRange:NULL];
  CGRect textRect =
      [_textKitComponents.layoutManager boundingRectForGlyphRange:glyphRange
...
- (BOOL)_delegateShouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text {
  if ([_delegate respondsToSelector:@selector(editableTextNode:
...
- (void)_delegateDidChangeSelectionFromSelectedRange:(NSRange)fromSelectedRange
  ...
  // 1. UITextView invokes its delegate methods when it's in the middle of text-processing. For
  // example, -textViewDidChange: is invoked before you can truly rely on the changes being
  // propagated throughout the Text Kit hierarchy.
  // 2. This delegate method (-textViewDidChangeSelection:) is called both before
  // -textViewDidChange: and before the layout manager/etc. has necessarily generated+laid out its
  // glyphs. Because of the former, we need to wait until -textViewDidChange: has had an opportunity
  // to be called so can accurately determine whether this selection change is due to editing
  // (_selectionChangedForEditedText). Thus, to avoid calling out to client code in the middle of
  // UITextView's processing, we call the delegate on the next run of the runloop, when all such
  // internal processing is surely done.
  dispatch_async(dispatch_get_main_queue(), ^{
    if ([_delegate respondsToSelector:@selector
                   (editableTextNodeDidChangeSelection:
...
- (void)_delegateDidUpdateText {
  // Note that because -editableTextNodeDidUpdateText: passes no state, the current state of the
  // receiver will be accessed. Thus, it's not useful to enqueue a second delegation call if the
  // first hasn't happened yet -- doing so will result in the delegate receiving
  // -editableTextNodeDidUpdateText: when the "updated text" has already been processed. This may
  // sound innocuous, but because our delegation may cause additional updates to the textview's
  // string, and because such updates discard spelling suggestions and autocompletions (like
  // double-space to `.`), it can actually be quite dangerous!
  if (_delegateDidUpdateEnqueued) return;

  _delegateDidUpdateEnqueued = YES;

  // UITextView invokes its delegate methods when it's in the middle of text-processing. For
  // example, -textViewDidChange: is invoked before you can truly rely on the changes being
  // propagated throughout the Text Kit hierarchy. Thus, to avoid calling out to client code in the
  // middle of UITextView's processing, we call the delegate on the next run of the runloop, when
  // all such internal processing is surely done.
  dispatch_async(dispatch_get_main_queue(), ^{
...
- (NSInteger)accessibilityElementCount {
  if (!self.isNodeLoaded) {
    ASDisplayNodeFailAssert(
        @"Cannot access accessibilityElementCount since ASEditableTextNode is not loaded");
    return 0;
  }
  return 1;
}
...
- (NSArray *)accessibilityElements {
  if (!self.isNodeLoaded) {
    ASDisplayNodeFailAssert(
        @"Cannot access accessibilityElements since ASEditableTextNode is not loaded");
    return @[];
  }
...
- (id)accessibilityElementAtIndex:(NSInteger)index {
  if (!self.isNodeLoaded) {
    ASDisplayNodeFailAssert(
        @"Cannot access accessibilityElementAtIndex: since ASEditableTextNode is not loaded");
    return nil;
  }
  return self.textView;
}
