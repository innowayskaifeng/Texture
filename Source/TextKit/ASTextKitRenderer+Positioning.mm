...
- (NSArray *)rectsForTextRange:(NSRange)textRange
  ...
  [self.context performBlockWithLockedTextKitComponents:^(NSLayoutManager *layoutManager,
...
/**
 Helper function that should be called within performBlockWithLockedTextKitComponents: in an already
 locked state to prevent a deadlock
 */
- (NSArray *)unlockedRectsForTextRange:(NSRange)textRange
  ...
  NSRange totalGlyphRange = [layoutManager glyphRangeForCharacterRange:clampedRange
                                                  actualCharacterRange:NULL];

  [layoutManager
      enumerateLineFragmentsForGlyphRange:totalGlyphRange
                               usingBlock:^(CGRect rect, CGRect usedRect,
                                            NSTextContainer *innerTextContainer, NSRange glyphRange,
                                            BOOL *stop) {
                                 CGRect lineRect = CGRectNull;
                                 // If we're empty, don't bother looping through glyphs, use the
                                 // default.
                                 if (CGRectIsEmpty(usedRect)) {
                                   lineRect = usedRect;
                                 } else {
                                   // TextKit's bounding rect computations are just a touch off, so
                                   // we actually compose the rects by hand from the center of the
                                   // given TextKit bounds and imposing the font attributes returned
                                   // by the glyph's font.
                                   NSRange lineGlyphRange =
                                       NSIntersectionRange(totalGlyphRange, glyphRange);
                                   for (NSUInteger i = lineGlyphRange.location;
                                     ...
                                     CGRect properGlyphRect =
                                         [self _internalRectForGlyphAtIndex:i
                                     ...
                                     if (!CGRectIsEmpty(properGlyphRect)) {
                                       lineRect = CGRectIsNull(lineRect)
                                                      ? properGlyphRect
                                 ...
                                 if (!CGRectIsNull(lineRect)) {
                                   if (measureOption == ASTextKitRendererMeasureOptionBlock) {
                                     // For the block measurement option we store the first & last
                                     // rect as special cases, then merge everything else into a
                                     // single block rect
                                     if (CGRectIsNull(firstRect)) {
                                       // We don't have a firstRect, so we must be on the first
                                       // line.
                                       firstRect = lineRect;
                                     } else if (CGRectIsNull(lastRect)) {
                                       // We don't have a lastRect, but we do have a firstRect, so
                                       // we must be on the second line.  No need to merge in the
                                       // blockRect just yet
                                       lastRect = lineRect;
                                     } else if (CGRectIsNull(blockRect)) {
                                       // We have both a first and last rect, so we must be on the
                                       // third line we don't have any blockRect to merge it into,
                                       // so we just set it directly.
                                       blockRect = lastRect;
                                       lastRect = lineRect;
                                     } else {
                                       // Everything is already set, so we just merge this line into
                                       // the block.
                                       blockRect = CGRectUnion(blockRect, lastRect);
                                       lastRect = lineRect;
                                     }
                                   } else {
                                     // If the block option isn't being used then each line is being
    ...
    if (!CGRectIsNull(firstRect)) {
      if (!CGRectIsNull(blockRect)) {
        ...
        blockRect.size.width +=
            MAX(CGRectGetMaxX(firstRect), CGRectGetMaxX(lastRect)) - CGRectGetMaxX(blockRect);
      }
      ...
      [mutableTextRects
          addObject:[NSValue valueWithCGRect:[self.shadower offsetRectWithInternalRect:firstRect]]];
    }
    if (!CGRectIsNull(blockRect)) {
      [mutableTextRects
          addObject:[NSValue valueWithCGRect:[self.shadower offsetRectWithInternalRect:blockRect]]];
    }
    if (!CGRectIsNull(lastRect)) {
      [mutableTextRects
          addObject:[NSValue valueWithCGRect:[self.shadower offsetRectWithInternalRect:lastRect]]];
    }
  }
  ...
  [self enumerateTextIndexesAtPosition:position
                            usingBlock:^(NSUInteger characterIndex, CGRect glyphBoundingRect,
                              ...
                              CGFloat currentDistance =
                                  std::sqrt(std::pow(position.x - glyphLocation.x, 2.f) +
...
/**
 Measured from the internal coordinate space of the context, not accounting for shadow offsets.
 Actually uses CoreText as an approximation to work around problems in TextKit's glyph sizing.
 */
- (CGRect)_internalRectForGlyphAtIndex:(NSUInteger)glyphIndex
  ...
  //                             |                         |
  // +------------------------+--|-------------------------|--+-----------+-----+ What TextKit
  // returns sometimes |                        |  |             XXXXXXXXXXX +  |           |     |
  // (approx. correct height, but |               ---------|--+---------+  XXX       XXXX
  // +|-----------|-----|  sometimes inaccurate bounding |               |        |             XXX
  // XXXXX|           |     |  widths) |               |        |             XX             XX | |
  // | |               |        |            XX                 |           |     | | |        | XXX
  // |           |     | |               |        |           XX                  |           | | |
  // |        |      XXXXXXXXXXX              |           |     | |   Cap Height->|        | XX | |
  // | |               |        |          XX                   |  Ascent-->|     | | |        | XX
  // |           |     | |               |        |          XX                   |           | | |
  // |        |          X                    |           |     | |               |        | X | | |
  // |               |        |          X                    |           |     |
  ...
  // If it is a NSTextAttachment, we don't have the matched glyph and use width of glyphRect instead
  // of advance.
  CGFloat advance =
  ...
  if (measureOption == ASTextKitRendererMeasureOptionCapHeight ||
    ...
    properGlyphRect =
        CGRectMake(glyphCenter.x - advance * 0.5,
    ...
    properGlyphRect = CGRectMake(glyphCenter.x - advance * 0.5, glyphRect.origin.y, advance,
...
- (void)enumerateTextIndexesAtPosition:(CGPoint)externalPosition
                            usingBlock:(as_text_component_index_block_t)block {
  // This method is a little complex because it has to call out to client code from inside an
  // enumeration that needs to achieve a lock on the textkit components.  It cannot call out to
  // client code from within that lock so we just perform the textkit-locked ops inside the locked
  // context.
  ...
  [lockingContext performBlockWithLockedTextKitComponents:^(NSLayoutManager *layoutManager,
                                                            NSTextStorage *textStorage,
                                                            NSTextContainer *textContainer) {
    invalidPosition = internalPosition.x > textContainer.size.width ||
                      internalPosition.y > textContainer.size.height || block == NULL;
  }];
  if (invalidPosition) {
  ...
  // We break it up into a 44pt box for the touch, and find the closest link attribute-containing
  // glyph to the center of the touch.
  ...
  // These are for tracking which point we're on.  We start with -pointsOnASide/2 and go to
  // pointsOnASide/2.  So if pointsOnASide=3, we go from -1 to 1.
  ...
  for (NSInteger i = startIndex; i <= endIndex && !stop; i++) {
    for (NSInteger j = startIndex; j <= endIndex && !stop; j++) {
      ...
      // Sometimes TextKit plays jokes on us and returns glyphs that really aren't close to the
      // point in question. Silly TextKit...
      if (!isValidGlyph ||
          !CGRectContainsPoint(CGRectInset(glyphRect, -ASTextKitRendererGlyphTouchHitSlop,
...
- (CGRect)frameForTextRange:(NSRange)textRange {
  ...
  [self.context performBlockWithLockedTextKitComponents:^(NSLayoutManager *layoutManager,
    ...
    NSRange glyphRange = [layoutManager glyphRangeForCharacterRange:textRange
                                               actualCharacterRange:NULL];
