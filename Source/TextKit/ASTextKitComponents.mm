...
+ (instancetype)componentsWithAttributedSeedString:(NSAttributedString *)attributedSeedString
  ...
  NSTextStorage *textStorage =
      attributedSeedString ? [[NSTextStorage alloc] initWithAttributedString:attributedSeedString]
...
- (CGSize)sizeForConstrainedWidth:(CGFloat)constrainedWidth {
  ...
  // If our text-view's width is already the constrained width, we can use our existing TextKit
  // stack for this sizing calculation. Otherwise, we create a temporary stack to size for
  // `constrainedWidth`.
  if (CGRectGetWidth(components.textView.threadSafeBounds) != constrainedWidth) {
    components = [ASTextKitComponents
        componentsWithAttributedSeedString:components.textStorage
  ...
  CGSize textSize =
      [components.layoutManager usedRectForTextContainer:components.textContainer].size;
...
- (CGSize)sizeForConstrainedWidth:(CGFloat)constrainedWidth
  ...
  CGFloat width =
      [components.layoutManager usedRectForTextContainer:components.textContainer].size.width;
