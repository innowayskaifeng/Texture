...
static NSCharacterSet *_defaultAvoidTruncationCharacterSet() {
  ...
  dispatch_once(&onceToken, ^{
    ...
    [mutableCharacterSet
        formUnionWithCharacterSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
...
@synthesize attributes = _attributes, context = _context, shadower = _shadower,
            truncater = _truncater, fontSizeAdjuster = _fontSizeAdjuster;
...
- (instancetype)initWithTextKitAttributes:(const ASTextKitAttributes &)attributes
  ...
  if (self = [super init]) {
    ...
    NSCharacterSet *avoidTailTruncationSet =
        attributes.avoidTailTruncationSet ?: _defaultAvoidTruncationCharacterSet();
    _truncater =
        [[ASTextKitTailTruncater alloc] initWithContext:[self context]
...
- (NSStringDrawingContext *)stringDrawingContext {
  ...
  if (isinf(_constrainedSize.width) == NO && _attributes.maximumNumberOfLines > 0) {
    ASDisplayNodeAssert(_attributes.maximumNumberOfLines != 1,
                        @"Max line count 1 is not supported in fast-path.");
    [stringDrawingContext setValue:@(_attributes.maximumNumberOfLines)
                            forKey:@"maximumNumberOfLines"];
  }
...
- (void)_calculateSize {
  // if we have no scale factors or an unconstrained width, there is no reason to try to adjust the
  // font size
  ...
  // If we do not scale, do exclusion, or do custom truncation, we should just use
  // NSAttributedString drawing for a fast-path.
  if (self.canUseFastPath) {
    CGRect rect =
        [_attributes.attributedString boundingRectWithSize:_constrainedSize
  ...
  if (isScaled) {
    // apply the string scale before truncating or else we may truncate the string after we've done
    // the work to shrink it.
  ...
  // TextKit often returns incorrect glyph bounding rects in the horizontal direction, so we clip to
  // our bounding rect to make sure our width calculations aren't being offset by glyphs going
  // beyond the constrained rect.
...
- (void)drawInContext:(CGContextRef)context bounds:(CGRect)bounds;
{
  ...
  if (self.canUseFastPath) {
    ...
    [[self context] performBlockWithLockedTextKitComponents:^(NSLayoutManager *layoutManager,
      ...
      LOG(@"usedRect: %@",
          NSStringFromCGRect([layoutManager usedRectForTextContainer:textContainer]));

      NSRange glyphRange =
          [layoutManager glyphRangeForBoundingRect:(CGRect){.size = textContainer.size}
                                   inTextContainer:textContainer];
      LOG(@"boundingRect: %@",
          NSStringFromCGRect([layoutManager boundingRectForGlyphRange:glyphRange
                                                      inTextContainer:textContainer]));
...
- (NSUInteger)lineCount {
  ...
  [[self context] performBlockWithLockedTextKitComponents:^(NSLayoutManager *layoutManager,
                                                            NSTextStorage *textStorage,
                                                            NSTextContainer *textContainer) {
    for (NSRange lineRange = {0, 0}; NSMaxRange(lineRange) < [layoutManager numberOfGlyphs];
         lineCount++) {
      [layoutManager lineFragmentRectForGlyphAtIndex:NSMaxRange(lineRange)
