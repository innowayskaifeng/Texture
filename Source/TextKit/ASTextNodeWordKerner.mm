...
- (NSUInteger)layoutManager:(NSLayoutManager *)layoutManager
  ...
  // If our typing attributes specify word kerning, specify the spaces as whitespace control
  // characters so we can customize their width. Are any of the characters spaces?
  ...
  // If we don't have any custom glyph properties, return 0 to indicate to the layout manager that
  // it should use the standard glyphs+properties.
  if (!newGlyphProperties) {
    if (usesWordKerning) {
      // If the text does use word kerning we have to make sure we return the correct glyphCount, or
      // the layout manager will just use the default properties and ignore our kerning.
...
- (NSControlCharacterAction)layoutManager:(NSLayoutManager *)layoutManager
  ...
  // If it's a space character and we have custom word kerning, use the whitespace action control
  // character.
...
- (CGRect)layoutManager:(NSLayoutManager *)layoutManager
  ...
  return CGRectMake(glyphPosition.x, glyphPosition.y, wordKernedSpaceWidth,
                    CGRectGetHeight(proposedRect));
}
...
- (CGFloat)_wordKernedSpaceWidthForCharacterAtIndex:(NSUInteger)characterIndex
  ...
  UIFont *font = [layoutManager.textStorage attribute:NSFontAttributeName
  ...
  // TextKit normally handles whitespace by increasing the advance of the previous glyph, rather
  // than displaying an actual glyph for the whitespace itself.  However, in order to implement word
  // kerning, we explicitly require a discrete glyph whose bounding box we can specify.  The problem
  // is that TextKit does not know this glyph is invisible. From TextKit's perspective, this
  // whitespace glyph is a glyph that MUST be displayed. Thus when it comes to determining
  // linebreaks, the width of this trailing whitespace glyph is considered. This causes our text to
  // wrap sooner than it otherwise would, as room is allocated at the end of each line for a glyph
  // that isn't actually visible.  To implement our desired behavior, we check to see if the current
  // whitespace glyph would break to the next line.  If it breaks to the next line, then this
  // constitutes trailing whitespace, and we specify enough room to fill up the remainder of the
  // line, but nothing more.
