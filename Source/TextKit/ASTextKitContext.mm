...

{
  if (self = [super init]) {
    ...
    // Concurrently initialising TextKit components crashes (rdar://18448377) so we use a global
    // lock.
    ...
    // Instead of calling [NSTextStorage initWithAttributedString:], setting attributedString just
    // after calling addlayoutManager can fix CJK language layout issues. See
