...
+ (ASTextKitShadower *)shadowerWithShadowOffset:(CGSize)shadowOffset
  ...
  BOOL hasShadow = shadowOpacity > 0 &&
                   (shadowRadius > 0 || CGSizeEqualToSize(shadowOffset, CGSizeZero) == NO) &&
...
- (BOOL)_shouldDrawShadow {
  return _shadowOpacity != 0.0 &&
         (_shadowRadius != 0 || !CGSizeEqualToSize(_shadowOffset, CGSizeZero)) &&
...
- (void)setShadowInContext:(CGContextRef)context {
  if ([self _shouldDrawShadow]) {
    ...
    if (textShadowOpacity != 1.0) {
      ...
      textShadowColor =
          CGColorCreateCopyWithAlpha(textShadowColor, inherentAlpha * textShadowOpacity);
...
- (CGRect)offsetRectWithInternalRect:(CGRect)internalRect {
  return (CGRect){.origin = [self offsetPointWithInternalPoint:internalRect.origin],
