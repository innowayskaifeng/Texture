...
+ (void)adjustFontSizeForAttributeString:(NSMutableAttributedString *)attrString
  ...
  [attrString
      enumerateAttributesInRange:NSMakeRange(0, attrString.length)
                         options:0
                      usingBlock:^(NSDictionary<NSString *, id> *_Nonnull attrs, NSRange range,
                                   BOOL *_Nonnull stop) {
                        ...
                        if (attrs[NSParagraphStyleAttributeName] != nil) {
                          NSMutableParagraphStyle *paragraphStyle =
                              [attrs[NSParagraphStyleAttributeName] mutableCopy];
                          paragraphStyle.lineSpacing = (paragraphStyle.lineSpacing * scaleFactor);
                          paragraphStyle.paragraphSpacing =
                              (paragraphStyle.paragraphSpacing * scaleFactor);
                          paragraphStyle.firstLineHeadIndent =
                              (paragraphStyle.firstLineHeadIndent * scaleFactor);
                          ...
                          paragraphStyle.minimumLineHeight =
                              (paragraphStyle.minimumLineHeight * scaleFactor);
                          paragraphStyle.maximumLineHeight =
                              (paragraphStyle.maximumLineHeight * scaleFactor);
...
- (NSUInteger)lineCountForString:(NSAttributedString *)attributedString {
  ...
  for (NSRange lineRange = {0, 0}; NSMaxRange(lineRange) < [sizingLayoutManager numberOfGlyphs] &&
                                   lineCount <= _attributes.maximumNumberOfLines;
       lineCount++) {
    [sizingLayoutManager lineFragmentRectForGlyphAtIndex:NSMaxRange(lineRange)
...
- (CGSize)boundingBoxForString:(NSAttributedString *)attributedString {
  ...
  [sizingLayoutManager ensureLayoutForTextContainer:sizingTextContainer];
  CGRect textRect =
      [sizingLayoutManager boundingRectForGlyphRange:NSMakeRange(0, [textStorage length])
...
- (NSLayoutManager *)sizingLayoutManager {
  ...
  if (_sizingLayoutManager == nil) {
    ...
    if (_sizingTextContainer == nil) {
      // make this text container unbounded in height so that the layout manager will compute the
      // total number of lines and not stop counting when height runs out.
      _sizingTextContainer =
          [[NSTextContainer alloc] initWithSize:CGSizeMake(_constrainedSize.width, CGFLOAT_MAX)];
...
- (CGFloat)scaleFactor {
  ...
  // We add the scale factor of 1 to our scaleFactors array so that in the first iteration of the
  // loop below, we are actually determining if we need to scale at all. If something doesn't fit,
  // we will continue to iterate our scale factors.
  NSArray *scaleFactors =
      [@[ @(1) ] arrayByAddingObjectsFromArray:_attributes.pointSizeScaleFactors];
  ...
  [_context performBlockWithLockedTextKitComponents:^(NSLayoutManager *layoutManager,
    ...
    // we may need to shrink for some reason, so let's iterate through our scale factors to see if
    // we actually need to shrink Note: the first scale factor in the array is 1.0 so will make sure
    // that things don't fit without shrinking
    for (NSNumber *adjustedScaleObj in scaleFactors) {
      ...
      if (longestWordFits == NO) {
        ...
        longestWordFits =
            std::ceil((longestWordSize.width * adjustedScale) <= self->_constrainedSize.width);
      }

      // if the longest word fits, go ahead and check max line and height. If it didn't fit continue
      // to the next scale factor
      if (longestWordFits == YES) {
        ...
        if (maxLinesFits == NO) {
          maxLinesFits =
              ([self lineCountForString:scaledString] <= self->_attributes.maximumNumberOfLines);
        }

        // if max lines still doesn't fit, continue without checking that we fit in the constrained
        // height
