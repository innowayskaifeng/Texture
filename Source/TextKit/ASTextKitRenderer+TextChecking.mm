...
{
  // Be explicit about the fact that we are overriding the super class' implementation of -range and
  // -resultType and substituting our own custom values. (We could use @synthesize to make these
  // ivars, but our linter correctly complains; it's weird to use @synthesize for properties that
  // are redeclared on top of an original declaration in the superclass. We only do it here because
  // NSTextCheckingResult doesn't expose an initializer, which is silly.)
...
- (NSTextCheckingResult *)textCheckingResultAtPoint:(CGPoint)point {
  ...
  if (truncationTokenRange.location == NSNotFound) {
    // The truncation string didn't specify a substring which should be highlighted, so we just
    // highlight it all
                          ...
                          usingBlock:^(NSUInteger index, CGRect glyphBoundingRect, BOOL *stop) {
                            if (index >= truncationTokenRange.location) {
                              ...
                              NSDictionary *attributes =
                                  [attributedString attributesAtIndex:index effectiveRange:&range];
                              ASTextKitEntityAttribute *entityAttribute =
                                  attributes[ASTextKitEntityAttributeName];
                              ...
                              if (entityAttribute && distance < minDistance) {
                                result = [[ASTextKitTextCheckingResult alloc]
                                       initWithType:ASTextKitTextCheckingTypeEntity
