...
/**
 * When shouldAutoplay is set to true, a video node will play when it has both loaded and entered
 * the "visible" interfaceState. If it leaves the visible interfaceState it will pause but will
 * resume once it has returned.
 */
 ...
 ** @discussion Setting the URL will override the current asset with a newly created AVURLAsset
 *created from the given URL, and AVAsset *asset will point to that newly created AVURLAsset. Please
 *don't set both assetURL and asset.
 ** @return Current URL the asset was initialized or nil if no URL was given.
 **/
@property(nullable, nonatomic) NSURL *assetURL;

/// You should never set any value on the backing video node. Use exclusivively the video player
...
/**
 * @abstract Delegate method invoked before creating default controls, asks delegate for custom
 * controls dictionary. This dictionary must constain only ASDisplayNode subclass objects.
 * @param videoPlayer The sender
 * @discussion - This method is invoked only when developer implements
 * videoPlayerNodeLayoutSpec:forControls:forMaximumSize: and gives ability to add custom constrols
 * to ASVideoPlayerNode, for example mute button.
 */
 ...
 * @discussion - Developer can layout whole ASVideoPlayerNode as he wants. ASVideoNode is locked and
 * it can't be changed
 */
 ...
 */
- (BOOL)videoPlayerNode:(ASVideoPlayerNode *)videoPlayer
    shouldChangeVideoNodeStateTo:(ASVideoNodePlayerState)state;
...
/**
 * @abstract Delegate method invoked when the ASVideoNode has constructed its AVPlayerItem for the
 * asset.
 * @param videoPlayer The video player node.
 * @param currentItem The AVPlayerItem that was constructed from the asset.
 */
- (void)videoPlayerNode:(ASVideoPlayerNode *)videoPlayer
      didSetCurrentItem:(AVPlayerItem *)currentItem;
 ...
 */
- (void)videoPlayerNode:(ASVideoPlayerNode *)videoPlayer
    didStallAtTimeInterval:(NSTimeInterval)timeInterval;
...
/**
 * @abstract Delegate method invoked when the ASVideoNode is done loading the asset and can start
 * the playback
 * @param videoPlayer The videoPlayer
 */
