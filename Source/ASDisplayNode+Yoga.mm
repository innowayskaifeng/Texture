...
- (void)setYogaChildren:(NSArray *)yogaChildren {
  ...
  for (ASDisplayNode *child in [_yogaChildren copy]) {
    ...
    // If this becomes a performance bottleneck, it can be optimized by not doing the NSArray
    // removals here.
...
- (void)semanticContentAttributeDidChange:(UISemanticContentAttribute)attribute {
  ...
  self.style.direction =
      (layoutDirection == UIUserInterfaceLayoutDirectionLeftToRight ? YGDirectionLTR
                                                                    : YGDirectionRTL);
}
...
- (void)setupYogaCalculatedLayout {
  ...
  for (ASLayout *sublayout in layout.sublayouts) {
    if (sublayout.sublayouts.count > 0 ||
        ASDynamicCast(sublayout.layoutElement, ASDisplayNode) == nil) {
      ASDisplayNodeAssert(NO, @"Yoga sublayout is not flattened! %@, %@", self, sublayout);
    }
  }
#endif

  // Because this layout won't go through the rest of the logic in calculateLayoutThatFits:, flatten
  // it now.
  ...
  // Setup _pendingDisplayNodeLayout to reference the Yoga-calculated ASLayout, *unless* we are a
  // leaf node. Leaf yoga nodes may have their own .sublayouts, if they use a layout spec (such as
  // ASButtonNode). Their _pending variable is set after passing the Yoga checks at the start of
  // -calculateLayoutThatFits:

  // For other Yoga nodes, there is no code that will set _pending unless we do it here. Why does it
  // need to be set? When CALayer triggers the -[ASDisplayNode __layout] call, we will check if our
  // current _pending layout has a size which matches our current bounds size. If it does, that
  // layout will be used without recomputing it.

  // NOTE: Yoga does not make the constrainedSize available to intermediate nodes in the tree (e.g.
  // not root or leaves). Although the size range provided here is not accurate, this will only
  // affect caching of calls to layoutThatFits: These calls will behave as if they are not cached,
  // starting a new Yoga layout pass, but this will tap into Yoga's own internal cache.
  ...
  if ([self shouldHaveYogaMeasureFunc] == NO) {
    ...
    // For the root node in a Yoga tree, make sure to preserve the constrainedSize originally
    // provided. This will be used for all relayouts triggered by children, since they escalate to
    // root.
    ASSizeRange range =
        parentNode ? ASSizeRangeUnconstrained : self.constrainedSizeForCalculatedLayout;
...
- (void)updateYogaMeasureFuncIfNeeded {
  ...
  // The YGNodeRef Context will retain the ASDisplayNode, which retains the style, which owns the
  // YGNodeRef.
  BOOL shouldHaveMeasureFunc =
      ([self shouldHaveYogaMeasureFunc] && checkFlag(YogaLayoutInProgress));
...
- (void)invalidateCalculatedYogaLayout {
  ...
  if (yogaNode && [self shouldHaveYogaMeasureFunc]) {
    // Yoga internally asserts that MarkDirty() may only be called on nodes with a measurement
    // function.
    ...
    if (needsTemporaryMeasureFunc) {
      ASDisplayNodeAssert(self.yogaLayoutInProgress == NO,
                          @"shouldHaveYogaMeasureFunc == YES, and inside a layout pass, but no "
...
- (void)calculateLayoutFromYogaRoot:(ASSizeRange)rootConstrainedSize {
  ...
  if (self != yogaRoot) {
    ...
    // TODO(appleguy): Consider how to get the constrainedSize for the yogaRoot when escalating
    // manually.
  ...
  // YGNodeCalculateLayout currently doesn't offer the ability to pass a minimum size (max is passed
  // there).
