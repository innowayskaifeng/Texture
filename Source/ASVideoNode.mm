...
- (void)imageAtTime:(CMTime)imageTime
    completionHandler:(void (^)(UIImage *image))completionHandler {
  ASPerformBlockOnBackgroundThread(^{
    AVAsset *asset = self.asset;

    // Skip the asset image generation if we don't have any tracks available that are capable of
    // supporting it
    NSArray<AVAssetTrack *> *visualAssetArray =
        ...
        generateCGImagesAsynchronouslyForTimes:@[ [NSValue valueWithCMTime:imageTime] ]
                             completionHandler:^(
                               ...
                               if (error != nil && result != AVAssetImageGeneratorCancelled) {
                                 NSLog(@"Asset preview image generation failed with error: %@",
                                       error);
                               }
  ...
  if (object == _currentPlayerItem) {
    if ([keyPath isEqualToString:kStatus]) {
      if ([change[NSKeyValueChangeNewKey] integerValue] == AVPlayerItemStatusReadyToPlay) {
        ...
        // If we don't yet have a placeholder image update it now that we should have data available
        // for it
        if (self.image == nil && self.URL == nil) {
          [self generatePlaceholderImage];
        }
      ...
      if (_shouldBePlaying && (_shouldAggressivelyRecoverFromStall || likelyToKeepUp) &&
        ...
        if (self.playerState == ASVideoNodePlayerStateLoading &&
...
- (void)setDelegate:(id<ASVideoNodeDelegate>)delegate {
  ...
  if (delegate == nil) {
    ...
    _delegateFlags.delegateVideoDidPlayToEnd =
        [delegate respondsToSelector:@selector(videoDidPlayToEnd:)];
    _delegateFlags.delegateDidTapVideoNode =
        [delegate respondsToSelector:@selector(didTapVideoNode:)];
    ...
    _delegateFlags.delegateVideoNodeDidPlayToTimeInterval =
        [delegate respondsToSelector:@selector(videoNode:didPlayToTimeInterval:)];
    ...
    _delegateFlags.delegateVideoNodeDidSetCurrentItem =
        [delegate respondsToSelector:@selector(videoNode:didSetCurrentItem:)];
    _delegateFlags.delegateVideoNodeDidStallAtTimeInterval =
        [delegate respondsToSelector:@selector(videoNode:didStallAtTimeInterval:)];
    _delegateFlags.delegateVideoNodeDidRecoverFromStall =
...
- (void)videoNodeDidStall:(NSNotification *)notification {
  ...
  if (_delegateFlags.delegateVideoNodeDidStallAtTimeInterval) {
    [self.delegate videoNode:self
        didStallAtTimeInterval:CMTimeGetSeconds(_player.currentItem.currentTime)];
  }
}
  ...
  if ([notification.name isEqualToString:AVPlayerItemFailedToPlayToEndTimeNotification]) {
    ...
    NSLog(@"AVPlayerItem error log entry added for video with error %@ status %@", item.error,
          (item.status == AVPlayerItemStatusFailed
               ? @"FAILED"
               : [NSString stringWithFormat:@"%ld", (long)item.status]));
    NSLog(@"Item is %@", item);

    if (logEvent) {
      NSLog(@"Log code %ld domain %@ comment %@", (long)logEvent.errorStatusCode,
            logEvent.errorDomain, logEvent.errorComment);
    }
  }
}
