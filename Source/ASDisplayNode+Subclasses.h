...
/**
 * The subclass header _ASDisplayNode+Subclasses_ defines the following methods that either must or
 * can be overriden by subclasses of ASDisplayNode.
 *
 * These methods should never be called directly by other classes.
 *
 * ## Drawing
 *
 * Implement one of +displayWithParameters:isCancelled: or +drawRect:withParameters:isCancelled: to
 * provide drawing for your node.
 *
 * Use -drawParametersForAsyncLayer: to copy any properties that are involved in drawing into an
 * immutable object for use on the display queue. The display and drawRect implementations *MUST* be
 * thread-safe, as they can be called on the displayQueue (asynchronously) or the main thread
 * (synchronously/displayImmediately).
 *
 * Class methods that require passing in copies of the values are used to minimize the need for
 * locking around instance variable access, and the possibility of the asynchronous display pass
 * grabbing an inconsistent state across multiple variables.
 */
 ...
 *
 * @discussion For node subclasses that implement manual layout (e.g., they have a custom -layout
 * method), calculatedLayout may be accessed on subnodes to retrieved cached information about their
 * size. This allows -layout to be very fast, saving time on the main thread. Note:
 * .calculatedLayout will only be set for nodes that have had -layoutThatFits: called on them. For
 * manual layout, make sure you call -layoutThatFits: in your implementation of
 ...
 * and the ASDisplayNode base class implementation of -layout will automatically make use of
 * .calculatedLayout on the subnodes.
 *
 * @return Layout that wraps calculated size returned by -calculateSizeThatFits: (in manual layout
 * mode), or layout already calculated from layout spec returned by -layoutSpecThatFits: (in
 * automatic layout mode).
 *
 * @warning Subclasses must not override this; it returns the last cached layout and is never
 * expensive.
 */
 ...
 *
 * @discussion Gives a chance for subclasses to perform actions after the subclass and superclass
 * have finished laying out.
 */
AS_CATEGORY_IMPLEMENTABLE
- (void)layoutDidFinish ASDISPLAYNODE_REQUIRES_SUPER;

/**
 * @abstract Called on a background thread if !isNodeLoaded - called on the main thread if
 * isNodeLoaded.
 *
 * @discussion When the .calculatedLayout property is set to a new ASLayout (directly from
 * -calculateLayoutThatFits: or calculated via use of -layoutSpecThatFits:), subclasses may inspect
 * it here.
 */
AS_CATEGORY_IMPLEMENTABLE
 ...
 *
 * @discussion This method is called on a non-main thread. The default implementation calls either
 * -layoutSpecThatFits: or -calculateSizeThatFits:, whichever method is overriden. Subclasses rarely
 * need to override this method, override -layoutSpecThatFits: or -calculateSizeThatFits: instead.
 *
 * @note This method should not be called directly outside of ASDisplayNode; use -layoutThatFits: or
 * -calculatedLayout instead.
 */
- (ASLayout *)calculateLayoutThatFits:(ASSizeRange)constrainedSize;

/**
 * ASDisplayNode's implementation of -layoutThatFits:parentSize: calls this method to resolve the
 * node's size against parentSize, intersect it with constrainedSize, and call
 * -calculateLayoutThatFits: with the result.
 *
 * In certain advanced cases, you may want to customize this logic. Overriding this method allows
 * you to receive all three parameters and do the computation yourself.
 *
 ...
 *
 * @discussion Subclasses that override should expect this method to be called on a non-main thread.
 * The returned size is wrapped in an ASLayout and cached for quick access during -layout. Other
 * expensive work that needs to be done before display can be performed here, and using ivars to
 * cache any valuable intermediate results is encouraged.
 *
 * @note Subclasses that override are committed to manual layout. Therefore, -layout: must be
 * overriden to layout all subnodes or subviews.
 *
 * @note This method should not be called directly outside of ASDisplayNode; use -layoutThatFits: or
 ...
 *
 * @discussion Subclasses should call this method to invalidate the previously measured and cached
 * layout for the display node, when the contents of the node change in such a way as to require
 * measuring it again.
 */
...
/**
 * Declare <ASInterfaceStateDelegate> methods as requiring super calls (this can't be required in
 * the protocol). For descriptions, see <ASInterfaceStateDelegate> definition.
 */
...
/**
 * @summary Delegate method to draw layer contents into a CGBitmapContext. The current UIGraphics
 * context will be set to an appropriate context.
 *
 ...
 * @param isCancelledBlock Execute this block to check whether the current drawing operation has
 * been cancelled to avoid unnecessary work. A return value of YES means cancel drawing and return.
 * @param isRasterizing YES if the layer is being rasterized into another layer, in which case
 * drawRect: probably wants to avoid doing things like filling its bounds with a zero-alpha color to
 ...
 * been cancelled to avoid unnecessary work. A return value of YES means cancel drawing and return.
 *
 * @return A UIImage with contents that are ready to display on the main thread. Make sure that the
...
+ (nullable UIImage *)displayWithParameters:(nullable id)parameters
                                isCancelled:
                                    (AS_NOESCAPE asdisplaynode_iscancelled_block_t)isCancelledBlock;
 ...
 *
 * @discussion Subclasses may override this method to be notified when display (asynchronous or
 * synchronous) is about to begin.
 *
 * @note Called on the main thread only
 */
...
/**
 * Provides an opportunity to clear backing store and other memory-intensive intermediates, such as
 * text layout managers on the current node.
 *
 * @discussion Called by -recursivelyClearContents. Always called on main thread. Base class
 * implements self.contents = nil, clearing any backing store, for asynchronous regeneration when
 * needed.
 */
- (void)clearContents ASDISPLAYNODE_REQUIRES_SUPER;

/**
 * @abstract Indicates that the receiver is about to display its subnodes. This method is not called
 * if there are no subnodes present.
 *
 * @param subnode The subnode of which display is about to begin.
 *
 * @discussion Subclasses may override this method to be notified when subnode display (asynchronous
 * or synchronous) is about to begin.
 */
- (void)subnodeDisplayWillStart:(ASDisplayNode *)subnode ASDISPLAYNODE_REQUIRES_SUPER;

/**
 * @abstract Indicates that the receiver is finished displaying its subnodes. This method is not
 * called if there are no subnodes present.
 *
 * @param subnode The subnode of which display is about to completed.
 *
 * @discussion Subclasses may override this method to be notified when subnode display (asynchronous
 * or synchronous) has completed.
 */
 ...
 *
 * @discussion Subclasses may override this if they require modifying the scale set on their child
 * nodes.
 *
 ...
 *
 * @discussion Use setNeedsDisplayAtScale: to set a value and then after display, the display node
 * will set the layer's contentsScale. This is to prevent jumps when re-rasterizing at a different
 * contentsScale. Read this property if you need to know the future contentsScale of your layer, eg
 * in drawParameters.
 *
 * @see setNeedsDisplayAtScale:
 */
 ...
 *
 * @discussion Override to make this node respond differently to touches: (e.g. hide touches from
 * subviews, send all touches to certain subviews (hit area maximizing), etc.)
 *
...
/**
 * @abstract Optionally provide an image to serve as the placeholder for the backing store while the
 * contents are being displayed.
 *
 * @discussion
 * Subclasses may override this method and return an image to use as the placeholder. Take caution
 * as there may be a time and place where this method is called on a background thread. Note that
 * -[UIImage imageNamed:] is not thread safe when using image assets.
 *
...
#define __ASDisplayNodeCheckForLayoutMethodOverrides                                               \
  ASDisplayNodeAssert(                                                                             \
      _layoutSpecBlock != NULL ||                                                                  \
          ((ASDisplayNodeSubclassOverridesSelector(self.class, @selector(calculateSizeThatFits:))  \
                ? 1                                                                                \
                : 0) +                                                                             \
           (ASDisplayNodeSubclassOverridesSelector(self.class, @selector(layoutSpecThatFits:))     \
                ? 1                                                                                \
                : 0) +                                                                             \
           (ASDisplayNodeSubclassOverridesSelector(self.class,                                     \
                                                   @selector(calculateLayoutThatFits:))            \
                ? 1                                                                                \
                : 0)) <= 1,                                                                        \
      @"Subclass %@ must at least provide a layoutSpecBlock or override at most one of the three " \
      @"layout methods: calculateLayoutThatFits:, layoutSpecThatFits:, or calculateSizeThatFits:", \
      NSStringFromClass(self.class))

#define ASDisplayNodeAssertThreadAffinity(viewNode)                                              \
  ASDisplayNodeAssert(                                                                           \
      !viewNode || ASMainThreadAssertionsAreDisabled() || ASDisplayNodeThreadIsMain() ||         \
          !(viewNode).nodeLoaded,                                                                \
      @"Incorrect display node thread affinity - this method should not be called off the main " \
      @"thread after the ASDisplayNode's view or layer have been created")
