...
- (void)_locked_setAnimatedImage:(id<ASAnimatedImageProtocol>)animatedImage {
  ...
  if (ASObjectIsEqual(_animatedImage, animatedImage) &&
      (animatedImage == nil || animatedImage.playbackReady)) {
    return;
  }
  ...
  if (animatedImage != nil) {
    ...
    // Don't bother using a `_locked` version for setting contnst as it should be pretty safe
    // calling it with reaquire the lock and would add overhead to introduce this version
    self.contents = nil;
    [self _locked_setCoverImage:nil];
  }

  // Push calling subclass to the next runloop cycle
  // We have to schedule the block on the common modes otherwise the tracking mode will not be
  // included and it will not fire e.g. while scrolling down
...
- (void)_locked_setShouldAnimate:(BOOL)shouldAnimate {
  ...
  // This test is explicitly done and not ASPerformBlockOnMainThread as this would perform the block
  // immediately on main if called on main thread and we have to call methods locked or unlocked
  // based on which thread we are on
  if (ASDisplayNodeThreadIsMain()) {
    ...
    // We have to dispatch to the main thread and call the regular methods as the lock is already
    // gone if the block is called
...
- (void)_locked_startAnimating {
  ...
  // It should be safe to call self.interfaceState in this case as it will only grab the lock of the
  // superclass
...
- (void)didExitDisplayState {
  ...
  // Also clear out the contents we've set to be good citizens, we'll put it back in when we become
  // visible.
  if (isAnimatedImage) {
    self.contents = nil;
...
- (NSUInteger)frameIndexAtPlayHeadPosition:(CFTimeInterval)playHead {
  ...
  for (NSUInteger durationIndex = 0; durationIndex < self.animatedImage.frameCount;
       durationIndex++) {
