...
typedef NS_ENUM(NSUInteger, ASCellNodeVisibilityEvent) {
  ...
  /**
   * Its position (determined by scrollView.contentOffset) has changed while at least 1px remains
   * visible. It is possible that 100% of the cell is visible both before and after and only its
   * position has changed, or that the position change has resulted in more or less of the cell
   * being visible. Use CGRectIntersect between cellFrame and scrollView.bounds to get this
   * rectangle
   */
...
/**
 * Generic cell node.  Subclass this instead of `ASDisplayNode` to use with `ASTableView` and
 `ASCollectionView`.
 ...
 *
 * @default NO
 * @discussion Normally, ASCellNodes are preloaded and have finished display before they are
 * onscreen. However, if the Table or Collection's rangeTuningParameters are set to small values (or
 * 0), or if the user is scrolling rapidly on a slow device, it is possible for a cell's display to
 ...
 *
 * @return The supplementary element kind, or @c nil if this node does not represent a supplementary
 * element.
 */
 ...
 *
 * @discussion This property is useful because it is set before @c
 * collectionView:willDisplayNode:forItemAtIndexPath: is called, when the node is not yet in the
 * hierarchy and its frame cannot be converted to/from other nodes. Instead you can use the layout
 * attributes object to learn where and how the cell will be displayed.
 */
...
/**
 * A Boolean value that is synchronized with the underlying collection or tableView cell property.
 * Setting this value is equivalent to calling highlightItem / unHighlightItem on the collection or
 * table.
 */
...
/**
 * BETA: API is under development. We will attempt to provide an easy migration pathway for any
 * changes.
 *
 * The view-model currently assigned to this node, if any.
 *
 * This property may be set off the main thread, but this method will never be invoked concurrently
 * on the
 */
...
/**
 * The backing view controller, or @c nil if the node wasn't initialized with backing view
 * controller
...
/*
 * ASCellNode must forward touch events in order for UITableView and UICollectionView tap handling
 * to work. Overriding these methods (e.g. for highlighting) requires the super method be called.
 */
...
/**
 * Called by the system when ASCellNode is used with an ASCollectionNode.  It will not be called by
 * ASTableNode. When the UICollectionViewLayout object returns a new
 * UICollectionViewLayoutAttributes object, the corresponding ASCellNode will be updated. See
 * UICollectionViewCell's applyLayoutAttributes: for a full description.
 */
 ...
 *
 * @return An ASCellNode created using the root view of the view controller provided by the
 * viewControllerBlock. The view controller's root view is resized to match the calculated size
 * produced during layout.
 *
 */
 ...
 * ASTableView uses these properties when configuring UITableViewCells that host ASCellNodes.
 * ASCollectionView uses these properties when configuring UICollectionViewCells that host
 * ASCellNodes.
 */
...
/* @abstract The accessory type view on the right side of the cell. Please take care of your
 * ASLayoutSpec so that doesn't overlay the accessoryView
...
- (void)setLayerBacked:(BOOL)layerBacked
    AS_UNAVAILABLE(
        "ASCellNode does not support layer-backing, although subnodes may be layer-backed.");
...
/**
 * A dictionary containing key-value pairs for text attributes. You can specify the font, text
 * color, text shadow color, and text shadow offset using the keys listed in NSString UIKit
 * Additions Reference.
 */
@property(copy) NSDictionary<NSAttributedStringKey, id> *textAttributes;

/**
 * The text inset or outset for each edge. The default value is 15.0 horizontal and 11.0 vertical
 * padding.
 */
